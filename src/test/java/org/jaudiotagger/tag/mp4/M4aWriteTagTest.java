...
public class M4aWriteTagTest extends TestCase {
	...
	public void testWriteFileSameSize() {
		...
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.m4a", Paths.get("testWriteFileSameSize.m4a"));
			...
			assertEquals(TEST_FILE1_SIZE, Files.size(testFile));
			// AudioInfo
			// Time in seconds
			...
			final Mp4TagReverseDnsField rvs = (Mp4TagReverseDnsField) mp4tag.getFirstField(Mp4FieldKey.MUSICBRAINZ_ALBUMID);
			...
			final List coverart = mp4tag.get(Mp4FieldKey.ARTWORK);
			// Should be one image
			assertEquals(1, coverart.size());

			final Mp4TagCoverField coverArtField = (Mp4TagCoverField) coverart.get(0);
			// Check type jpeg
	...
	public void testWriteFileSmallerSize() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.m4a", Paths.get("testWriteFileSmallerSize.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
	...
	/**
	 * Test to write tag data, new tagdata is alrger size than existing data, but not so large that it cant fit into the
	 * sapce already allocated to meta (ilst + free atom)
	 */
	public void testWriteFileLargerSize() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.m4a", Paths.get("testWriteFileLargerSize.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
	...
	/**
	 * Test to write tag data, new tagdata is a larger size than existing data, and too large to fit into the space
	 * already allocated to meta (ilst + free atom) but can fit into the second free atom
	 */
	public void testWriteFileAlotLargerSize() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.m4a", Paths.get("testWriteFileAlot.m4a"));
			...
			// Add new image
			final RandomAccessFile imageFile = new RandomAccessFile(AbstractTestCase.dataPath.resolve("coverart_small.png").toFile(), "r");
			...
			assertEquals(3883555, Files.size(testFile));
	...
	/**
	 * Test to write tag data, new tagdata is a larger size than existing data, and too large to fit into the space
	 * already allocated to meta (ilst + free atom) and is even too large to fit into the second free atom, so mdat data
	 * gets moved
	 */
	public void testWriteFileMuchLargerSize() {
		...
		try (final RandomAccessFile imageFile = new RandomAccessFile(AbstractTestCase.dataPath.resolve("coverart.png").toFile(), "r")) {
			...
			assertEquals(3901001, Files.size(testFile));
	...
	public void testDeleteTag() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.m4a", Paths.get("testDeleteMeta.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
	...
	public void testDeleteTag2() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.m4a", Paths.get("testDeleteMeta2.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			AudioFileIO.delete(f);
	...
	public void testWriteFileSameSizeNoMetaFreeAtom() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.m4a", Paths.get("testWriteFileSameSizeNoMetaFree.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
			...
			// Total FileSize should not have changed
			assertEquals(TEST_FILE2_SIZE, Files.size(testFile));
			// AudioInfo
			// Time in seconds
			assertEquals(241, f.getAudioHeader().getTrackLength());
			assertEquals(44100, f.getAudioHeader().getSampleRateAsNumber());
	...
	public void testWriteFileSmallerSizeMoreThanEightBytesSmallerNoMetaFreeAtom() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.m4a", Paths.get("testWriteFileSmallerSizeNoMetaFreeMoreThanEight.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
	...
	public void testWriteFileSmallerSizeLessThanEightBytesNoMetaFreeAtom() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.m4a", Paths.get("testWriteFileLessThanEight2.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
			...
			// Total FileSize should not have changed
			assertEquals(TEST_FILE2_SIZE - 7, Files.size(testFile));
	...
	public void testWriteFileSmallerSizeLessThanEightBytesNoFreeAtoms() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test8.m4a", Paths.get("testWriteFileLessThanEight3.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
	...
	public void testWriteFileSmallerNoFreeAtoms() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test8.m4a", Paths.get("testWriteFileNoFreeAtom2.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
	...
	public void testWriteFileLargerSizeNoMetaFreeAtom() {
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.m4a", Paths.get("testWriteFileLargerSizeNoMetaFree.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
	...
	public void testWriteFileWhichDoesntHaveAMetadataAtom() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test4.m4a", Paths.get("testWriteNewMetadata.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
			...
			final List coverart = tag.getFields(FieldKey.COVER_ART);
			assertEquals(2, coverart.size());
	...
	public void testWriteFileLargerSizeNoTopLevelFreeAtom() {
		final File orig = new File("testdata", "test6.m4p");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test6.m4a", Paths.get("testWriteNoTopFree.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
			...
			// Total FileSize should not have changed
			assertEquals(5208679, Files.size(testFile));
	...
	public void testWriteFileLargerSizeEqualToTopLevelFreeSpace() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.m4a", Paths.get("testWriteFileEqualToFreeSpace.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();

			// Frig adding pretend image which will require exactly the same size as space available in top level atom ,
			// (there is
			// no meta atom) in test3.m4a
			final byte[] imagedata = new byte[2032];
	...
	public void testWriteAllFields() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test5.m4a", Paths.get("testWriteAllFields.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();

			assertEquals(TEST_FILE5_SIZE, Files.size(testFile));
	...
	public void testWriteGenres() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test5.m4a", Paths.get("testWriteGenres.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Mp4Tag tag = (Mp4Tag) f.getTag();
	...
	public void testWriteCustomGenresAlways() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test5.m4a", Paths.get("testWriteCustomGenres.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Mp4Tag tag = (Mp4Tag) f.getTag();
	...
	public void testWriteWhenFreeisAfterMdat() {
		Exception exceptionCaught = null;
		try {
			final File orig = new File("testdata", "unable_to_write.m4p");
			if (!orig.isFile())
				return;
			final Path testFile = AbstractTestCase.copyAudioToTmp("unable_to_write.m4p");
	...
	public void testWriteFileLargerSizeLessThanTopLevelFreeWhenFreeAafterMdat() {
		Exception exceptionCaught = null;
		try {
			final Path orig = AbstractTestCase.dataPath.resolve("unable_to_write.m4p");
			...
			// free atoms
			final byte[] imagedata = new byte[1340];
			imagedata[0] = (byte) 0x89;
			imagedata[1] = (byte) 0x50;
			imagedata[2] = (byte) 0x4E;
			imagedata[3] = (byte) 0x47;
	...
	public void testWriteFileLargerSizeEqualToTopLevelFreeWhenFreeAafterMdat() {
		...
		try {
			...
			// Frig adding pretend image which will require exactly the same size as space available in the two
			// free atoms
			final byte[] imagedata = new byte[1349];
			imagedata[0] = (byte) 0x89;
			imagedata[1] = (byte) 0x50;
			imagedata[2] = (byte) 0x4E;
			imagedata[3] = (byte) 0x47;
	...
	public void testWritingIssue198() throws Exception {
		final File orig = new File("testdata", "test27.m4a");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test27.m4a", Paths.get("rvdnswithoutdata.m4a"));
	...
	public void testWriteMultipleFields() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("test.m4a", Paths.get("testWriteMultiple.m4a"));
		AudioFile f = AudioFileIO.read(testFile);
