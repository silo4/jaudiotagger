...
public class M4aWriteDrmTagTest extends TestCase {
	...
	public void testShowStco() throws Exception {
		final File orig = new File("testdata", "test9.m4p");
		if (!orig.isFile())
			return;
		final Path testFile = AbstractTestCase.copyAudioToTmp("test9.m4p", Paths.get("WriteDrmFile1.m4p"));
	...
	/**
	 * Test to write all metadata from an Apple iTunes encoded mp4 file, note also uses fixed genre rather than custom
	 * genre
	 */
