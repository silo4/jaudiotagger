...
public class M4aWriteDatatoMultTrackAudioTest extends TestCase {
	...
	public void testWriteFileOption1SameSize() {
		...
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test16.m4a", Paths.get("testWriteMultiTrack1.m4a"));

			// First lets just createField tree
			Mp4AtomTree atomTree = new Mp4AtomTree(new RandomAccessFile(testFile.toFile(), "r"));
			...
			// See tree again
			atomTree = new Mp4AtomTree(new RandomAccessFile(testFile.toFile(), "r"));
			atomTree.printAtomTree();
	...
	public void testWriteFileOption3SmallerSizeCreateFree() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test16.m4a", Paths.get("testWriteMultiTrack3.m4a"));
	...
	public void testWriteFileOption4SmallerSizeNoFree() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test16.m4a", Paths.get("testWriteMultiTrack4.m4a"));
	...
	public void testWriteFileOption8CannoutUseTopLevelFree() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test16.m4a", Paths.get("testWriteMultiTrack8.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
	...
	public void testWriteFileOption6LargerCanUseTopLevelFree() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test21.m4a", Paths.get("testWriteMultiTrack6.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
	...
	public void testWriteFileOption7LargerCanUseTopLevelFree() {
		Exception exceptionCaught = null;
		try {
			Path testFile = AbstractTestCase.copyAudioToTmp("test21.m4a", Paths.get("testWriteMultiTrack7.m4a"));
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
			...
			// free atom
			final byte[] imagedata = new byte[822];
