...
public class M4aReadTagTest extends TestCase {
	...
	public void testReadFile() {
		...
		try {
			...
			// MPEG Specific
			final Mp4AudioHeader audioheader = (Mp4AudioHeader) f.getAudioHeader();
			...
			final List<TagField> coverart = mp4tag.get(Mp4FieldKey.ARTWORK);
			// Should be one image
			...
			// Recreate the image
			final BufferedImage bi = ImageIO.read(ImageIO.createImageInputStream(new ByteArrayInputStream(coverArtField.getData())));
			assertNotNull(bi);
	...
	public void testReadFileFromMediaMonkey306() {
		final Path orig = AbstractTestCase.dataPath.resolve("test38.m4a");
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test38.m4a");
			final AudioFile f = AudioFileIO.read(testFile);
			final Tag tag = f.getTag();
	...
	public void testReadFileFromWinamp5531() {
		final Path orig = AbstractTestCase.dataPath.resolve("test39.m4a");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test39.m4a");
			final AudioFile f = AudioFileIO.read(testFile);
			final Tag tag = f.getTag();
	...
	/**
	 * Test to read all metadata from an Apple iTunes encoded m4a file , this tests a few items that could not be tested
	 * with first test. Namely genre picked from list, and png item instead of jpg
	 * <p/>
	...
	public void testReadFile2() {
		...
		try {
			...
			// Recreate the image
			bi = ImageIO.read(ImageIO.createImageInputStream(new ByteArrayInputStream(coverArtField.getData())));
			assertNotNull(bi);
	...
	public void testReadFileWithNoMetaFreeAtom() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.m4a");
			final AudioFile f = AudioFileIO.read(testFile);
			final Tag tag = f.getTag();
	...
	public void testDetectMultiTrackAudio() {
		final File orig = new File("testdata", "test7.mp4");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test7.mp4");
			final Mp4AtomTree tree = new Mp4AtomTree(new RandomAccessFile(testFile.toFile(), "r"), false);
			tree.printAtomTree();
	...
	public void testDetectMultiTrackAudio2() {
		final File orig = new File("testdata", "test86.mp4");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test86.mp4");
			final Mp4AtomTree tree = new Mp4AtomTree(new RandomAccessFile(testFile.toFile(), "r"), false);
			tree.printAtomTree();
	...
	public void testDetectVideo() {
		final File orig = new File("testdata", "test87.mp4");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test87.mp4");
	...
	public void testMonoLowbitRateReadFile() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test5.m4a");
			final AudioFile f = AudioFileIO.read(testFile);
			final Tag tag = f.getTag();
	...
	public void testIssue156() throws Exception {
		Exception exceptionCaught = null;
		try {
			final File orig = new File("testdata", "test13.m4a");
			if (!orig.isFile())
				return;
			final Path testFile = AbstractTestCase.copyAudioToTmp("test13.m4a");
		...
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.m4a", Paths.get("testIssue156.m4a"));
			final AudioFile f = AudioFileIO.read(testFile);
			final Tag tag = f.getTag();
	...
	public void testIssue163() {
		...
		try {
			...
			final File orig = new File("testdata", "unable_to_read.m4a");
			if (!orig.isFile())
				return;
			final Path testFile = AbstractTestCase.copyAudioToTmp("unable_to_read.m4a");
	...
	public void testIssue168() throws Exception {
		Exception exceptionCaught = null;
		try {
			final File orig = new File("testdata", "test14.m4a");
			if (!orig.isFile())
				return;
			final Path testFile = AbstractTestCase.copyAudioToTmp("test14.m4a");
	...
	public void testIssue182() throws Exception {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test16.m4a");

			final AudioFile f = AudioFileIO.read(testFile);
			final Mp4Tag tag = (Mp4Tag) f.getTag();
	...
	public void testIssue198() throws Exception {
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test27.m4a");
	...
	public void testIssue227() throws Exception {
		final File orig = new File("testdata", "test31.m4a");
		if (!orig.isFile())
			return;
		...
		try {
			// Read Image
			final Path testFile = AbstractTestCase.copyAudioToTmp("test31.m4a");
	...
	public void testIssue226Mono() throws Exception {
		...
		try {
			// Read Image
			final Path testFile = AbstractTestCase.copyAudioToTmp("test32.m4a");
	...
	public void testIssue226Stereo() throws Exception {
		...
		try {
			// Read Image
			final Path testFile = AbstractTestCase.copyAudioToTmp("test33.m4a");
	...
	public void testNumericGenres() throws Exception {
		final Path orig = AbstractTestCase.dataPath.resolve("test75.m4a");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		try {
			...
			// Read Image
			final Path testFile = AbstractTestCase.copyAudioToTmp("test75.m4a");
			final RandomAccessFile raf = new RandomAccessFile(testFile.toFile(), "r");
			final Mp4Tag tagReader = new Mp4TagReader().read(raf);
	...
	public void testReadFile3() throws Exception {
		final Path orig = AbstractTestCase.dataPath.resolve("test84.m4a");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test84.m4a");
			AudioFile f = AudioFileIO.read(testFile);
			Tag tag = f.getTag();
	...
	public void testReadFile4() throws Exception {
		final Path orig = AbstractTestCase.dataPath.resolve("test86.m4a");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test86.m4a");
			final AudioFile f = AudioFileIO.read(testFile);
			final Tag tag = f.getTag();
