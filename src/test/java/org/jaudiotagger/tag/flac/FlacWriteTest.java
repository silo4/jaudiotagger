...
public class FlacWriteTest extends TestCase {
	...
	public void testWriteAllFieldsToFile() {
		...
		try {
			...
			final Path testFile = AbstractTestCase.copyAudioToTmp("test2.flac", Paths.get("test2write.flac"));
			...
			final MetadataBlockDataPicture pic = tag.getImages().get(0);
	...
	public void testDeleteTagFile() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.flac", Paths.get("testdeletetag.flac"));
			AudioFile f = AudioFileIO.read(testFile);
	...
	public void testWriteFileWithCueSheet() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.flac", Paths.get("testWriteWithCueSheet.flac"));
			AudioFile f = AudioFileIO.read(testFile);
	...
	public void testWriteFileWithId3Header() {
		...
		try {
			...
			final Path testFile = AbstractTestCase.copyAudioToTmp("test22.flac", Paths.get("testWriteFlacWithId3.flac"));
			AudioFile f = AudioFileIO.read(testFile);
			FlacInfoReader infoReader = new FlacInfoReader();
			assertEquals(4, infoReader.countMetaBlocks(f.getFile()));
			f.getTag().setField(FieldKey.ALBUM, "BLOCK");
	...
	public void testWriteFileWithId3HeaderAudioShifted() {
		...
		try {
			...
			final Path testFile = AbstractTestCase.copyAudioToTmp("test22.flac", Paths.get("testWriteFlacWithId3Shifted.flac"));
			AudioFile f = AudioFileIO.read(testFile);
	...
	public void testDeleteTag() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("test2.flac", Paths.get("testDelete.flac"));
		AudioFile f = AudioFileIO.read(testFile);
		AudioFileIO.delete(f);
	...
	public void testWriteMultipleFields() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("test.flac", Paths.get("testWriteMultiple.flac"));
		AudioFile f = AudioFileIO.read(testFile);
