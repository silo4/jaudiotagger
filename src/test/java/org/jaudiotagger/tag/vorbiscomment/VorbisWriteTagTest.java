...
public class VorbisWriteTagTest extends AbstractTestCase {
	...
	public void testSummarizeFile() {
		...
		try {
			// Can summarize file
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.ogg", Paths.get("testSummarizeFile.ogg"));
			final RandomAccessFile raf = new RandomAccessFile(testFile.toFile(), "r");
			final OggFileReader oggFileReader = new OggFileReader();
	...
	public void testWriteTagToFile() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.ogg", Paths.get("testWriteTagTest.ogg"));
			...
			final VorbisCommentTag vorbisTag = tag;
			...
			// List methods
			final List<TagField> list = tag.getFields(FieldKey.ARTIST);
			assertEquals(1, list.size());
			for (final TagField field : list)
			...
			final BufferedImage bi = ImageIO.read(ImageIO.createImageInputStream(new ByteArrayInputStream(Base64Coder.decode(vorbisTag.getFirst(VorbisCommentFieldKey.COVERART).toCharArray()))));
			assertNotNull(bi);

			final OggFileReader ofr = new OggFileReader();
			final OggPageHeader oph = ofr.readOggPageHeader(new RandomAccessFile(testFile.toFile(), "r"), 0);
	...
	public void testWriteToFileMuchLarger() {
		final File orig = new File("testdata", "test.ogg");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.ogg", Paths.get("testWriteTagTestRequiresTwoPages.ogg"));
	...
	public void testWriteToFileMuchLargerSetupHeaderSplit() {
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.ogg", Paths.get("testWriteTagTestRequiresTwoPagesHeaderSplit.ogg"));
	...
	public void testWriteToFileWithExtraPacketsOnSamePageAsSetupHeader() {
		final File orig = new File("testdata", "test2.ogg");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test2.ogg", Paths.get("testWriteTagWithExtraPacketsHeaderSameSize.ogg"));
			...
			final int packetsInSecondPageCount = pageHeader.getPacketList().size();
			pageHeader = null;
			raf.close();
	...
	public void testWriteToFileWithExtraPacketsOnSamePageAsSetupHeaderLarger() {
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test2.ogg", Paths.get("testWriteTagWithExtraPacketsHeaderLargerSize.ogg"));
	...
	public void testWriteToFileWithExtraPacketsOnSamePageAsSetupHeaderMuchLarger() {
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test.ogg", Paths.get("testWriteTagWithExtraPacketsHeaderMuchLargerSize.ogg"));
	...
	/**
	 * Issue 197, test writing to file when audio packet come straight after setup packet on same oggPage, edit enough
	 * so that comment is much larger, so that comment, header and extra packets can no longer fit on page 2 AND setup
	 * header is also split over two
	 */
	public void testWriteToFileWithExtraPacketsOnSamePageAsSetupHeaderMuchLargerAndSplit() {
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test2.ogg", Paths.get("testWriteTagWithExtraPacketsHeaderMuchLargerSizeAndSplit.ogg"));

			AudioFile f = AudioFileIO.read(testFile);
			assertTrue(f.getTag() instanceof VorbisCommentTag);
			final VorbisCommentTag tag = (VorbisCommentTag) f.getTag();
	...
	public void testWriteToFileNoLongerRequiresTwoPages() {
		final File orig = new File("testdata", "test3.ogg");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.ogg", Paths.get("testWriteTagTestNoLongerRequiresTwoPages.ogg"));
	...
	public void testWriteToFileNoLongerRequiresTwoPagesNorSplit() {
		final File orig = new File("testdata", "test5.ogg");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test5.ogg", Paths.get("testWriteTagTestNoLongerRequiresTwoPagesNorSplit.ogg"));
	...
	public void testWriteToFileWriteToFileWithExtraPacketsNoLongerRequiresTwoPages() {
		final File orig = new File("testdata", "test4.ogg");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test4.ogg", Paths.get("testWriteTagTestWithPacketsNoLongerRequiresTwoPages.ogg"));
	...
	public void testDeleteTag() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("test.ogg", Paths.get("testDelete.ogg"));
		AudioFile f = AudioFileIO.read(testFile);
	...
	public void testDeleteTag2() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("test.ogg", Paths.get("testDelete2.ogg"));
		AudioFile f = AudioFileIO.read(testFile);
	...
	public void testWriteMultipleFields() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("test.ogg", Paths.get("testWriteMultiple.ogg"));
		AudioFile f = AudioFileIO.read(testFile);
		f.getTag().addField(FieldKey.ALBUM_ARTIST, "artist1");
		f.getTag().addField(FieldKey.ALBUM_ARTIST, "artist2");
		f.commit();
		f = AudioFileIO.read(testFile);
		final List<TagField> tagFields = f.getTag().getFields(FieldKey.ALBUM_ARTIST);
		assertEquals(tagFields.size(), 2);
	}

	public void testDeleteFields() throws Exception {
		// Delete using generic key
		final Path testFile = AbstractTestCase.copyAudioToTmp("test.ogg", Paths.get("testDeleteFields.ogg"));
		AudioFile f = AudioFileIO.read(testFile);
