...
public abstract class WmaTestCase extends TestCase {
	 ...
	 * 
	 * @param sourceFile
	 *            The filename of the file to perform tests on. ({@linkplain AbstractTestCase#copyAudioToTmp(String)
	 ...
	 * @throws Exception
	 *             Upon IO errors, or ASF parsing faults.
	 */
	 ...
	 * @throws Exception
	 *             from call of {@link #getAudioFile()}.
	 */
	 ...
	 */
	public Path prepareTestFile(final String fileName) {
		assertNotNull(sourceTestFile);
		Path result = null;
		if (!Utils.isBlank(fileName))
			result = AbstractTestCase.copyAudioToTmp(sourceTestFile, Paths.get(fileName));
		else
