...
public class WmaSimpleTest extends AbstractTestCase {
	public void testReadFileFromPicardQtInvalidHeaderSizeException() {
		final Path orig = AbstractTestCase.dataPath.resolve("test2.wma");
		...
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test2.wma");
	 ...
	 * <p/>
	 * Checking our fields match the fields used by media Monkey 3 (Defacto Standard) by ensuring we can read fields
	 * written in Media Monkey
	 */
	public void testReadFileFromMediaMonkey3() {
		...
		try {
			...
			final AsfTag tag = (AsfTag) f.getTag();
	 ...
	 * <p/>
	 * Checking our fields match the fields used by picard Qt3 (Defacto Standard for Musicbrainz fields) by ensuring we
	 * can read fields written in Picard Qt
	 */
	...
	public void testWriteFile() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test1.wma", Paths.get("testwrite1.wma"));
		...
		try {
			...
			for (int i = 0; i < 2; i++) {
				...
				for (final FieldKey key : FieldKey.values())
					 ...
					 */
					if (!(key == FieldKey.COVER_ART)) {
						final String value = key.name() + "_value_" + i;
	...
	public void testReadFileWithHeaderSizeDoesntMatchFileSize() {
		final Path orig = AbstractTestCase.dataPath.resolve("test3.wma");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test3.wma");
			final AudioFile f = AudioFileIO.read(testFile);
	...
	public void testReadFileWithGifArtwork() {
		...
		try {
			...
			final TagField tagField = tag.getFields(FieldKey.COVER_ART).get(0);
			...
			final AsfTagCoverField coverartField = (AsfTagCoverField) tagField;
			...
			final BufferedImage bi = ImageIO.read(ImageIO.createImageInputStream(new ByteArrayInputStream(tagField.getRawContent(), endOfName, tagField.getRawContent().length - endOfName)));
			assertNotNull(bi);
	...
	public void testReadFileWithGifArtworkNoDescription() {
		final Path orig = AbstractTestCase.dataPath.resolve("test4.wma");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test4.wma");
			final AudioFile f = AudioFileIO.read(testFile);
			final Tag tag = f.getTag();
			assertEquals(1, tag.getFields(FieldKey.COVER_ART).size());
	...
	public void testReadFileWithPngArtwork() {
		final Path orig = AbstractTestCase.dataPath.resolve("test5.wma");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test5.wma");
			final AudioFile f = AudioFileIO.read(testFile);
			final Tag tag = f.getTag();
			assertEquals(1, tag.getFields(FieldKey.COVER_ART).size());
	...
	public void testReadFileWithJpgArtwork() {
		final Path orig = AbstractTestCase.dataPath.resolve("test6.wma");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("test6.wma");
			final AudioFile f = AudioFileIO.read(testFile);
			final Tag tag = f.getTag();
			assertEquals(1, tag.getFields(FieldKey.COVER_ART).size());
	...
	public void testWritePngArtworkToFile() {
		...
		/*
		 * TODO Why does it fail Exception exceptionCaught = null; try { Path testFile =
		 * AbstractTestCase.copyAudioToTmp("test7.wma"); AudioFile f = AudioFileIO.read(testFile); Tag tag = f.getTag();
		 * assertEquals(0, tag.getFields(FieldKey.COVER_ART).size());
		 * 
		 * //Now createField artwork field RandomAccessFile imageFile = new RandomAccessFile(new File("testdata",
		 * "coverart.png"), "r"); byte[] imagedata = new byte[(int) imageFile.length()]; imageFile.read(imagedata);
		 * AsfTag asftag = (AsfTag) tag; asftag.setField(asftag.createArtworkField(imagedata)); f.commit();
		 * 
		 * f = AudioFileIO.read(testFile); tag = f.getTag(); assertEquals(1, tag.getFields(FieldKey.COVER_ART).size());
		 * 
		 * TagField tagField = tag.getFields(FieldKey.COVER_ART).get(0); AsfTagCoverField coverartField =
		 * (AsfTagCoverField) tagField; assertEquals("WM/Picture", tagField.getId()); assertEquals((Integer)
		 * PictureTypes.DEFAULT_ID, (Integer) coverartField.getPictureType()); assertEquals(18572,
		 * tagField.getRawContent().length); assertEquals(18545, coverartField.getRawImageData().length);
		 * assertEquals(coverartField.getImageDataSize(), coverartField.getRawImageData().length); assertEquals(200,
		 * coverartField.getImage().getWidth()); assertEquals(200, coverartField.getImage().getHeight());
		 * assertEquals(BufferedImage.TYPE_3BYTE_BGR, coverartField.getImage().getType());
		 * 
		 * } catch (Exception e) { e.printStackTrace(); exceptionCaught = e; }
		 * 
		 * assertNull(exceptionCaught);
		 */
	}
	 ...
	 * 
	 * public void testWriteMultipleFields() throws Exception { Path testFile =
	 * AbstractTestCase.copyAudioToTmp("test1.wma", Paths.get("testWriteMultiple.wma")); AudioFile f =
	 * AudioFileIO.read(testFile); List<TagField> tagFields = f.getTag().getFields(FieldKey.ALBUM_ARTIST_SORT);
	 * assertEquals(tagFields.size(),0); f.getTag().addField(FieldKey.ALBUM_ARTIST_SORT,"artist1");
	 * f.getTag().addField(FieldKey.ALBUM_ARTIST_SORT,"artist2"); f.commit(); f = AudioFileIO.read(testFile); tagFields
	 * = f.getTag().getFields(FieldKey.ALBUM_ARTIST_SORT); assertEquals(2,tagFields.size()); }
	 */

	public void testDeleteFields() throws Exception {
		// Delete using generic key
		final Path testFile = AbstractTestCase.copyAudioToTmp("test1.wma", Paths.get("testDeleteFields.wma"));
		AudioFile f = AudioFileIO.read(testFile);
