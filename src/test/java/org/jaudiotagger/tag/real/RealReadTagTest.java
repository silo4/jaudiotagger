...
public class RealReadTagTest extends AbstractTestCase {
	...
	public void checkRealTag(final String filename, final String title, final String artist, final String comment) throws InvalidAudioFrameException, IOException, ReadOnlyFileException, TagException, CannotReadException {
		final Path testFile = AbstractTestCase.copyAudioToTmp(filename);
		final AudioFile f = AudioFileIO.read(testFile);
		final Tag tag = f.getTag();
		assertEquals(3, tag.getFieldCount()); // If this line fails we need to update our test as the RealMedia tag
												// parser has been augmented
