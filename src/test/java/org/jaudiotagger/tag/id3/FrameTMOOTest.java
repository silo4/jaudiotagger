...
public class FrameTMOOTest extends AbstractTestCase {
	public static ID3v24Frame getInitialisedFrame() {
		final ID3v24Frame frame = new ID3v24Frame(ID3v24Frames.FRAME_ID_MOOD);
		final FrameBodyTMOO fb = FrameBodyTMOOTest.getInitialisedBody();
		frame.setBody(fb);
		return frame;
	}

	/*
	 * public static ID3v23Frame getV23InitialisedFrame() { ID3v23Frame frame = new
	 * ID3v23Frame(ID3v23Frames.FRAME_ID_V3_USER_DEFINED_INFO); FrameBodyXSOP fb =
	 * FrameBodyXSOPTest.getInitialisedBody(); frame.setBody(fb); return frame; }
	 */
	...
	/*
	 * public void testCreateID3v23Frame() { Exception exceptionCaught = null; ID3v23Frame frame = null; FrameBodyXSOP
	 * fb = null; try { frame = new ID3v23Frame(ID3v23Frames.FRAME_ID_V3_ARTIST_SORT_ORDER_MUSICBRAINZ); fb =
	 * FrameBodyXSOPTest.getInitialisedBody(); frame.setBody(fb); } catch (Exception e) { exceptionCaught = e; }
	 * 
	 * assertNull(exceptionCaught); assertEquals(ID3v23Frames.FRAME_ID_V3_ARTIST_SORT_ORDER_MUSICBRAINZ,
	 * frame.getIdentifier()); assertEquals(TextEncoding.ISO_8859_1, fb.getTextEncoding());
	 ...
	 */
	/*
	 * public void testCreateID3v22Frame() { Exception exceptionCaught = null; ID3v22Frame frame = null; FrameBodyTSOP
	 * fb = null; try { frame = new ID3v22Frame(ID3v22Frames.FRAME_ID_V2_ARTIST_SORT_ORDER_ITUNES); fb =
	 * FrameBodyTSOPTest.getInitialisedBody(); frame.setBody(fb); } catch (Exception e) { exceptionCaught = e; }
	 * 
	 * assertNull(exceptionCaught); assertEquals(ID3v22Frames.FRAME_ID_V2_ARTIST_SORT_ORDER_ITUNES,
	 * frame.getIdentifier()); assertEquals(TextEncoding.ISO_8859_1, fb.getTextEncoding());
	...
	public void testSaveToFile() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("test1016.mp3"));
		...
		final ID3v24Frame frame = (ID3v24Frame) mp3File.getID3v2Tag().getFrame(ID3v24Frames.FRAME_ID_MOOD);
		final FrameBodyTMOO body = (FrameBodyTMOO) frame.getBody();
		assertEquals(TextEncoding.ISO_8859_1, body.getTextEncoding());
	}

	public void testSaveEmptyFrameToFile() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("test1004.mp3"));
		MP3File mp3File = new MP3File(testFile);
	...
	/**
	 * Testing not only conversion of MOOD but also what hapens when have two frame of different types (TMOO and TXXX)
	 * that become the same type TXXX
	 * 
	 * @throws Exception
	 */
	public void testConvertV24ToV23() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("test1005.mp3"));
		MP3File mp3File = new MP3File(testFile);
		...
		mp3File = new MP3File(testFile);
		final ID3v23Tag v23Tag = new ID3v23Tag(mp3File.getID3v2TagAsv24());
		assertEquals(2, v23Tag.getFields("TXXX").size());
		mp3File.setID3v2TagOnly(v23Tag);

		final Iterator i = v23Tag.getFields();
		while (i.hasNext())
	...
	/**
	 * Testing not only conversion of MOOD but also what happens when have two frame of different types (TMOO and TXXX)
	 * that become the same type TXXX
	 * 
	 * @throws Exception
	 */
	public void testConvertV23ToV24() throws Exception {
		...
		final FrameBodyTXXX frameBody = new FrameBodyTXXX();
		frameBody.setDescription(FrameBodyTXXX.MOOD);
		frameBody.setText("Tranquil");
		final ID3v23Frame frame = new ID3v23Frame(ID3v23Frames.FRAME_ID_V3_USER_DEFINED_INFO);
		...
		// Reload and convert to v23 and save
		mp3File = new MP3File(testFile);
		final ID3v24Tag v24Tag = new ID3v24Tag(mp3File.getID3v2Tag());
		final Iterator i = v24Tag.getFields();
		while (i.hasNext())
			System.out.println("kkk" + ((ID3v24Frame) i.next()).getIdentifier());
		assertEquals(1, v24Tag.getFieldCount());
		final ID3v24Frame v24frame = (ID3v24Frame) v24Tag.getFrame("TMOO");
		assertTrue(v24frame.getBody() instanceof FrameBodyTMOO);
		final FrameBodyTMOO v24framebody = (FrameBodyTMOO) v24frame.getBody();
