...
public class FrameCOMMTest extends AbstractTestCase {
	/**
	 * Should run without throwing Runtime excception, although COMMFrame wont be loaded and will throwe invalid size
	 * exception
	 */
	public void testReadFileContainingInvalidSizeCOMMFrame() throws Exception {
		Exception e = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("Issue77.id3", "testV1.mp3");
	...
	/**
	 * Should run without throwing Runtime excception, although COMMFrame wont be loaded and will throwe invalid
	 * datatype exception
	 */
	public void testReadFileContainingInvalidTextEncodingCOMMFrame() throws Exception {
		Exception e = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("Issue80.id3", "testV1.mp3");
			final MP3File mp3File = new MP3File(testFile);
		} catch (final Exception ie) {
			e = ie;
		}
	...
	/**
	 * Can read file containing a language code that does not actually map to a code , and write it back In this real
	 * example the language code has been held as three space characters
	 */
	public void testreadFrameContainingInvalidlanguageCodeCOMMFrame() throws Exception {
		final String INVALID_LANG_CODE = "   ";
		Exception e = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("Issue108.id3", "testV1.mp3");
			final MP3File mp3File = new MP3File(testFile);

			assertTrue(mp3File.getID3v2Tag().hasFrame("COMM"));

			final ID3v24Frame commFrame = (ID3v24Frame) mp3File.getID3v2Tag().getFrame("COMM");
			final FrameBodyCOMM frameBody = (FrameBodyCOMM) commFrame.getBody();
