...
public class FrameTIPLTest extends AbstractTestCase {
	public static ID3v24Frame getInitialisedFrame() {
		...
		final FrameBodyTIPL fb = FrameBodyTIPLTest.getInitialisedBody();
		frame.setBody(fb);
		return frame;
	}
	...
	public static ID3v24Frame getInitialisedFrameOdd() {
		final ID3v24Frame frame = new ID3v24Frame(ID3v24Frames.FRAME_ID_INVOLVED_PEOPLE);
		final FrameBodyTIPL fb = FrameBodyTIPLTest.getInitialisedBodyOdd();
		frame.setBody(fb);
		return frame;
	}
	...
	public void testSaveToFile() throws Exception {
		...
		final ID3v24Frame frame = (ID3v24Frame) mp3File.getID3v2Tag().getFrame(ID3v24Frames.FRAME_ID_INVOLVED_PEOPLE);
		final FrameBodyTIPL body = (FrameBodyTIPL) frame.getBody();
	...
	public void testConvertV24ToV23() throws Exception {
		...
		// Reload
		mp3File = new MP3File(testFile);
		final ID3v23Frame frame = (ID3v23Frame) mp3File.getID3v2Tag().getFrame(ID3v23Frames.FRAME_ID_V3_IPLS);
		final FrameBodyIPLS body = (FrameBodyIPLS) frame.getBody();
		assertEquals(TextEncoding.ISO_8859_1, body.getTextEncoding());
