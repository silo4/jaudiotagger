...
public class FrameULSTTest extends AbstractTestCase {
	public void testReadULST() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("test23.mp3");

		final MP3File mp3File = new MP3File(testFile);
		final ID3v24Frame v24frame = (ID3v24Frame) mp3File.getID3v2Tag().getFrame(ID3v24Frames.FRAME_ID_UNSYNC_LYRICS);

		// Old method
		final FrameBodyUSLT lyricsBody = (FrameBodyUSLT) v24frame.getBody();
		...
		final AudioFile file = AudioFileIO.read(testFile);
	...
	public void testWriteULSTID3v23Test3() throws Exception {
		...
		lyricsBody.setDescription("");
		lyricsBody.setLyric("lyric1\u111f");
		final ID3v24Frame v24frame = new ID3v24Frame(ID3v24Frames.FRAME_ID_UNSYNC_LYRICS);
		...
		// Check normal values
		mp3File = new MP3File(testFile);
		final ID3v23Frame v23frame = (ID3v23Frame) mp3File.getID3v2Tag().getFrame(ID3v23Frames.FRAME_ID_V3_UNSYNC_LYRICS);
		lyricsBody = (FrameBodyUSLT) v23frame.getBody();
		assertEquals(Languages.DEFAULT_ID, lyricsBody.getLanguage());
		assertEquals("", lyricsBody.getDescription());
		assertEquals("lyric1\u111f", lyricsBody.getLyric());
