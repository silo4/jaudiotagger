...
public class NewInterfaceTest extends TestCase {
	 ...
	 * @param arg0
	 */
	public NewInterfaceTest(final String arg0) {
		super(arg0);
	}
	...
	public void testBasicWrite() {
		Exception ex = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("testBasicWrite.mp3"));
	...
	public void testNewInterfaceBasicReadandWriteID3v1() throws Exception {
		...
		final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("testnewIntId3v1.mp3"));
		...
		final ID3v11Tag v1tag = new ID3v11Tag();
		v1tag.setField(FieldKey.ARTIST, V1_ARTIST);
		v1tag.setField(FieldKey.ALBUM, "V1ALBUM" + "\u01ff"); // Note always convert to single byte so will be written
																// as FF
	...
	public void testNewInterfaceBasicReadandWriteID3v24() throws Exception {
		Exception e = null;
		final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("testnewIntId3v24.mp3"));
		final MP3File mp3File = new MP3File(testFile);
		...
		final ID3v24Frame frame = new ID3v24Frame(ID3v24Frames.FRAME_ID_ARTIST);
		...
		final AbstractID3v2Frame checkframe = (AbstractID3v2Frame) ((MP3File) af).getID3v2Tag().getFrame(ID3v24Frames.FRAME_ID_ALBUM);
		...
		final ID3v24Frame commentFrame = (ID3v24Frame) ((ID3v24Tag) af.getTag()).getFrame("COMM");
		final FrameBodyCOMM fb = (FrameBodyCOMM) commentFrame.getBody();
		...
		// Because has different description the following setField will addField another comment rather than
		// overwriting the first one
		af.getTag().setField(FieldKey.COMMENT, "Comment2");
		assertEquals(2, af.getTag().getFields(FieldKey.COMMENT).size());
		// Add third Comment
		final List<TagField> comments = af.getTag().getFields(FieldKey.COMMENT);
		...
		RandomAccessFile imageFile = new RandomAccessFile(AbstractTestCase.dataPath.resolve("coverart.png").toFile(), "r");
		...
		imageFile = new RandomAccessFile(AbstractTestCase.dataPath.resolve("coverart_small.png").toFile(), "r");
		...
		final TagField imageField = af.getTag().getFields(FieldKey.COVER_ART).get(0);
		if (imageField instanceof AbstractID3v2Frame) {
			...
			if (!imageFrameBody.isImageUrl()) {
				final byte[] imageRawData = (byte[]) imageFrameBody.getObjectValue(DataTypes.OBJ_PICTURE_DATA);
		...
		// Add a linked Image
		af.getTag().addField(tag.createLinkedArtworkField("../data/coverart.jpg"));
		af.commit();

		af = AudioFileIO.read(testFile);
		assertEquals(3, af.getTag().getFields(FieldKey.COVER_ART).size());
		assertEquals(12, af.getTag().getFieldCount());
		final List<TagField> imageFields = af.getTag().getFields(FieldKey.COVER_ART);
		...
		assertEquals("../data/coverart.jpg", apicframebody.getImageUrl());

	}

	/*
	 * public void testReadUrlImage() throws Exception { Exception ex = null; try { Path testFile =
	 * AbstractTestCase.copyAudioToTmp("testV1withurlimage.mp3"); org.jaudiotagger.audio.AudioFile audioFile =
	 * org.jaudiotagger.audio.AudioFileIO.read(testFile); ID3v23Tag newTag = (ID3v23Tag)audioFile.getTag();
	 * assertEquals(1, newTag.getFields(FieldKey.COVER_ART).size()); TagField tagField =
	 * newTag.getFirstField(ID3v23FieldKey.COVER_ART.getFieldName()); assertTrue(tagField instanceof ID3v23Frame);
	 * ID3v23Frame apicFrame = (ID3v23Frame)tagField; assertTrue(apicFrame.getBody() instanceof FrameBodyAPIC);
	 * FrameBodyAPIC apicframebody = (FrameBodyAPIC)apicFrame.getBody(); assertFalse(apicframebody.isImageUrl()); }
	 * catch(Exception e) { ex=e; ex.printStackTrace(); } assertNull(ex); }
	 */
	public void testNewInterfaceBasicReadandWriteID3v23() throws Exception {
		Exception e = null;
		final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("testnewIntId3v23.mp3"));
		final MP3File mp3File = new MP3File(testFile);
		...
		final ID3v23Frame frame = new ID3v23Frame(ID3v23Frames.FRAME_ID_V3_ARTIST);
		((FrameBodyTPE1) frame.getBody()).setText(FrameBodyTPE1Test.TPE1_TEST_STRING);
		tag.setFrame(frame);
		mp3File.setID3v2TagOnly(tag);
		mp3File.save();
		...
		// Read back album (old method)
		final AbstractID3v2Frame checkframe = (AbstractID3v2Frame) ((MP3File) af).getID3v2Tag().getFrame(ID3v23Frames.FRAME_ID_V3_ALBUM);
		assertEquals(ALBUM_TEST_STRING, ((FrameBodyTALB) checkframe.getBody()).getText());
	...
	public void testNewInterfaceBasicReadandWriteID3v22() throws Exception {
		Exception e = null;
		final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("testnewIntId3v22.mp3"));
		final MP3File mp3File = new MP3File(testFile);
		...
		final ID3v22Frame frame = new ID3v22Frame(ID3v22Frames.FRAME_ID_V2_ARTIST);
		((FrameBodyTPE1) frame.getBody()).setText(FrameBodyTPE1Test.TPE1_TEST_STRING);
		tag.setFrame(frame);
		mp3File.setID3v2TagOnly(tag);
		mp3File.save();
		...
		// Read back album (old method)
		final AbstractID3v2Frame checkframe = (AbstractID3v2Frame) ((MP3File) af).getID3v2Tag().getFrame(ID3v22Frames.FRAME_ID_V2_ALBUM);
		assertEquals(ALBUM_TEST_STRING, ((FrameBodyTALB) checkframe.getBody()).getText());
	...
	public void testSettingMultipleFramesofSameType() throws Exception {
		final Exception e = null;
		final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("testSetMultiple.mp3"));
		AudioFile af = AudioFileIO.read(testFile);
		...
		// TXXX
		{
			...
			tag.setFrame(frame);
			final ID3v24Frame txxxFrame = (ID3v24Frame) tag.getFrame("TXXX");
			final FrameBodyTXXX fb = (FrameBodyTXXX) txxxFrame.getBody();
			assertEquals("", fb.getDescription());
			assertEquals("UserDefined", fb.getText());
			// Change description, cant do this with common interface
			fb.setDescription("test");
			// Because has different description the following setField will addField another txxx rather than
			// overwriting the first one
		...
		// UFID
		{
			...
			tag.setFrame(frame);
			final ID3v24Frame ufidFrame = (ID3v24Frame) tag.getFrame("UFID");
			final FrameBodyUFID fb = (FrameBodyUFID) ufidFrame.getBody();
			assertEquals("owner", fb.getOwner());

			// Because has different owner the following setField will addField another ufid rather than overwriting the
			// first one
		...
		// ULST
		{
			...
			tag.setFrame(frame);
			final ID3v24Frame usltFrame = (ID3v24Frame) tag.getFrame("USLT");
			final FrameBodyUSLT fb = (FrameBodyUSLT) usltFrame.getBody();
			assertEquals("lyrics1", fb.getDescription());

			// Because has different desc the following setField will addField another uslt rather than overwriting the
			// first one
		...
		// POPM TODO not a supported FieldKey yet
		{
			...
			tag.setFrame(frame);
			final ID3v24Frame popmFrame = (ID3v24Frame) tag.getFrame("POPM");
			final FrameBodyPOPM fb = (FrameBodyPOPM) popmFrame.getBody();
	...
	public void testIterator() throws Exception {
		final File orig = new File("testdata", "test26.mp3");
		if (!orig.isFile())
			return;

		Exception e = null;
		final Path testFile = AbstractTestCase.copyAudioToTmp("test26.mp3");
		final MP3File mp3File = new MP3File(testFile);
	...
	/**
	 * Currently genres are written to and from v2 tag as is, the decoding from genre number to string has to be done
	 * manually
	 */
	...
	public void testRemoveFrameOfType() {
		final File orig = new File("testdata", "test30.mp3");
		if (!orig.isFile())
			return;

		Exception exceptionCaught = null;
		final Path testFile = AbstractTestCase.copyAudioToTmp("test30.mp3");
