...
public class FramePOPMTest extends AbstractTestCase {
	...
	public static ID3v24Frame getInitialisedFrame() {
		final ID3v24Frame frame = new ID3v24Frame(ID3v24Frames.FRAME_ID_POPULARIMETER);
		final FrameBodyPOPM fb = FrameBodyPOPMTest.getInitialisedBody();
		frame.setBody(fb);
		return frame;
	}
	...
	public void testSaveToFile() throws Exception {
		...
		final ID3v24Frame frame = (ID3v24Frame) mp3File.getID3v2Tag().getFrame(ID3v24Frames.FRAME_ID_POPULARIMETER);
		final FrameBodyPOPM body = (FrameBodyPOPM) frame.getBody();
	...
	public void testReadFileContainingPOMFrameWithoutCounter() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("Issue72.id3", "testV1.mp3");

		final MP3File mp3File = new MP3File(testFile);
		final ID3v23Frame frame = (ID3v23Frame) mp3File.getID3v2Tag().getFrame(ID3v24Frames.FRAME_ID_POPULARIMETER);
		final FrameBodyPOPM body = (FrameBodyPOPM) frame.getBody();
		assertEquals(TextEncoding.ISO_8859_1, body.getTextEncoding());
