...
public class FrameBodyTPOSTest extends AbstractTestCase {
	...
	public static FrameBodyTPOS getInitialisedBody() {
		final FrameBodyTPOS fb = new FrameBodyTPOS();
		fb.setDiscNo(1);
		fb.setDiscTotal(11);
		return fb;
	}
	...
	public void testCreateFrameBodyWithPaddedRawTextCount() {
		TagOptionSingleton.getInstance().setPadNumbers(false);
		final FrameBodyTPOS fb = createFrameBodyAndAssertNumericValuesAndRawValueRetained("01/11", 1, 11);
	...
	public void testCreateFrameBodyWithUnpaddedRawTextCount() {
		TagOptionSingleton.getInstance().setPadNumbers(false);
		final FrameBodyTPOS fb = createFrameBodyAndAssertNumericValuesAndRawValueRetained("1/11", 1, 11);
	...
	public void testCreateFrameBodyWithPaddedRawTextTotal() {
		TagOptionSingleton.getInstance().setPadNumbers(false);
		final FrameBodyTPOS fb = createFrameBodyAndAssertNumericValuesAndRawValueRetained("1/03", 1, 3);
		assertEquals("1", fb.getDiscNoAsText());
	...
	public void testCreateFrameBodyWithPaddedRawTextTotal2() {
		TagOptionSingleton.getInstance().setPadNumbers(false);
		final FrameBodyTPOS fb = createFrameBodyAndAssertNumericValuesAndRawValueRetained("01/03", 1, 3);
		assertEquals("01", fb.getDiscNoAsText());
		assertEquals("03", fb.getDiscTotalAsText());
	}
	...
	public void testCreateFrameBodyWithUnpaddedRawTextTotal() {
		TagOptionSingleton.getInstance().setPadNumbers(false);
		final FrameBodyTPOS fb = createFrameBodyAndAssertNumericValuesAndRawValueRetained("1/3", 1, 3);
		assertEquals("1", fb.getDiscNoAsText());
		assertEquals("3", fb.getDiscTotalAsText());
	}

	private FrameBodyTPOS createFrameBodyAndAssertNumericValuesAndRawValueRetained(final String rawText, final int expectedCount, final int expectedTotal) {
		Exception exceptionCaught = null;
		FrameBodyTPOS fb = null;
		try {
