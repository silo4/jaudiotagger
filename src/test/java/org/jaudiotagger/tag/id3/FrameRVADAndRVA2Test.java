...
public class FrameRVADAndRVA2Test extends AbstractTestCase {
	public static String cmp(final byte[] a, final byte[] b) {
	...
	public static ID3v24Frame getInitialisedFrame() {
		final ID3v24Frame frame = new ID3v24Frame(ID3v24Frames.FRAME_ID_RELATIVE_VOLUME_ADJUSTMENT2);
		final FrameBodyRVA2 fb = FrameBodyRVA2Test.getInitialisedBody();
		frame.setBody(fb);
		return frame;
	}

	public static ID3v23Frame getV23InitialisedFrame() {
		final ID3v23Frame frame = new ID3v23Frame(ID3v23Frames.FRAME_ID_V3_RELATIVE_VOLUME_ADJUSTMENT);
		final FrameBodyRVAD fb = FrameBodyRVADTest.getInitialisedBody();
		frame.setBody(fb);
		return frame;
	}

	public static ID3v22Frame getV22InitialisedFrame() {
		final ID3v22Frame frame = new ID3v22Frame(ID3v22Frames.FRAME_ID_V2_RELATIVE_VOLUME_ADJUSTMENT);
		final FrameBodyRVAD fb = FrameBodyRVADTest.getInitialisedBody();
		frame.setBody(fb);
		return frame;
	}
	...
	public void testSaveToFile() throws Exception {
		...
		final ID3v24Frame frame = (ID3v24Frame) mp3File.getID3v2Tag().getFrame(ID3v24Frames.FRAME_ID_RELATIVE_VOLUME_ADJUSTMENT2);
		assertTrue(frame != null);
		final FrameBodyRVA2 body = (FrameBodyRVA2) frame.getBody();
	...
	public void testConvertV24ToV23() throws Exception {
		...
		// Reload
		mp3File = new MP3File(testFile);
		final ID3v23Frame frame = (ID3v23Frame) mp3File.getID3v2Tag().getFrame(ID3v23Frames.FRAME_ID_V3_RELATIVE_VOLUME_ADJUSTMENT);
		assertTrue(frame != null);
		final FrameBodyRVAD body = (FrameBodyRVAD) frame.getBody();
	...
	public void testConvertV24ToV22() throws Exception {
		...
		// Reload
		mp3File = new MP3File(testFile);
		final ID3v22Frame frame = (ID3v22Frame) mp3File.getID3v2Tag().getFrame(ID3v22Frames.FRAME_ID_V2_RELATIVE_VOLUME_ADJUSTMENT);
		assertTrue(frame != null);
		final FrameBodyRVAD body = (FrameBodyRVAD) frame.getBody();
		assertTrue(body instanceof FrameBodyRVAD);
	}
