...
public class ItunesTest extends AbstractTestCase {
	...
	/**
	 * This tests that we work out that the frame is not unsynced and read the frame size as a normal integer using an
	 * integral algorithm
	 * 
	 * @throws Exception
	 */
	public void testv24TagWithNonSyncSafeFrame() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("Issue96-1.id3", "testV1.mp3");

		final MP3File mp3File = new MP3File(testFile);

		final ID3v24Frame v24frame = (ID3v24Frame) mp3File.getID3v2Tag().getFrame(ID3v24Frames.FRAME_ID_ATTACHED_PICTURE);
		assertNotNull(v24frame);
		final FrameBodyAPIC fb = (FrameBodyAPIC) v24frame.getBody();
		assertEquals(FRAME_SIZE, fb.getSize());
	}

	/**
	 * This tests that we work out that the frame is not unsynced because its highest order bit size is set and read the
	 * frame size as a normal integer using an integral algorithm
	 * 
	 * @throws Exception
	 */
	public void testv24TagWithNonSyncSafeFrame2() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("Issue96-3.id3", "testV1.mp3");
	 ...
	 * when not unsynced ( it is the USLT frame that is bering tested in this case, we only get to APIC if read USLT
	 * frame ok)
	 * 
	 * @throws Exception
	 */
	public void testv24TagWithNonSyncSafeFrame3() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("Issue96-4.id3", "testV1.mp3");
	...
	public void testv24TagWithSyncSafeFrame() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("Issue96-2.id3", "testV1.mp3");
	...
	/**
	 * test can read string with spurious null at end, and can retrieve string without this if we want to. Can then
	 * write to file without null if options set correctly, can add multiple values
	 * 
	 * @throws Exception
	 */
	...
	public void testCanReadEmptyString() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("Issue92-2.id3", "testV1.mp3");
		final MP3File mp3File = new MP3File(testFile);

		final ID3v23Frame v23frame = (ID3v23Frame) mp3File.getID3v2Tag().getFrame(ID3v23Frames.FRAME_ID_V3_COMMENT);
		assertNotNull(v23frame);
		final FrameBodyCOMM fb = (FrameBodyCOMM) v23frame.getBody();
	...
	public void testCanFindStartOfMp3AudioWithinUTF16LETag() throws Exception {
		final long START_OF_AUDIO_LOCATION = 2048;
		final int FRAME_COUNT = 11;
		final Path testFile = AbstractTestCase.copyAudioToTmp("Issue104-1.id3", "testV1.mp3");
		final MP3File mp3File = new MP3File(testFile);
	...
	/**
	 * Because last frame is large it has to check that size is unsynced, because no padding code have to be careful not
	 * to have buffer underflow exception
	 * 
	 * @throws Exception
	 */
	public void testv24TagWithlargeSyncSafeFrameAndNoPadding() throws Exception {
		final Path testFile = AbstractTestCase.copyAudioToTmp("issue115.id3", "testV1.mp3");

		final MP3File mp3File = new MP3File(testFile);

		final List apicFrames = (List) mp3File.getID3v2Tag().getFrame(ID3v24Frames.FRAME_ID_ATTACHED_PICTURE);
