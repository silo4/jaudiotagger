...
/**
 * This class tests the correct implementation of {@link MetadataContainer} implementations.<br>
 * 
...
public abstract class AbstractMetadataContainer<T extends MetadataContainer> extends AbstractChunk<T> {
	...
	/**
	 * This method creates some or all supported descriptors for the given container.<br>
	...
	public void testVariousDescriptorMethods() {
		for (final T curr : createTestContainers()) {
			...
			for (final MetadataDescriptor desc : createSupportedDescriptors(curr)) {
			...
			final List<MetadataDescriptor> removed = new ArrayList<MetadataDescriptor>(allDescriptors);
			...
			for (final String name : descriptorMap.keySet()) {
				assertTrue(curr.hasDescriptor(name));
				assertTrue(descriptorMap.get(name).containsAll(curr.getDescriptorsByName(name)));
			}
			for (final MetadataDescriptor desc : allDescriptors)
				assertTrue(curr.containsDescriptor(desc));
			assertFalse(curr.isEmpty());
			for (final String name : descriptorMap.keySet()) {
				final List<MetadataDescriptor> list = descriptorMap.get(name);
				curr.removeDescriptorsByName(name);
				for (final MetadataDescriptor desc : list)
