...
public class ChunkContainerTest extends AbstractChunk<ChunkContainer> {
	 ...
	 */
	@Override
	protected ChunkContainer createChunk(final long pos, final BigInteger size) {
		return new ChunkContainer(GUID.GUID_UNSPECIFIED, pos, size);
	}

	/**
	 * Creates an instance of {@link ChunkContainer} and adds a {@link Chunk} for each (and with) GUID of
	 * {@link GUID#KNOWN_GUIDS}.<br>
	 * 
	...
	protected ChunkContainer createFilledChunk() {
		...
		long position = 0;
		for (final GUID curr : GUID.KNOWN_GUIDS)
	...
	/**
	 * Test method for {@link org.jaudiotagger.audio.asf.data.ChunkContainer#getChunks()}.
	 */
	public void testGetChunks() {
		...
		final Set<GUID> known = new HashSet<GUID>(Arrays.asList(GUID.KNOWN_GUIDS));
	...
	public void testGetFirst() {
		...
		assertSame(audio, container.getFirst(GUID.GUID_STREAM, AudioStreamChunk.class));
		assertNull(container.getFirst(GUID.GUID_STREAM, VideoStreamChunk.class));
		container = createChunk(0, BigInteger.ZERO);
		container.addChunk(video);
		container.addChunk(audio);
		assertSame(video, container.getFirst(GUID.GUID_STREAM, VideoStreamChunk.class));
