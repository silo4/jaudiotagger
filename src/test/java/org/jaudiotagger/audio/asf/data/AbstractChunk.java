...
public abstract class AbstractChunk<T extends Chunk> extends AbstractTestCase {
	...
	/**
	 * Invokes {@link #createChunk(long, BigInteger)} and returns possible occurred exceptions.
	 * 
	 * @param pos
	 *            position
	 * @param size
	...
	/**
	 * Tests the correctness of various methods from {@link Chunk} directly after construction.<br>
	 */
	public void testBasicChunkMethods() {
		final BigInteger size = BigInteger.TEN;
		final long position = 300;
		final long newPosition = 400;
		final T chunk = createChunk(position, size);
		assertNotNull(chunk);
		assertEquals(position, chunk.getPosition());
		assertEquals(size, chunk.getChunkLength());
		assertEquals(size.add(BigInteger.valueOf(position)).longValue(), chunk.getChunkEnd());
		/*
		...
		assertEquals(size, chunk.getChunkLength());
		assertEquals(size.add(BigInteger.valueOf(newPosition)).longValue(), chunk.getChunkEnd());
	}

	/**
	 * Tests chunk creation by using invalid and valid creation arguments on {@link #createChunk(long, BigInteger)}.
	 */
