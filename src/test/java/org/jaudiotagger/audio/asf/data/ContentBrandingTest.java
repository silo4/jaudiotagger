...
public class ContentBrandingTest extends AbstractMetadataContainer<ContentBranding> {
	 ...
	 */
	@Override
	protected ContentBranding createChunk(final long pos, final BigInteger size) {
	 ...
	 */
	@Override
	protected MetadataDescriptor[] createSupportedDescriptors(final ContentBranding container) {
		assertSame(ContainerType.CONTENT_BRANDING, container.getContainerType());
		final List<MetadataDescriptor> result = new ArrayList<MetadataDescriptor>();
		MetadataDescriptor curr = new MetadataDescriptor(ContentBranding.KEY_COPYRIGHT_URL);
	...
	public void testIsAddSupported() {
		...
		assertTrue(chunk.isAddSupported(new MetadataDescriptor(ContainerType.CONTENT_BRANDING, ContentBranding.KEY_BANNER_URL, MetadataDescriptor.TYPE_STRING)));
	}
