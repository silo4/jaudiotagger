...
public class UtilsTest extends TestCase {
	...
	public void testCopy() {
		final byte[] src = new byte[] { 0, 1, 2, 3, 4, 5 };
		final byte[] dst = new byte[8];
		Utils.copy(src, dst, 2);
		assertTrue(dst[2] == 0 && dst[3] == 1 && dst[4] == 2 && dst[5] == 3 && dst[6] == 4 && dst[7] == 5);
	}

	public void testGetExtension() {
		assertEquals("jpeg", Utils.getExtension(Paths.get("_12XYZ.jpeg")));
	}

	public void testReadUInt16() {
		try {
			final byte[] maxUnsignedBuf = { (byte) 0XFF, (byte) 0XFF };
			...
			final byte[] smallIntBuf = { (byte) 0X01, (byte) 0X10 };
	...
	public void testReadUInt32() {
		try {
			final byte[] maxUnsignedBuf = { (byte) 0XFF, (byte) 0XFF, (byte) 0XFF, (byte) 0XFF };
			ByteArrayInputStream ins = new ByteArrayInputStream(maxUnsignedBuf);
			DataInputStream dis = new DataInputStream(ins);
			long val = Utils.readUint32(dis);
			assertEquals(val, 0XFFFFFFFFL);

			final byte[] smallIntBuf = { (byte) 0X03, (byte) 0XFF, (byte) 0X01, (byte) 0X12 };
			ins = new ByteArrayInputStream(smallIntBuf);
			dis = new DataInputStream(ins);
	...
	public void testReadString() {
		try {
			final byte[] strBuf = { (byte) 'A', (byte) '=', (byte) '1', (byte) '+', (byte) '7' };
			final ByteArrayInputStream ins = new ByteArrayInputStream(strBuf);
			final DataInputStream dis = new DataInputStream(ins);
			final String s = Utils.readString(dis, 5);
	...
	public void testGetSizeLEInt32() {
		final byte[] bytes = Utils.getSizeLEInt32(0X010203F4);
	...
	public void testGetSizeBEInt32() {
		final byte[] bytes = Utils.getSizeBEInt32(0X010203F4);
		assertEquals(bytes.length, 4);
		// need to AND with FF to avoid sign extension of byte
	...
	public void testGetSizeBEInt16() {
		final byte[] bytes = Utils.getSizeBEInt16((short) 0X0182);
	...
	public void testgetLongLE() {
		...
		final byte[] bytes = new byte[] { 0X32, 0X01, (byte) 0XF0, 0X18 };
		bb.put(bytes);
		final long val = Utils.getLongLE(bb, 0, 3);
