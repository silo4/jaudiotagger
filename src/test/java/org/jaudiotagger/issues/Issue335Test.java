...
/**
 * Test when write multiple strings using UTF16 with BOM that it writes the BOM for all strings not just the first one
 */
public class Issue335Test extends AbstractTestCase {
	...
	public void testConvertv24Tov23ConvertsUTF8ToISO8859IfItCan() throws Exception {
		final Path orig = AbstractTestCase.dataPath.resolve("test79.mp3");
		...
		final Path testFile = AbstractTestCase.copyAudioToTmp("test79.mp3");
		...
		final ID3v23Tag tag = new ID3v23Tag(f.getID3v2Tag());
		assertEquals(3, body.getTextEncoding());
		f.setID3v2Tag(tag);
		f.commit();
	...
	public void testConvertv23Twice() throws Exception {
		...
		final ID3v24Tag v24tag = f.getID3v2TagAsv24();
	...
	public void testConvertCharsAtStartOfFile() throws Exception {
		...
		final String s = (isMP3v2) ? v24tag.getFirst(ID3v24Frames.FRAME_ID_ARTIST) : tag.getFirst(FieldKey.ARTIST);
