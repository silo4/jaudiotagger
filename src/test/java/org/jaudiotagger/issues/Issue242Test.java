...
public class Issue242Test extends AbstractTestCase {
	...
	public void testWriteNewUrlsFilev24() {
		...
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("test1030.mp3"));

			// Add a v24Tag
			AudioFile af = AudioFileIO.read(testFile);
			MP3File mp3File = (MP3File) af;
			// Checking not overwriting audio when have to pad to fix data
			final long mp3AudioLength = Files.size(testFile) - mp3File.getMP3AudioHeader().getMp3StartByte();
			mp3File.setID3v2Tag(new ID3v24Tag());
			mp3File.save();

			af = AudioFileIO.read(testFile);
			mp3File = (MP3File) af;
			assertEquals(mp3AudioLength, Files.size(testFile) - mp3File.getMP3AudioHeader().getMp3StartByte());
	...
	public void testWriteNewUrlsFilev23() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("test1031.mp3"));
	...
	public void testWriteNewUrlsFilev22() {
		Exception exceptionCaught = null;
		try {
			final Path testFile = AbstractTestCase.copyAudioToTmp("testV1.mp3", Paths.get("test1032.mp3"));
