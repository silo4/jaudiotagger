...
public class Issue271Test extends AbstractTestCase {
	...
	public void testReadMp3WithExtendedHeaderFlagSetButNoExtendedHeader() {
		final Path orig = AbstractTestCase.dataPath.resolve("test46.mp3");
	...
	public void testReadMp3WithExtendedHeaderAndCrc() {
		final Path orig = AbstractTestCase.dataPath.resolve("test47.mp3");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		try {
			...
			final ID3v23Tag id3v23Tag = (ID3v23Tag) af.getTag();
	...
	public void testReadMp3WithEncryptedField() {
		final Path orig = AbstractTestCase.dataPath.resolve("test48.mp3");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
		...
		try {
			...
			final AbstractID3v2Frame frame = (AbstractID3v2Frame) id3v23Tag.getFrame(ID3v23Frames.FRAME_ID_V3_ENCODEDBY);
	...
	public void testReadFlagsCompressed() {
		final Path orig = AbstractTestCase.dataPath.resolve("test51.mp3");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
	...
	public void testReadFlagsUnsyced() {
		final Path orig = AbstractTestCase.dataPath.resolve("test52.mp3");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
	...
	public void testReadFlagsUnsycedCompressed() {
		final Path orig = AbstractTestCase.dataPath.resolve("test53.mp3");
		if (!Files.isRegularFile(orig)) {
			System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
			return;
		}
