...
/**
 * Mp3s can handle writing multiple fields which actually map to a single field using generic interface but retrieval
 * shows them as just one field, contrast with Flac
 */
public class Issue411Test extends AbstractTestCase {
	...
	public void testIssue2() throws Exception {
		Exception caught = null;
		try {
			final Path orig = AbstractTestCase.dataPath.resolve("test.flac");
			if (!Files.isRegularFile(orig)) {
				System.err.println("Unable to test file " + orig.toAbsolutePath() + " - not available");
				return;
			}
