 ...
 * <p/>
 * <p>
 * The audio header consists of a number of audio frames. Because we are not trying to play the audio but only extract
 * some information regarding the audio we only need to read the first audio frames to ensure that we have correctly
 * identified them as audio frames and extracted the metadata we reuire.
 * <p/>
 * <p>
 * Start of Audio id 0xFF (11111111) and then second byte anded with 0xE0(11100000). For example 2nd byte doesnt have to
 * be 0xE0 is just has to have the top 3 signicant bits set. For example 0xFB (11111011) is a common occurence of the
 * second match. The 2nd byte defines flags to indicate various mp3 values.
 * <p/>
 * <p>
 * Having found these two values we then read the header which comprises these two bytes plus a further two to ensure
 * this really is a MP3Header, sometimes the first frame is actually a dummy frame with summary information held within
 * about the whole file, typically using a Xing Header or LAme Header. This is most useful when the file is variable bit
 * rate, if the file is variable bit rate but does not use a summary header it will not be correctly identified as a VBR
 * frame and the track length will be incorrectly calculated. Strictly speaking MP3 means Layer III file but MP2 Layer
 * II), MP1 Layer I) and MPEG-2 files are sometimes used and named with the .mp3 suffix so this library attempts to
...
public class MP3AudioHeader implements AudioHeader {
	...
	/**
	 * After testing the average location of the first MP3Header bit was at 5000 bytes so this is why chosen as a
	 * default.
	 */
	 ...
	 * <p/>
	 * The search starts from the start of the file, it is usually safer to use the alternative constructor that allows
	 * you to provide the length of the tag header as a parameter so the tag can be skipped over.
	 * 
	 * @param seekFile
	 * @throws IOException
	 * @throws InvalidAudioFrameException
	 */
	public MP3AudioHeader(final Path seekFile) throws IOException, InvalidAudioFrameException {
		if (!seek(seekFile, 0))
			throw new InvalidAudioFrameException("No audio header found within" + seekFile.getFileName());
	}

	/**
	 * Search for the first MP3Header in the file
	 * <p/>
	 * Starts searching from location startByte, this is because there is likely to be an ID3TagHeader before the start
	 * of the audio. If this tagHeader contains unsynchronized information there is a possibility that it might be
	 * inaccurately identified as the start of the Audio data. Various checks are done in this code to prevent this
	 * happening but it cannot be guaranteed.
	 * <p/>
	 * Of course if the startByte provided overstates the length of the tag header, this could mean the start of the
	 * MP3AudioHeader is missed, further checks are done within the MP3 class to recognize if this has occurred and take
	 * appropriate action.
	 * 
	 * @param seekFile
	 * @param startByte
	 ...
	 * @throws InvalidAudioFrameException
	 */
	public MP3AudioHeader(final Path seekFile, final long startByte) throws IOException, InvalidAudioFrameException {
		if (!seek(seekFile, startByte))
			throw new InvalidAudioFrameException(ErrorMessage.NO_AUDIO_HEADER_FOUND.getMsg(seekFile.getFileName()));
	}
	 ...
	 */
	public boolean seek(final Path seekFile, final long startByte) throws IOException {
		...
		final FileInputStream fis = new FileInputStream(seekFile.toFile());
		final FileChannel fc = fis.getChannel();

		// Read into Byte Buffer in Chunks
		final ByteBuffer bb = ByteBuffer.allocateDirect(FILE_BUFFER_SIZE);
				...
				if (MPEGFrameHeader.isMPEGFrame(bb))
					try {
						...
						if ((header = XingFrame.isXingFrame(bb, mp3FrameHeader)) != null) {
							...
							} catch (final InvalidAudioFrameException ex) {
		...
		setFileSize(Files.size(seekFile));
	 ...
	 */
	private boolean isNextFrameValid(final Path seekFile, final long filePointerCount, final ByteBuffer bb, final FileChannel fc) throws IOException {
		if (MP3AudioHeader.logger.isLoggable(Level.FINEST))
			MP3AudioHeader.logger.finer("Checking next frame" + seekFile.getFileName() + ":fpc:" + filePointerCount + "skipping to:" + (filePointerCount + mp3FrameHeader.getFrameLength()));
	...
	/**
	 * @return The number of frames within the Audio File, calculated by dividing the filesize by the number of frames,
	 *         this may not be the most accurate method available.
	 */
	...
	/**
	 * Set the time each frame contributes to the audio in fractions of seconds, the higher the sampling rate the
	 * shorter the audio segment provided by the frame, the number of samples is fixed by the MPEG Version and Layer
	 */
	...
	/**
	 * Estimate the length of the audio track in seconds Calculation is Number of frames multiplied by the Time Per
	 * Frame using the first frame as a prototype Time Per Frame is the number of samples in the frame (which is defined
	 * by the MPEGVersion/Layer combination) divided by the sampling rate, i.e the higher the sampling rate the shorter
	 * the audio represented by the frame is going to be.
	 */
	 ...
	 * @param fileSize
	 */
	protected void setFileSize(final long fileSize) {
	...
	public String toString() {
		String s = "fileSize:" + fileSize + " encoder:" + encoder + " startByte:" + Hex.asHex(startByte) + " numberOfFrames:" + numberOfFrames + " numberOfFramesEst:" + numberOfFramesEstimate + " timePerFrame:" + timePerFrame + " bitrate:"
				+ bitrate + " trackLength:" + getTrackLengthAsString();
