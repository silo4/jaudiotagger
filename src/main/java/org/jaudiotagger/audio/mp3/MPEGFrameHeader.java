...
/**
 * Represents a MPEGFrameHeader, an MP3 is made up of a number of frames each frame starts with a four byte frame
 * header.
 */
...
public class MPEGFrameHeader {
	...
	private void setBitrate() throws InvalidAudioFrameException {
		...
		final int bitRateIndex = mpegBytes[BYTE_3] & MASK_MP3_BITRATE | mpegBytes[BYTE_2] & MASK_MP3_ID | mpegBytes[BYTE_2] & MASK_MP3_LAYER;
	...
	private void setModeExtension() throws InvalidAudioFrameException {
		final int index = (mpegBytes[BYTE_4] & MASK_MP3_MODE_EXTENSION) >> 4;
		if (layer == LAYER_III) {
	...
	private void setSamplingRate() throws InvalidAudioFrameException {
		// Frequency
		final int index = (mpegBytes[BYTE_3] & MASK_MP3_FREQUENCY) >>> 2;
		final Map<Integer, Integer> samplingRateMapForVersion = samplingRateMap.get(version);
	...
	/**
	 * Get the number of samples in a frame, all frames in a file have a set number of samples as defined by their MPEG
	 * Versiona and Layer
	 * 
	 * @return
	 */
	public int getNoOfSamples() {
		final Integer noOfSamples = samplesPerFrameMap.get(version).get(layer);
		return noOfSamples;
	}
	...
	/**
	 * Try and create a new MPEG frame with the given byte array and decodes its contents If decoding header causes a
	 * problem it is not a valid header
	 * 
	 * @param b
	 *            the array of bytes representing this mpeg frame
	 * @throws InvalidAudioFrameException
	 *             if does not match expected format
	 */
	private MPEGFrameHeader(final byte[] b) throws InvalidAudioFrameException {
		mpegBytes = b;
		setBitrate();
		setVersion();
		setLayer();
	 ...
	 * @return
	 * @throws InvalidAudioFrameException
	 *             if there is no header at this point
	 */
	public static MPEGFrameHeader parseMPEGHeader(final ByteBuffer bb) throws InvalidAudioFrameException {
		final int position = bb.position();
		bb.get(header, 0, HEADER_SIZE);
		bb.position(position);
		final MPEGFrameHeader frameHeader = new MPEGFrameHeader(header);
	 ...
	 */
	public static boolean isMPEGFrame(final ByteBuffer bb) {
		final int position = bb.position();
		return (((bb.get(position) & SYNC_BYTE1) == SYNC_BYTE1) && ((bb.get(position + 1) & SYNC_BYTE2) == SYNC_BYTE2) && ((bb.get(position + 2) & SYNC_BIT_ANDSAMPING_BYTE3) != SYNC_BIT_ANDSAMPING_BYTE3));
	}
	...
	public String toString() {
		return " mpeg frameheader:" + " frame length:" + getFrameLength() + " version:" + versionAsString + " layer:" + layerAsString + " channelMode:" + channelModeAsString + " noOfSamples:" + getNoOfSamples() + " samplingRate:"
				+ samplingRate + " isPadding:" + isPadding + " isProtected:" + isProtected + " isPrivate:" + isPrivate + " isCopyrighted:" + isCopyrighted + " isOriginal:" + isCopyrighted + " isVariableBitRate" + this.isVariableBitRate()
				+ " header as binary:" + AbstractTagDisplayFormatter.displayAsBinary(mpegBytes[BYTE_1]) + " " + AbstractTagDisplayFormatter.displayAsBinary(mpegBytes[BYTE_2]) + " "
				+ AbstractTagDisplayFormatter.displayAsBinary(mpegBytes[BYTE_3]) + " " + AbstractTagDisplayFormatter.displayAsBinary(mpegBytes[BYTE_4]);
	}
}
