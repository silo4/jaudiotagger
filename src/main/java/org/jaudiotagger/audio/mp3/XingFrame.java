 ...
 * this library is details allowing us to determine the bitrate of a Variable Bit Rate VBR file without having to
 * process the whole file.
 * <p/>
 * Xing VBR Tag data format is 120 bytes long 4 bytes for Header Tag 4 bytes for Header Flags 4 bytes for FRAME SIZE 4
 * bytes for AUDIO_SIZE 100 bytes for entry (NUMTOCENTRIES) 4 bytes for VBR SCALE. a VBR quality indicator: 0=best
 * 100=worst
 * <p/>
...
public class XingFrame {
	 ...
	 */
	private XingFrame(final ByteBuffer header) {
		this.header = header;
		...
		final byte flagBuffer[] = new byte[XING_FLAG_BUFFER_SIZE];
	...
	private void setVbr() {
		// Is it VBR or CBR
		final byte[] identifier = new byte[XING_IDENTIFIER_BUFFER_SIZE];
	...
	private void setFrameCount() {
		final byte frameCountBuffer[] = new byte[XING_FRAMECOUNT_BUFFER_SIZE];
	...
	private void setAudioSize() {
		final byte frameSizeBuffer[] = new byte[XING_AUDIOSIZE_BUFFER_SIZE];
	 ...
	 */
	public static XingFrame parseXingFrame(final ByteBuffer header) throws InvalidAudioFrameException {
		final XingFrame xingFrame = new XingFrame(header);
		return xingFrame;
	}
	 ...
	 */
	public static ByteBuffer isXingFrame(final ByteBuffer bb, final MPEGFrameHeader mpegFrameHeader) {
		...
		else if (mpegFrameHeader.getChannelMode() == MPEGFrameHeader.MODE_MONO)
