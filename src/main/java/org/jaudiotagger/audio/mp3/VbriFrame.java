 ...
 * <p/>
 * <p>
 * In MP3s encoded using the franhofer encoder which variable bit rate the first frame in the file contains a special
 * frame called a Vrbi Frame, instead of audio data (Other Vbr encoders use the more common Xing Frame). This is used to
 * store additional information about the file. The most important aspect for this library is details allowing us to
 ...
 * from the XING header. You find it exactly 32 bytes after the end of the first MPEG audio header in the file. (Note
 * that the position is zero-based; position, length and example are each in byte-format.) Position Length Meaning
 * Example 0 4 VBR header ID in 4 ASCII chars, always 'VBRI', not NULL-terminated 'VBRI' 4 2 Version ID as Big-Endian
 * WORD 1 6 2 Delay as Big-Endian float 7344 8 2 Quality indicator 75 10 4 Number of Bytes of Audio as Big-Endian DWORD
 * 45000 14 4 Number of Frames as Big-Endian DWORD 7344 18 2 Number of entries within TOC table as Big-Endian WORD 100
 * 20 2 Scale factor of TOC table entries as Big-Endian DWORD 1 22 2 Size per table entry in bytes (max 4) as Big-Endian
 * WORD 2 24 2 Frames per table entry as Big-Endian WORD 845 26 TOC entries for seeking as Big-Endian integral. From
 * size per table entry and number of entries, you can calculate the length of this field.
 * <p/>
 */
public class VbriFrame {
	 ...
	 */
	private VbriFrame(final ByteBuffer header) {
		this.header = header;
	...
	private void setAudioSize() {
		final byte frameSizeBuffer[] = new byte[VBRI_AUDIOSIZE_BUFFER_SIZE];
		header.get(frameSizeBuffer);
		final boolean audioSizeEnabled = true;
	...
	private void setFrameCount() {
		final byte frameCountBuffer[] = new byte[VBRI_FRAMECOUNT_BUFFER_SIZE];
		header.get(frameCountBuffer);
		final boolean frameCountEnabled = true;
	 ...
	 * 
	 */
	public static VbriFrame parseVBRIFrame(final ByteBuffer header) throws InvalidAudioFrameException {
		final VbriFrame VBRIFrame = new VbriFrame(header);
		return VBRIFrame;
	}
	 ...
	 */
	public static ByteBuffer isVbriFrame(final ByteBuffer bb, final MPEGFrameHeader mpegFrameHeader) {
		...
		final ByteBuffer header = bb.slice();

		// Return Buffer to start Point
		bb.position(startPosition);

		// Check Identifier
		final byte[] identifier = new byte[VBRI_IDENTIFIER_BUFFER_SIZE];
