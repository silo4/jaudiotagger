...
public class Mp4TagReader {
	 ...
	 */
	public Mp4Tag read(final RandomAccessFile raf) throws CannotReadException, IOException {
		...
		if (boxHeader != null) {
			...
			final Mp4MetaBox meta = new Mp4MetaBox(boxHeader, moovBuffer);
			meta.processData();
		...
		final int length = boxHeader.getLength() - Mp4BoxHeader.HEADER_LENGTH;
		final ByteBuffer metadataBuffer = moovBuffer.slice();
	 ...
	 * <p/>
	 * Note:In the case of coverart MP4 holds all the coverart within individual dataitems all within a single covr
	 * atom, we will add separate mp4field for each image.
	 * 
	 * @param tag
	 * @param header
	 * @param raw
	 ...
	 */
	private void createMp4Field(final Mp4Tag tag, final Mp4BoxHeader header, final ByteBuffer raw) throws UnsupportedEncodingException {
		// Reverse Dns Atom
		if (header.getId().equals(Mp4TagReverseDnsField.IDENTIFIER))
			//
			try {
				final TagField field = new Mp4TagReverseDnsField(header, raw);
				tag.addField(field);
			} catch (final Exception e) {
				logger.warning(ErrorMessage.MP4_UNABLE_READ_REVERSE_DNS_FIELD.getMsg(e.getMessage()));
				final TagField field = new Mp4TagRawBinaryField(header, raw);
				tag.addField(field);
			}
		else {
			final int currentPos = raw.position();
			final boolean isDataIdentifier = Utils.getString(raw, Mp4BoxHeader.IDENTIFIER_POS, Mp4BoxHeader.IDENTIFIER_LENGTH, "ISO-8859-1").equals(Mp4DataBox.IDENTIFIER);
			...
			if (isDataIdentifier) {
				...
				if (header.getId().equals(Mp4FieldKey.TRACK.getFieldName())) {
					final TagField field = new Mp4TrackField(header.getId(), raw);
					tag.addField(field);
				} else if (header.getId().equals(Mp4FieldKey.DISCNUMBER.getFieldName())) {
					final TagField field = new Mp4DiscNoField(header.getId(), raw);
					tag.addField(field);
				} else if (header.getId().equals(Mp4FieldKey.GENRE.getFieldName())) {
					final TagField field = new Mp4GenreField(header.getId(), raw);
					tag.addField(field);
					...
					while (processedDataSize < header.getDataLength()) {
						...
						final Mp4TagCoverField field = new Mp4TagCoverField(raw, fieldType);
						tag.addField(field);
						processedDataSize += field.getDataAndHeaderSize();
						imageCount++;
					}
				} else if (fieldType == Mp4FieldType.TEXT) {
					final TagField field = new Mp4TagTextField(header.getId(), raw);
					tag.addField(field);
				} else if (fieldType == Mp4FieldType.IMPLICIT) {
					final TagField field = new Mp4TagTextNumberField(header.getId(), raw);
					tag.addField(field);
				} else if (fieldType == Mp4FieldType.INTEGER) {
					final TagField field = new Mp4TagByteField(header.getId(), raw);
					tag.addField(field);
				} else {
					boolean existingId = false;
					for (final Mp4FieldKey key : Mp4FieldKey.values())
					...
					if (!existingId) {
						...
						final TagField field = new Mp4TagBinaryField(header.getId(), raw);
						tag.addField(field);
					}
				}
			}
			// Special Cases
			else // MediaMonkey 3 CoverArt Attributes field, does not have data items so just
