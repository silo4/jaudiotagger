 ...
 * <p/>
 * <p/>
 * When writing changes the size of all the atoms upto ilst has to be recalculated, then if the size of the metadata is
 * increased the size of the free atom (below meta) should be reduced accordingly or vice versa. If the size of the
 * metadata has increased by more than the size of the free atom then the size of meta, udta and moov should be
 * recalculated and the top level free atom reduced accordingly If there is not enough space even if using both of the
 * free atoms, then the mdat atom has to be shifted down accordingly to make space, and the stco atom has to have its
 * offsets to mdat chunks table adjusted accordingly.
 * 
...
public class Mp4TagWriter {
	 ...
	 * @throws IOException
	 */
	private void writeMetadataSameSize(final ByteBuffer rawIlstData, final long oldIlstSize, final long startIstWithinFile, final FileChannel fileReadChannel, final FileChannel fileWriteChannel, final Mp4BoxHeader tagsHeader)
	...
	/**
	 * If the existing files contains a tags atom and chp1 atom underneath the meta atom that means the file was encoded
	 * by Nero. Applications such as foobar read this non-standard tag before the more usual data within ilst causing
	 * problems. So the solution is to convert the tags atom and its children into a free atom whilst leaving the chp1
	 * atom alone.
	 * 
	 * @param fileReadChannel
	 * @param fileWriteChannel
	 ...
	 * @throws IOException
	 */
	private void writeNeroData(final FileChannel fileReadChannel, final FileChannel fileWriteChannel, final Mp4BoxHeader tagsHeader) throws IOException, CannotWriteException {
		// Write from after ilst upto tags atom
		final long writeBetweenIlstAndTags = tagsHeader.getFilePos() - fileReadChannel.position();
	...
	/**
	 * When the size of the metadata has changed and it cant be compensated for by free atom we have to adjust the size
	 * of the size field upto the moovheader level for the udta atom and its child meta atom.
	 * 
	 * @param moovHeader
	 * @param moovBuffer
	 ...
	 */
	private void adjustSizeOfMoovHeader(final Mp4BoxHeader moovHeader, final ByteBuffer moovBuffer, final int sizeAdjustment, final Mp4BoxHeader udtaHeader, final Mp4BoxHeader metaHeader) throws IOException {
	...
	private void createMetadataAtoms(final Mp4BoxHeader moovHeader, final ByteBuffer moovBuffer, final int sizeAdjustment, final Mp4BoxHeader udtaHeader, final Mp4BoxHeader metaHeader) throws IOException {
	...
	public void write(final Tag tag, final RandomAccessFile raf, final RandomAccessFile rafTemp) throws CannotWriteException, IOException {
		...
		final FileChannel fileReadChannel = raf.getChannel();

		// Write channel for writing to new file
		final FileChannel fileWriteChannel = rafTemp.getChannel();
		...
		final Mp4BoxHeader mdatHeader = atomTree.getBoxHeader(atomTree.getMdatNode());
		...
		final ByteBuffer rawIlstData = tc.convert(tag);
		rawIlstData.rewind();
		newIlstSize = rawIlstData.limit();

		// Moov Box header
		final Mp4BoxHeader moovHeader = atomTree.getBoxHeader(atomTree.getMoovNode());
		final long positionWithinFileAfterFindingMoovHeader = moovHeader.getFilePos() + Mp4BoxHeader.HEADER_LENGTH;
		endOfMoov = moovHeader.getFilePos() + moovHeader.getLength();

		final Mp4StcoBox stco = atomTree.getStco();
		final Mp4BoxHeader ilstHeader = atomTree.getBoxHeader(atomTree.getIlstNode());
		Mp4BoxHeader udtaHeader = atomTree.getBoxHeader(atomTree.getUdtaNode());
		final Mp4BoxHeader metaHeader = atomTree.getBoxHeader(atomTree.getMetaNode());
		final Mp4BoxHeader hdlrMetaHeader = atomTree.getBoxHeader(atomTree.getHdlrWithinMetaNode());
		final Mp4BoxHeader tagsHeader = atomTree.getBoxHeader(atomTree.getTagsNode());
		final Mp4BoxHeader trakHeader = atomTree.getBoxHeader(atomTree.getTrakNodes().get(0));
		final ByteBuffer moovBuffer = atomTree.getMoovBuffer();
		...
		if (udtaHeader != null) {
			...
			if (metaHeader != null) {
				...
				} else // Place ilst immediately after existing hdlr atom
		...
		else // Create new structure just after the end of the trak atom
		if (metaHeader != null) {
		...
		for (final DefaultMutableTreeNode freeNode : atomTree.getFreeNodes()) {
			final DefaultMutableTreeNode parentNode = (DefaultMutableTreeNode) freeNode.getParent();
		...
		// no other changes necessary and total file size remains the same
			...
			if (oldMetaLevelFreeAtomSize > 0) {
				...
				final int newFreeSize = oldMetaLevelFreeAtomSize + (oldIlstSize - newIlstSize);
				final Mp4FreeBox newFreeBox = new Mp4FreeBox(newFreeSize - Mp4BoxHeader.HEADER_LENGTH);
			...
			// No free atom we need to create a new one or adjust top level free atom
				...
				final int newFreeSize = (oldIlstSize - newIlstSize) - Mp4BoxHeader.HEADER_LENGTH;
				...
				if (newFreeSize > 0) {
					...
					// Create new free box
					final Mp4FreeBox newFreeBox = new Mp4FreeBox(newFreeSize);
					fileWriteChannel.write(newFreeBox.getHeader().getHeaderData());
					fileWriteChannel.write(newFreeBox.getData());
				...
				// Ok everything in this bit of tree has to be recalculated because eight or less bytes smaller
					...
					final int sizeReducedBy = oldIlstSize - newIlstSize;
					...
					// Edit stco atom within moov header, we need to adjust offsets by the amount mdat is going to be
					// shifted
		...
		// Size of metadata has increased, the most complex situation, more atoms affected
			...
			final int additionalSpaceRequiredForMetadata = newIlstSize - oldIlstSize;
			...
			if (additionalSpaceRequiredForMetadata <= (oldMetaLevelFreeAtomSize - Mp4BoxHeader.HEADER_LENGTH)) {
				final int newFreeSize = oldMetaLevelFreeAtomSize - (additionalSpaceRequiredForMetadata);
			...
			// need their sizes adjusted.
				...
				if (udtaHeader == null) {
					...
					final Mp4HdlrBox hdlrBox = Mp4HdlrBox.createiTunesStyleHdlrBox();
					final Mp4MetaBox metaBox = Mp4MetaBox.createiTunesStyleMetaBox(hdlrBox.getHeader().getLength() + rawIlstData.limit());
					...
					if ((!topLevelFreeAtomComesBeforeMdatAtomAndAfterMetadata)
							|| ((topLevelFreeSize - Mp4BoxHeader.HEADER_LENGTH < additionalMetaSizeThatWontFitWithinMetaAtom) && (topLevelFreeSize != additionalMetaSizeThatWontFitWithinMetaAtom)))
						// We don't bother using the top level free atom coz not big enough anyway, we need to adjust
						// offsets
					...
					// #291:In this case we throwaway editing udta header and create a new one, would be beter if we
					// coul
					// keep the info but rather difficult.
					logger.config("Writing:Option 5.2;No meta atom");

					final int oldUdtaHeaderLength = udtaHeader.getLength();
					final Mp4HdlrBox hdlrBox = Mp4HdlrBox.createiTunesStyleHdlrBox();
					final Mp4MetaBox metaBox = Mp4MetaBox.createiTunesStyleMetaBox(hdlrBox.getHeader().getLength() + rawIlstData.limit());
					udtaHeader = new Mp4BoxHeader(Mp4AtomIdentifier.UDTA.getFieldName());
					udtaHeader.setLength(Mp4BoxHeader.HEADER_LENGTH + metaBox.getHeader().getLength());
				...
				if (tagsHeader != null) {
					...
					fileReadChannel.position(tagsHeader.getFilePos() + tagsHeader.getLength());
					final long extraData = endOfMoov - fileReadChannel.position();
				...
				if (topLevelFreeAtomComesBeforeMdatAtomAndAfterMetadata && (topLevelFreePosition >= startIlstWithinFile)) {
					...
					// note could be a double negative as additionalMetaSizeThatWontFitWithinMetaAtom could be -1 to -8
					// but thats ok stills works
					// ok
					if (topLevelFreeSize - Mp4BoxHeader.HEADER_LENGTH >= additionalMetaSizeThatWontFitWithinMetaAtom) {
						...
						final Mp4FreeBox freeBox = new Mp4FreeBox((topLevelFreeSize - Mp4BoxHeader.HEADER_LENGTH) - additionalMetaSizeThatWontFitWithinMetaAtom);
	 ...
	 * @throws CannotWriteException
	 */
	private void writeDataInChunks(final FileChannel fileReadChannel, final FileChannel fileWriteChannel) throws IOException, CannotWriteException {
		final long amountToBeWritten = fileReadChannel.size() - fileReadChannel.position();
		long written = 0;
	 ...
	 * @throws IOException
	 */
	private void convertandWriteTagsAtomToFreeAtom(final FileChannel fileWriteChannel, final Mp4BoxHeader tagsHeader) throws IOException {
		final Mp4FreeBox freeBox = new Mp4FreeBox(tagsHeader.getDataLength());
		fileWriteChannel.write(freeBox.getHeader().getHeaderData());
		fileWriteChannel.write(freeBox.getData());
	}
	 ...
	 * @throws IOException
	 */
	private void writeDataUptoIncludingIlst(final FileChannel fileReadChannel, final FileChannel fileWriteChannel, final int oldIlstSize, final int startIlstWithinFile, final ByteBuffer rawIlstData) throws IOException {
		fileReadChannel.position(0);
	 ...
	 * @throws IOException
	 */
	private void writeDataAfterIlst(final FileChannel fileReadChannel, final FileChannel fileWriteChannel, final Mp4BoxHeader tagsHeader) throws IOException, CannotWriteException {
		if (tagsHeader != null)
	 ...
	 * @return
	 */
	private int getMetaLevelFreeAtomSize(final Mp4AtomTree atomTree) {
		int oldMetaLevelFreeAtomSize;// Level 4 - Free
		oldMetaLevelFreeAtomSize = 0;

		for (final DefaultMutableTreeNode freeNode : atomTree.getFreeNodes()) {
			final DefaultMutableTreeNode parentNode = (DefaultMutableTreeNode) freeNode.getParent();
			final DefaultMutableTreeNode brotherNode = freeNode.getPreviousSibling();
			if (!parentNode.isRoot()) {
				final Mp4BoxHeader parentHeader = ((Mp4BoxHeader) parentNode.getUserObject());
				final Mp4BoxHeader freeHeader = ((Mp4BoxHeader) freeNode.getUserObject());

				// We are only interested in free atoms at this level if they come after the ilst node
				if (brotherNode != null) {
					final Mp4BoxHeader brotherHeader = ((Mp4BoxHeader) brotherNode.getUserObject());
	 ...
	 * @throws IOException
	 */
	private void checkFileWrittenCorrectly(final RandomAccessFile rafTemp, final Mp4BoxHeader mdatHeader, final FileChannel fileWriteChannel, final Mp4StcoBox stco) throws CannotWriteException, IOException {
		...
		try {
			...
			final Mp4BoxHeader newMdatHeader = newAtomTree.getBoxHeader(newAtomTree.getMdatNode());
			...
			final Mp4BoxHeader newUdtaHeader = newAtomTree.getBoxHeader(newAtomTree.getUdtaNode());
			...
			final Mp4BoxHeader newMetaHeader = newAtomTree.getBoxHeader(newAtomTree.getMetaNode());
			...
			final Mp4StcoBox newStco = newAtomTree.getStco();
			...
			if ((newStco.getFirstOffSet() - newMdatHeader.getFilePos()) != diff) {
				final int discrepancy = (int) ((newStco.getFirstOffSet() - newMdatHeader.getFilePos()) - diff);
	 ...
	 * @throws IOException
	 */
	public void delete(final RandomAccessFile raf, final RandomAccessFile rafTemp) throws IOException {
		final Mp4Tag tag = new Mp4Tag();

		try {
			write(tag, raf, rafTemp);
		} catch (final CannotWriteException cwe) {
