 ...
 * <p/>
 * We are most interested in boxes that are used to hold metadata, but we have to know about some other boxes as well in
 * order to find them.
 * <p/>
 * All boxes consist of a 4 byte box length (big Endian), and then a 4 byte identifier, this is the header which is
 * model in this class.
 * <p/>
 * The length includes the length of the box including the identifier and the length itself. Then they may contain data
 * and/or sub boxes, if they contain subboxes they are known as a parent box. Parent boxes shouldn't really contain
 * data, but sometimes they do.
 * <p/>
 * Parent boxes length includes the length of their immediate sub boxes
 * <p/>
 * This class is normally used by instantiating with the empty constructor, then use the update method to pass the
 * header data which is used to read the identifier and the the size of the box
 */
public class Mp4BoxHeader {
	 ...
	 * @param id
	 */
	public Mp4BoxHeader(final String id) {
	 ...
	 * @param headerData
	 */
	public Mp4BoxHeader(final ByteBuffer headerData) {
		update(headerData);
	}
	 ...
	 * @param headerData
	 */
	public void update(final ByteBuffer headerData) {
		// Read header data into byte array
		final byte[] b = new byte[HEADER_LENGTH];
		headerData.get(b);
	...
	public void setLength(final int length) {
		final byte[] headerSize = Utils.getSizeBEInt32(length);
	 ...
	 * @param length
	 */
	public void setId(final int length) {
		final byte[] headerSize = Utils.getSizeBEInt32(length);
	 ...
	 * <p/>
	 * Note it wont find the box if it is contained with a level below the current level, nor if we are at a parent atom
	 * that also contains data and we havent yet processed the data. It will work if we are at the start of a child box
	 * even if it not the required box as long as the box we are looking for is the same level (or the level above in
	 * some cases).
	 * 
	 * @param raf
	 * @param id
	 ...
	 * @return
	 */
	public static Mp4BoxHeader seekWithinLevel(final RandomAccessFile raf, final String id) throws IOException {
		logger.finer("Started searching for:" + id + " in file at:" + raf.getChannel().position());

		final Mp4BoxHeader boxHeader = new Mp4BoxHeader();
		final ByteBuffer headerBuffer = ByteBuffer.allocate(HEADER_LENGTH);
		...
		while (!boxHeader.getId().equals(id)) {
			...
			if (boxHeader.getLength() < Mp4BoxHeader.HEADER_LENGTH)
				return null;
			final int noOfBytesSkipped = raf.skipBytes(boxHeader.getDataLength());
	 ...
	 * Seek for box with the specified id starting from the current location of filepointer,
	 * <p/>
	 * Note it won't find the box if it is contained with a level below the current level, nor if we are at a parent
	 * atom that also contains data and we havent yet processed the data. It will work if we are at the start of a child
	 * box even if it not the required box as long as the box we are looking for is the same level (or the level above
	 * in some cases).
	 * 
	 * @param data
	 * @param id
	 ...
	 * @return
	 */
	public static Mp4BoxHeader seekWithinLevel(final ByteBuffer data, final String id) throws IOException {
	 ...
	 * @param filePos
	 */
	public void setFilePos(final long filePos) {
