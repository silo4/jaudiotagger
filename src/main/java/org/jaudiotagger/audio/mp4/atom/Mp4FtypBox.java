...
public class Mp4FtypBox extends AbstractMp4Box {
	 ...
	 */
	public Mp4FtypBox(final Mp4BoxHeader header, final ByteBuffer dataBuffer) {
		this.header = header;
	...
	public void processData() throws CannotReadException {
		...
		while ((dataBuffer.position() < dataBuffer.limit()) && (dataBuffer.limit() - dataBuffer.position() >= COMPATIBLE_BRAND_LENGTH)) {
			...
			decoder.onMalformedInput(CodingErrorAction.REPORT);
			try {
				final String brand = decoder.decode((ByteBuffer) dataBuffer.slice().limit(COMPATIBLE_BRAND_LENGTH)).toString();
	...
	/**
	 * Major brand, helps identify whats contained in the file, used by major and compatible brands but this is not an
	 * exhaustive list, so for that reason we don't force the values read from the file to tie in with this enum.
	 */
	public static enum Brand {
		ISO14496_1_BASE_MEDIA("isom", "ISO 14496-1"), ISO14496_12_BASE_MEDIA("iso2", "ISO 14496-12"), ISO14496_1_VERSION_1("mp41", "ISO 14496-1"), ISO14496_1_VERSION_2("mp42", "ISO 14496-2:Multi track with BIFS scenes"), QUICKTIME_MOVIE(
				"qt  ", "Original Quicktime"), JVT_AVC("avc1", "JVT"), THREEG_MOBILE_MP4("MPA ", "3G Mobile"), APPLE_AAC_AUDIO("M4P ", "Apple Audio"), AES_ENCRYPTED_AUDIO("M4B ", "Apple encrypted Audio"), APPLE_AUDIO("mp71", "Apple Audio"), ISO14496_12_MPEG7_METADATA(
				"mp71", "MAIN_SYNTHESIS"), APPLE_AUDIO_ONLY("M4A ", "M4A Audio"), // SOmetimes used by protected mutli
																					// track audio
		;
		 ...
		 */
		Brand(final String id, final String description) {
			this.id = id;
