 ...
 * <p/>
 * It contains a number of (possibly optional?) sections (section 3 - 6) (containing optional filler) with differeent
 * info in each section.
 * <p/>
 * <p/>
 * -> 4 bytes version/flags = 8-bit hex version + 24-bit hex flags (current = 0)
 * <p/>
 * Section 3 -> 1 byte ES descriptor type tag = 8-bit hex value 0x03 -> 3 bytes optional extended descriptor type tag
 * string = 3 * 8-bit hex value - types are 0x80,0x81,0xFE -> 1 byte descriptor type length = 8-bit unsigned length -> 2
 * bytes ES ID = 16-bit unsigned value -> 1 byte stream priority = 8-bit unsigned value - Defaults to 16 and ranges from
 * 0 through to 31
 * <p/>
 * Section 4 -> 1 byte decoder config descriptor type tag = 8-bit hex value 0x04 -> 3 bytes optional extended descriptor
 * type tag string = 3 * 8-bit hex value - types are 0x80,0x81,0xFE -> 1 byte descriptor type length = 8-bit unsigned
 * length * -> 1 byte object type ID = 8-bit unsigned value -> 6 bits stream type = 3/4 byte hex value - type IDs are
 * object descript. = 1 ; clock ref. = 2 - type IDs are scene descript. = 4 ; visual = 4 - type IDs are audio = 5 ;
 * MPEG-7 = 6 ; IPMP = 7 - type IDs are OCI = 8 ; MPEG Java = 9 - type IDs are user private = 32 -> 1 bit upstream flag
 * = 1/8 byte hex value -> 1 bit reserved flag = 1/8 byte hex value set to 1 -> 3 bytes buffer size = 24-bit unsigned
 * value -> 4 bytes maximum bit rate = 32-bit unsigned value -> 4 bytes average bit rate = 32-bit unsigned value
 * <p/>
 * Section 5 -> 1 byte decoder specific descriptor type tag 8-bit hex value 0x05 -> 3 bytes optional extended descriptor
 * type tag string = 3 * 8-bit hex value - types are 0x80,0x81,0xFE -> 1 byte descriptor type length = 8-bit unsigned
 * length -> 1 byte Audio profile Id - 5 bits Profile Id - 3 bits Unknown -> 8 bits other flags - 3 bits unknown - 2
 * bits is No of Channels - 3 bits unknown
 * <p/>
 * Section 6
 * <p/>
 * -> 1 byte SL config descriptor type tag = 8-bit hex value 0x06 -> 3 bytes optional extended descriptor type tag
 * string = 3 * 8-bit hex value - types are 0x80,0x81,0xFE -> 1 byte descriptor type length = 8-bit unsigned length -> 1
 * byte SL value = 8-bit hex value set to 0x02
 */
public class Mp4EsdsBox extends AbstractMp4Box {
	...
	static {
		...
		for (final Kind next : Kind.values())
			kindMap.put(next.getId(), next);

		audioProfileMap = new HashMap<Integer, AudioProfile>();
		for (final AudioProfile next : AudioProfile.values())
	 ...
	 */
	public Mp4EsdsBox(final Mp4BoxHeader header, final ByteBuffer dataBuffer) {
		this.header = header;
		...
		if (dataBuffer.get() == SECTION_FIVE) {
			...
			// Channels
			final byte channelByte = dataBuffer.get();
	 ...
	 */
	public int processSectionHeader(final ByteBuffer dataBuffer) {
		int datalength;
		final byte nextByte = dataBuffer.get();
	...
	public static enum Kind {
		V1(1), V2(2), MPEG4_VIDEO(32), MPEG4_AVC_SPS(33), MPEG4_AVC_PPS(34), MPEG4_AUDIO(64), MPEG2_SIMPLE_VIDEO(96), MPEG2_MAIN_VIDEO(97), MPEG2_SNR_VIDEO(98), MPEG2_SPATIAL_VIDEO(99), MPEG2_HIGH_VIDEO(100), MPEG2_422_VIDEO(101), MPEG4_ADTS_MAIN(
				102), MPEG4_ADTS_LOW_COMPLEXITY(103), MPEG4_ADTS_SCALEABLE_SAMPLING(104), MPEG2_ADTS_MAIN(105), MPEG1_VIDEO(106), MPEG1_ADTS(107), JPEG_VIDEO(108), PRIVATE_AUDIO(192), PRIVATE_VIDEO(208), PCM_LITTLE_ENDIAN_AUDIO(224), VORBIS_AUDIO(
				225), DOLBY_V3_AUDIO(226), ALAW_AUDIO(227), MULAW_AUDIO(228), ADPCM_AUDIO(229), PCM_BIG_ENDIAN_AUDIO(230), YV12_VIDEO(240), H264_VIDEO(241), H263_VIDEO(242), H261_VIDEO(243);
	...
	public static enum AudioProfile {
		MAIN(1, "Main"), LOW_COMPLEXITY(2, "Low Complexity"), SCALEABLE(3, "Scaleable Sample rate"), T_F(4, "T/F"), T_F_MAIN(5, "T/F Main"), T_F_LC(6, "T/F LC"), TWIN_VQ(7, "TWIN"), CELP(8, "CELP"), HVXC(9, "HVXC"), HILN(10, "HILN"), TTSI(
				11, "TTSI"), MAIN_SYNTHESIS(12, "MAIN_SYNTHESIS"), WAVETABLE(13, "WAVETABLE"), ;
		...
		/**
		 * @param id
		 *            it is stored as in file
		 * @param description
		 *            human readable description
		 */
		AudioProfile(final int id, final String description) {
			this.id = id;
