...
public class Utils {
	 ...
	 * @param dstOffset
	 *            The start offset for the bytes to be copied.
	 */
	public static void copy(final byte[] src, final byte[] dst, final int dstOffset) {
	 ...
	 * @return The bytes.
	 */
	public static byte[] getDefaultBytes(final String s, final String charSet) {
		try {
	...
	/*
	 * Returns the extension of the given file. The extension is empty if there is no extension The extension is the
	 ...
	 */
	public static String getExtension(final Path f) {
		final String name = f.getFileName().toString().toLowerCase();
	 ...
	 * 
	 * @param b The byte array @param start The starting offset in b (b[offset]). The less significant byte @param end
	 * The end index (included) in b (b[end]). The most significant byte @return a long number represented by the byte
	 * sequence.
	 * 
	...
	/*
	 * Computes a number whereby the 1st byte is the least significant and the last byte is the most significant. This
	 * version doesn't take a length, and it returns an int rather than a long.
	 * 
	...
	/*
	 * Computes a number whereby the 1st byte is the least significant and the last byte is the most significant. end -
	 * start must be no greater than 4.
	 * 
	 ...
	 */
	public static String getString(final byte[] b, final int offset, final int length, final String encoding) {
		try {
			return new String(b, offset, length, encoding);
		} catch (final UnsupportedEncodingException ue) {
			// Shouldnt have to worry about this exception as should only be
			// calling with well defined charsets
			throw new RuntimeException(ue);
		}
	}

	/**
	 * Create String offset from position by offset upto length using encoding, and position of buffer is moved to after
	 ...
	 * @throws RuntimeException
	 */
	public static String getString(final ByteBuffer buffer, final int offset, final int length, final String encoding) {
	...
	/*
	 * Tries to convert a string into an UTF8 array of bytes If the conversion fails, return the string converted with
	 * the default encoding.
	 * 
	 * @param s The string to convert @return The byte array representation of this string in UTF8 encoding
	 */
	public static byte[] getUTF8Bytes(final String s) throws UnsupportedEncodingException {
		return s.getBytes("UTF-8");
	}

	/**
	 * Reads a 32-bit integer and returns it as a (signed) int. Does overflow checking since java can't handle unsigned
	 * numbers.
	 * 
	 * @param di
	 *            The input source
	...
	public static long readUInt64(final ByteBuffer b) {
		long result = 0;
	...
	public static int readUBEInt32(final ByteBuffer b) {
		int result = 0;
	 ...
	 */
	public static int readUBEInt24(final ByteBuffer b) {
		int result = 0;
		result += readUBEInt16(b) << 16;
		result += readUInt8(b);
		return result;
	 ...
	 */
	public static int readUBEInt16(final ByteBuffer b) {
		int result = 0;
	 ...
	 */
	public static int readUInt8(final ByteBuffer b) {
		return read(b);
	}

	public static int read(final ByteBuffer b) {
		final int result = (b.get() & 0xFF);
		return result;
	}
	 ...
	 * @return
	 */
	public static String getBaseFilenameForTempFile(final Path file) {
	 ...
	 *         creation.
	 */
	public static String getMinBaseFilenameAllowedForTempFile(final Path file) {
	 ...
	 * @return
	 */
	public static boolean rename(final Path fromFile, final Path toFile) {
		logger.log(Level.CONFIG, "Renaming From:" + fromFile.toAbsolutePath() + " to " + toFile.toAbsolutePath());
		...
		// Rename File, could fail because being used or because trying to
		// rename over filesystems
		try {
			Files.move(fromFile, toFile);
		} catch (final IOException e) {
			// Might be trying to rename over filesystem, so try copy and delete
			// instead
			if (copy(fromFile, toFile)) {
				// If copy works but deletion of original file fails then it is
				// because the file is being used
				// so we need to delete the file we have just created
				final boolean deleteResult = fromFile.toFile().delete();
				if (!deleteResult) {
	...
	public static boolean copy(final Path fromPath, final Path toPath) {
		...
		try {
			...
			if (Files.size(fromPath) != Files.size(toPath)) {
				Files.delete(toPath);
				return false;
			}
		} catch (final IOException e) {
