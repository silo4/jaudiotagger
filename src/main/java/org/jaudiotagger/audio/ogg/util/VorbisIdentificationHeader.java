 ...
 * coded as follows:
 * <p/>
 * 1) [vorbis_version] = read 32 bits as unsigned integer 2) [audio_channels] = read 8 bit integer as unsigned 3)
 * [audio_sample_rate] = read 32 bits as unsigned integer 4) [bitrate_maximum] = read 32 bits as signed integer 5)
 * [bitrate_nominal] = read 32 bits as signed integer 6) [bitrate_minimum] = read 32 bits as signed integer 7)
 * [blocksize_0] = 2 exponent (read 4 bits as unsigned integer) 8) [blocksize_1] = 2 exponent (read 4 bits as unsigned
 * integer) 9) [framing_flag] = read one bit
 * <p/>
 * $Id$
 * 
...
public class VorbisIdentificationHeader implements VorbisHeader {
	...
	public VorbisIdentificationHeader(final byte[] vorbisData) {
	...
	public void decodeHeader(final byte[] b) {
		...
		final String vorbis = Utils.getString(b, VorbisHeader.FIELD_CAPTURE_PATTERN_POS, VorbisHeader.FIELD_CAPTURE_PATTERN_LENGTH, "ISO-8859-1");
		...
		if (packetType == VorbisPacketType.IDENTIFICATION_HEADER.getType() && vorbis.equals(CAPTURE_PATTERN)) {
			...
			final int framingFlag = b[FIELD_FRAMING_FLAG_POS];
