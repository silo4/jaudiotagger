...
public class OggPageHeader {
	 ...
	 */
	public static OggPageHeader read(final ByteBuffer byteBuffer) throws IOException, CannotReadException {
		// byteBuffer
		...
		final int pageSegments = byteBuffer.get() & 0xFF; // unsigned
		byteBuffer.position(start);

		b = new byte[OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + pageSegments];
		byteBuffer.get(b);
		final OggPageHeader pageHeader = new OggPageHeader(b);
	 ...
	 * @throws CannotReadException
	 */
	public static OggPageHeader read(final RandomAccessFile raf) throws IOException, CannotReadException {
		...
		final int pageSegments = raf.readByte() & 0xFF; // unsigned
		raf.seek(start);
	...
	public OggPageHeader(final byte[] b) {
		this.rawHeaderData = b;
		final int streamStructureRevision = b[FIELD_STREAM_STRUCTURE_VERSION_POS];
		...
		if (streamStructureRevision == 0) {
			...
			final int pageSegments = u(b[FIELD_PAGE_SEGMENTS_POS]);
	...
	public String toString() {
		String out = "Ogg Page Header:isValid:" + isValid + ":type:" + headerTypeFlag + ":oggPageHeaderLength:" + rawHeaderData.length + ":length:" + pageLength + ":seqNo:" + getPageSequence() + ":packetIncomplete:"
				+ isLastPacketIncomplete() + ":serNum:" + this.getSerialNumber();

		for (final PacketStartAndLength packet : getPacketList())
			out += packet.toString();
		return out;
	}
	 ...
	 * 
	 * This is useful for Ogg files that contain unsupported additional data at the start of the file such as ID3 data
	 */
	public long getStartByte() {
		return startByte;
	}

	public void setStartByte(final long startByte) {
		this.startByte = startByte;
	}

	/**
	 * Within the page specifies the start and length of each packet in the page offset from the end of the pageheader
	 * (after the segment table)
	 */
	public static class PacketStartAndLength {
		private Integer startPosition = 0;
		private Integer length = 0;

		public PacketStartAndLength(final int startPosition, final int length) {
	...
	/**
	 * This represents all the flags that can be set in the headerType field. Note these values can be ORED together.
	 * For example the last packet in a file would normally have a value of 0x5 because both the CONTINUED_PACKET bit
	 * and the END_OF_BITSTREAM bit would be set.
	 */
	public static enum HeaderTypeFlag {
		FRESH_PACKET((byte) 0x0), CONTINUED_PACKET((byte) 0x1), START_OF_BITSTREAM((byte) 0x2), END_OF_BITSTREAM((byte) 0x4);

		byte fileValue;

		HeaderTypeFlag(final byte fileValue) {
