...
public class OggVorbisCommentTagCreator {
	...
	public ByteBuffer convert(final Tag tag) throws UnsupportedEncodingException {
		final ByteBuffer ogg = creator.convert(tag);
		final int tagLength = ogg.capacity() + VorbisHeader.FIELD_PACKET_TYPE_LENGTH + VorbisHeader.FIELD_CAPTURE_PATTERN_LENGTH + OggVorbisCommentTagCreator.FIELD_FRAMING_BIT_LENGTH;

		final ByteBuffer buf = ByteBuffer.allocate(tagLength);
