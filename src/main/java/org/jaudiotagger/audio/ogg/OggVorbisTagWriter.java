...
public class OggVorbisTagWriter {
	...
	public void delete(final RandomAccessFile raf, final RandomAccessFile tempRaf) throws IOException, CannotReadException, CannotWriteException {
		try {
			reader.read(raf);
		...
		final VorbisCommentTag emptyTag = VorbisCommentTag.createNewTag();
	...
	public void write(final Tag tag, final RandomAccessFile raf, final RandomAccessFile rafTemp) throws CannotReadException, CannotWriteException, IOException {
		...
		final OggPageHeader secondPageHeader = OggPageHeader.read(raf);

		// 2nd Page:Store the end of Header
		final long secondPageHeaderEndPos = raf.getFilePointer();
		logger.fine("Read 2nd Page:comment and setup and possibly audio:Header finishes at file position:" + secondPageHeaderEndPos);

		// Get header sizes
		raf.seek(0);
		final OggVorbisTagReader.OggVorbisHeaderSizes vorbisHeaderSizes = reader.readOggVorbisHeaderSizes(raf);

		// Convert the OggVorbisComment header to raw packet data
		final ByteBuffer newComment = tc.convert(tag);

		// Compute new comment length(this may need to be spread over multiple pages)
		final int newCommentLength = newComment.capacity();

		// Calculate new size of new 2nd page
		final int newSecondPageDataLength = vorbisHeaderSizes.getSetupHeaderSize() + newCommentLength + vorbisHeaderSizes.getExtraPacketDataSize();
		...
		if (isCommentAndSetupHeaderFitsOnASinglePage(newCommentLength, vorbisHeaderSizes.getSetupHeaderSize(), vorbisHeaderSizes.getExtraPacketList())) {
			...
			if ((secondPageHeader.getPageLength() < OggPageHeader.MAXIMUM_PAGE_DATA_SIZE)
					&& (((secondPageHeader.getPacketList().size() == 2) && (!secondPageHeader.isLastPacketIncomplete())) || (secondPageHeader.getPacketList().size() > 2))) {
	 ...
	 * @param page
	 */
	private void calculateChecksumOverPage(final ByteBuffer page) {
		...
		final byte[] crc = OggCRCFactory.computeCRC(page.array());
	...
	/**
	 * Create a second Page, and add comment header to it, but page is incomplete may want to add addition header and
	 ...
	 * @throws IOException
	 */
	private ByteBuffer startCreateBasicSecondPage(final OggVorbisTagReader.OggVorbisHeaderSizes vorbisHeaderSizes, final int newCommentLength, final int newSecondPageLength, final OggPageHeader secondPageHeader, final ByteBuffer newComment)
			throws IOException {
		logger.fine("WriteOgg Type 1");
		final byte[] segmentTable = createSegmentTable(newCommentLength, vorbisHeaderSizes.getSetupHeaderSize(), vorbisHeaderSizes.getExtraPacketList());
		final int newSecondPageHeaderLength = OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
		logger.fine("New second page header length:" + newSecondPageHeaderLength);
		logger.fine("No of segments:" + segmentTable.length);

		final ByteBuffer secondPageBuffer = ByteBuffer.allocate(newSecondPageLength + newSecondPageHeaderLength);
		...
		// Page segment table
		for (final byte aSegmentTable : segmentTable)
	...
	/**
	 * Usually can use this method, previously comment and setup header all fit on page 2 and they still do, so just
	 * replace this page. And copy further pages as is.
	 * 
	 * @param vorbisHeaderSizes
	 * @param newCommentLength
	 * @param newSecondPageLength
	 ...
	 * @throws IOException
	 */
	private void replaceSecondPageOnly(final OggVorbisTagReader.OggVorbisHeaderSizes vorbisHeaderSizes, final int newCommentLength, final int newSecondPageLength, final OggPageHeader secondPageHeader, final ByteBuffer newComment,
			final long secondPageHeaderEndPos, final RandomAccessFile raf, final RandomAccessFile rafTemp) throws IOException {
		logger.fine("WriteOgg Type 1");
		final ByteBuffer secondPageBuffer = startCreateBasicSecondPage(vorbisHeaderSizes, newCommentLength, newSecondPageLength, secondPageHeader, newComment);
	...
	/**
	 * Previously comment and/or setup header was on a number of pages now can just replace this page fitting all on 2nd
	 * page, and renumber subsequent sequence pages
	 * 
	 ...
	 */
	private void replaceSecondPageAndRenumberPageSeqs(final OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes, final int newCommentLength, final int newSecondPageLength, final OggPageHeader secondPageHeader,
			final ByteBuffer newComment, final RandomAccessFile raf, final RandomAccessFile rafTemp) throws IOException, CannotReadException, CannotWriteException {
		logger.fine("WriteOgg Type 2");
		final ByteBuffer secondPageBuffer = startCreateBasicSecondPage(originalHeaderSizes, newCommentLength, newSecondPageLength, secondPageHeader, newComment);

		// Add setup header and packets
		final int pageSequence = secondPageHeader.getPageSequence();
		final byte[] setupHeaderData = reader.convertToVorbisSetupHeaderPacketAndAdditionalPackets(originalHeaderSizes.getSetupHeaderStartPosition(), raf);
	...
	/**
	 * CommentHeader extends over multiple pages OR Comment Header doesnt but it's got larger causing some extra packets
	 * to be shifted onto another page.
	 * 
	 * @param originalHeaderSizes
	 * @param newCommentLength
	 * @param secondPageHeader
	 ...
	 */
	private void replacePagesAndRenumberPageSeqs(final OggVorbisTagReader.OggVorbisHeaderSizes originalHeaderSizes, final int newCommentLength, final OggPageHeader secondPageHeader, final ByteBuffer newComment, final RandomAccessFile raf,
			final RandomAccessFile rafTemp) throws IOException, CannotReadException, CannotWriteException {
		int pageSequence = secondPageHeader.getPageSequence();

		// We need to work out how to split the newcommentlength over the pages
		final int noOfCompletePagesNeededForComment = newCommentLength / OggPageHeader.MAXIMUM_PAGE_DATA_SIZE;
		...
		if (noOfCompletePagesNeededForComment > 0)
			for (int i = 0; i < noOfCompletePagesNeededForComment; i++) {
				...
				final byte[] segmentTable = this.createSegments(OggPageHeader.MAXIMUM_PAGE_DATA_SIZE, false);
				final int pageHeaderLength = OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
				final ByteBuffer pageBuffer = ByteBuffer.allocate(pageHeaderLength + OggPageHeader.MAXIMUM_PAGE_DATA_SIZE);
				...
				final ByteBuffer nextPartOfComment = newComment.slice();
		...
		final int lastPageCommentPacketSize = newCommentLength % OggPageHeader.MAXIMUM_PAGE_DATA_SIZE;
		...
		if (!isCommentAndSetupHeaderFitsOnASinglePage(lastPageCommentPacketSize, originalHeaderSizes.getSetupHeaderSize(), originalHeaderSizes.getExtraPacketList())) {
			...
			{
				final byte[] segmentTable = createSegments(lastPageCommentPacketSize, true);
				final int pageHeaderLength = OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
				final ByteBuffer pageBuffer = ByteBuffer.allocate(lastPageCommentPacketSize + pageHeaderLength);
				pageBuffer.order(ByteOrder.LITTLE_ENDIAN);
				pageBuffer.put(secondPageHeader.getRawHeaderData(), 0, OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH - 1);
				pageBuffer.put((byte) segmentTable.length);
				for (final byte aSegmentTable : segmentTable)
			...
			{
				final byte[] segmentTable = this.createSegmentTable(originalHeaderSizes.getSetupHeaderSize(), originalHeaderSizes.getExtraPacketList());
				final int pageHeaderLength = OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH + segmentTable.length;
				final byte[] setupHeaderData = reader.convertToVorbisSetupHeaderPacketAndAdditionalPackets(originalHeaderSizes.getSetupHeaderStartPosition(), raf);
				final ByteBuffer pageBuffer = ByteBuffer.allocate(setupHeaderData.length + pageHeaderLength);
				pageBuffer.order(ByteOrder.LITTLE_ENDIAN);
				pageBuffer.put(secondPageHeader.getRawHeaderData(), 0, OggPageHeader.OGG_PAGE_HEADER_FIXED_LENGTH - 1);
				pageBuffer.put((byte) segmentTable.length);
				for (final byte aSegmentTable : segmentTable)
			...
			final int newSecondPageDataLength = originalHeaderSizes.getSetupHeaderSize() + lastPageCommentPacketSize + originalHeaderSizes.getExtraPacketDataSize();
			newComment.position(newCommentOffset);
			final ByteBuffer lastComment = newComment.slice();
			final ByteBuffer lastHeaderBuffer = startCreateBasicSecondPage(originalHeaderSizes, lastPageCommentPacketSize, newSecondPageDataLength, secondPageHeader, lastComment);
			// Now find the setupheader which is on a different page
			raf.seek(originalHeaderSizes.getSetupHeaderStartPosition());

			// Add setup Header and Extra Packets (although it will fit in this page, it may be over multiple pages in
			// its original form
	 ...
	 * @throws CannotWriteException
	 */
	public void writeRemainingPages(int pageSequence, final RandomAccessFile raf, final RandomAccessFile rafTemp) throws IOException, CannotReadException, CannotWriteException {
		final long startAudio = raf.getFilePointer();
		final long startAudioWritten = rafTemp.getFilePointer();

		// TODO there is a risk we wont have enough memory to create these buffers
		final ByteBuffer bb = ByteBuffer.allocate((int) (raf.length() - raf.getFilePointer()));
		final ByteBuffer bbTemp = ByteBuffer.allocate((int) (raf.length() - raf.getFilePointer()));

		// Read in the rest of the data into bytebuffer and rewind it to start
		raf.getChannel().read(bb);
		bb.rewind();
		while (bb.hasRemaining()) {
			final OggPageHeader nextPage = OggPageHeader.read(bb);

			// Create buffer large enough for next page (header and data) and set byte order to LE so we can use
			// putInt method
			final ByteBuffer nextPageHeaderBuffer = ByteBuffer.allocate(nextPage.getRawHeaderData().length + nextPage.getPageLength());
			nextPageHeaderBuffer.order(ByteOrder.LITTLE_ENDIAN);
			nextPageHeaderBuffer.put(nextPage.getRawHeaderData());
			final ByteBuffer data = bb.slice();
	...
	public void writeRemainingPagesOld(int pageSequence, final RandomAccessFile raf, final RandomAccessFile rafTemp) throws IOException, CannotReadException, CannotWriteException {
		...
		while (raf.getFilePointer() < raf.length()) {
			...
			final OggPageHeader nextPage = OggPageHeader.read(raf);
	 ...
	 * 
	 * @param newCommentLength
	 *            The length of the Vorbis Comment
	 * @param setupHeaderLength
	 *            The length of Setup Header, zero if comment String extends over multiple pages and this is not the
	 *            last page.
	 * @param extraPackets
	 *            If there are packets immediately after setup header in same page, they need including in the segment
	 *            table
	 * @return new segment table.
	 */
	private byte[] createSegmentTable(final int newCommentLength, final int setupHeaderLength, final List<OggPageHeader.PacketStartAndLength> extraPackets) {
		...
		try {
			...
			if (extraPackets.size() > 0) {
				...
				for (final OggPageHeader.PacketStartAndLength packet : extraPackets) {
	 ...
	 * @return new segment table.
	 */
	private byte[] createSegmentTable(final int setupHeaderLength, final List<OggPageHeader.PacketStartAndLength> extraPackets) {
		final ByteArrayOutputStream resultBaos = new ByteArrayOutputStream();
	 ...
	 * 
	 * @param length
	 *            Size of the page which should be represented as 255 byte packets.
	 * @param quitStream
	 *            If true and a length is a multiple of 255 we need another segment table entry with the value of 0.
	 *            Else it's the last stream of the table which is already ended.
	 * @return Array of packet sizes. However only the last packet will differ from 255.
	 *         <p/>
	 */
	...
	private byte[] createSegments(final int length, final boolean quitStream) {
		...
		final byte[] result = new byte[length / OggPageHeader.MAXIMUM_SEGMENT_SIZE + ((length % OggPageHeader.MAXIMUM_SEGMENT_SIZE == 0 && !quitStream) ? 0 : 1)];
		int i = 0;
	 ...
	 * @return true if there is enough room to fit the comment and the setup headers on one page taking into account the
	 *         maximum no of segments allowed per page and zero lacing values.
	 */
	private boolean isCommentAndSetupHeaderFitsOnASinglePage(final int commentLength, final int setupHeaderLength, final List<OggPageHeader.PacketStartAndLength> extraPacketList) {
		int totalDataSize = 0;
		...
		for (final OggPageHeader.PacketStartAndLength extraPacket : extraPacketList)
