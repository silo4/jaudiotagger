...
public class OggVorbisTagReader {
	 ...
	 * @throws IOException
	 */
	public Tag read(final RandomAccessFile raf) throws CannotReadException, IOException {
		logger.config("Starting to read ogg vorbis tag from file:");
		final byte[] rawVorbisCommentData = readRawPacketData(raf);

		// Begin tag reading
		final VorbisCommentTag tag = vorbisCommentReader.read(rawVorbisCommentData, true);
	 ...
	 * @throws IOException
	 */
	public int readOggVorbisRawSize(final RandomAccessFile raf) throws CannotReadException, IOException {
		final byte[] rawVorbisCommentData = readRawPacketData(raf);
	 ...
	 * @throws IOException
	 */
	public byte[] readRawPacketData(final RandomAccessFile raf) throws CannotReadException, IOException {
		...
		final byte[] b = new byte[VorbisHeader.FIELD_PACKET_TYPE_LENGTH + VorbisHeader.FIELD_CAPTURE_PATTERN_LENGTH];
		raf.read(b);
		if (!isVorbisCommentHeader(b))
			throw new CannotReadException("Cannot find comment block (no vorbiscomment header)");

		// Convert the comment raw data which maybe over many pages back into raw packet
		final byte[] rawVorbisCommentData = convertToVorbisCommentPacket(pageHeader, raf);
	 ...
	 */
	public boolean isVorbisCommentHeader(final byte[] headerData) {
		final String vorbis = Utils.getString(headerData, VorbisHeader.FIELD_CAPTURE_PATTERN_POS, VorbisHeader.FIELD_CAPTURE_PATTERN_LENGTH, "ISO-8859-1");
	 ...
	 */
	public boolean isVorbisSetupHeader(final byte[] headerData) {
		final String vorbis = Utils.getString(headerData, VorbisHeader.FIELD_CAPTURE_PATTERN_POS, VorbisHeader.FIELD_CAPTURE_PATTERN_LENGTH, "ISO-8859-1");
	...
	/**
	 * The Vorbis Comment may span multiple pages so we we need to identify the pages they contain and then extract the
	 * packet data from the pages
	 * 
	 ...
	 * @return
	 */
	private byte[] convertToVorbisCommentPacket(final OggPageHeader startVorbisCommentPage, final RandomAccessFile raf) throws IOException, CannotReadException {
		...
		while (true) {
			...
			final OggPageHeader nextPageHeader = OggPageHeader.read(raf);
	...
	/**
	 * The Vorbis Setup Header may span multiple(2) pages, athough it doesnt normally. We pass the start of the file
	 * offset of the OggPage it belongs on, it probably won't be first packet.
	 * 
	 ...
	 * @return
	 */
	public byte[] convertToVorbisSetupHeaderPacket(final long fileOffsetOfStartingOggPage, final RandomAccessFile raf) throws IOException, CannotReadException {
		final ByteArrayOutputStream baos = new ByteArrayOutputStream();
		...
		// Read Page
		final OggPageHeader setupPageHeader = OggPageHeader.read(raf);
	...
	/**
	 * The Vorbis Setup Header may span multiple(2) pages, athough it doesnt normally. We pass the start of the file
	 * offset of the OggPage it belongs on, it probably won't be first packet, also returns any addditional packets that
	 * immediately follow the setup header in original file
	 * 
	 * @param fileOffsetOfStartingOggPage
	 * @param raf
	 * @throws org.jaudiotagger.audio.exceptions.CannotReadException
	 ...
	 * @return
	 */
	public byte[] convertToVorbisSetupHeaderPacketAndAdditionalPackets(final long fileOffsetOfStartingOggPage, final RandomAccessFile raf) throws IOException, CannotReadException {
		final ByteArrayOutputStream baos = new ByteArrayOutputStream();
	 ...
	 * @throws IOException
	 */
	public OggVorbisHeaderSizes readOggVorbisHeaderSizes(final RandomAccessFile raf) throws CannotReadException, IOException {
		...
		while (true) {
			final List<OggPageHeader.PacketStartAndLength> packetList = pageHeader.getPacketList();
	...
	/**
	 * Find the length of the raw packet data and the start position of the ogg page header they start in for the two
	 * OggVorbisHeader we need to know about when writing data (sizes included vorbis header)
	 */
	public static class OggVorbisHeaderSizes {
		...
		OggVorbisHeaderSizes(final long commentHeaderStartPosition, final long setupHeaderStartPosition, final int commentHeaderSize, final int setupHeaderSize, final List<OggPageHeader.PacketStartAndLength> packetList) {
		...
		/**
		 * Return the size required by all the extra packets on same page as setup header, usually there are no packets
		 * immediately after the setup packet.
		 * 
		...
		public int getExtraPacketDataSize() {
			...
			for (final OggPageHeader.PacketStartAndLength packet : packetList)
