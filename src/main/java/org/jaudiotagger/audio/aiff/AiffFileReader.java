...
public class AiffFileReader extends AudioFileReader {
	...
	public AiffFileReader(final RandomAccessFile raf) {
		aiffHeader = new AiffAudioHeader();
		aiffTag = new AiffTag();
	}

	/**
	 * Reads the file and fills in the audio header and tag information. Holds the tag information for later and returns
	 * the audio header.
	 */
	@Override
	 ...
	 */
	private boolean readFileType(final RandomAccessFile raf) throws IOException {
		final String typ = AiffUtil.read4Chars(raf);
		if ("AIFF".equals(typ)) {
	 ...
	 * 
	 */
	protected boolean readChunk(final RandomAccessFile raf, long bytesRemaining) throws IOException {
		Chunk chunk = null;
		final ChunkHeader chunkh = new ChunkHeader();
		if (!chunkh.readHeader(raf))
			return false;
		final int chunkSize = (int) chunkh.getSize();
		bytesRemaining -= chunkSize + 8;

		final String id = chunkh.getID();
		if ("FVER".equals(id))
