...
public class ID3Chunk extends Chunk {
	 ...
	 */
	public ID3Chunk(final ChunkHeader hdr, final RandomAccessFile raf, final AiffTag tag) {
		super(raf, hdr);
		aiffTag = tag;
	}
	...
	public boolean readChunk() throws IOException {
		...
		raf.read(buf);
		final ByteBuffer bb = ByteBuffer.allocate((int) bytesLeft);
		bb.put(buf);
		try {
			id3Tag.read(bb);
	 ...
	 */
	public void parse(final byte[] rawdata, final AiffTag aiffTag) throws IOException, CannotReadException {
	...
	private boolean isId3v2Tag() throws IOException {
		...
		raf.read(buf);
		final String id = new String(buf, "ASCII");
