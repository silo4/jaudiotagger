...
public class AsfFileWriter extends AudioFileWriter {
	...
	protected void writeTag(final Tag tag, final RandomAccessFile raf, final RandomAccessFile rafTemp) throws CannotWriteException, IOException {
		/*
		 * Since this implementation should not change the structure of the ASF file (locations of content description
		 * chunks), we need to read the content description chunk and the extended content description chunk from the
		 * source file. In the second step we need to determine which modifier (asf header or asf extended header) gets
		 * the appropriate modifiers. The following policies are applied: if the source does not contain any descriptor,
		 * the necessary descriptors are appended to the header object.
		 * 
		 * if the source contains only one descriptor in the header extension object, and the other type is needed as
		 * well, the other one will be put into the header extension object.
		 * 
		...
		/*
		 * Now createField modifiers for metadata descriptor and extended content descriptor as implied by the given
		 * Tag.
		 */
		...
		for (int i = 0; i < distribution.length; i++) {
			...
			else // Objects (chunks) will be added here.
			if (i == 0 || i == 2 || i == 1)
