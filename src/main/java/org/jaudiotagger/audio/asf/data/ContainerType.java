 ...
 * <br>
 * The {@link #METADATA_LIBRARY_OBJECT} allows the most variations of data, as well as no size limitation (if it can be
 * stored within a DWORD amount of bytes).<br>
 * 
...
public enum ContainerType {
	 ...
	 * {@linkplain MetadataDescriptor#DWORD_MAXVALUE maximum data length} applies, no language index and stream number
	 * are allowed, as well as no multiple values.
	 */
	 ...
	 * {@linkplain MetadataDescriptor#DWORD_MAXVALUE maximum data size} applies, and no language index and stream number
	 * other than &quot;0&quot; is allowed. Additionally no multiple values are permitted.
	 */
	EXTENDED_CONTENT(GUID.GUID_EXTENDED_CONTENT_DESCRIPTION, 16, false, false, false, false),
	/**
	 * The descriptor is used in a metadata library object. No real size limit (except DWORD range) applies. Stream
	 * numbers and language indexes can be specified.
	 */
	METADATA_LIBRARY_OBJECT(GUID.GUID_METADATA_LIBRARY, 32, true, true, true, true),
	/**
	 * The descriptor is used in a metadata object. The {@linkplain MetadataDescriptor#DWORD_MAXVALUE maximum data size}
	 * applies. Stream numbers can be specified. But no language index (always &quot;0&quot;).
	 */
	METADATA_OBJECT(GUID.GUID_METADATA, 16, false, true, false, true);

	/**
	 * Determines if low has <= index as high, in respect to {@link #getOrdered()}
	 * 
	 * @param low
	 * @param high
	...
	/**
	 * Returns the elements in an order, that indicates more capabilities (ascending).<br>
	 * 
	...
	/**
	 * <code>true</code> if the container may store multiple values of the same metadata descriptor specification
	 * (equality on name, language, and stream).<br>
	 * WindowsMedia players advanced tag editor for example stores the WM/Picture attribute once in the extended content
	 * description, and all others in the metadata library object.
	 */
	private final boolean multiValued;

	/**
	 * if <code>-1</code> a size value has to be compared against {@link #maximumDataLength} because
	 * {@link Long#MAX_VALUE} is exceeded.<br>
	 ...
	 *            The amount of bits that is used to represent an unsigned value for the containers size descriptors.
	 *            Will create a maximum value for {@link #maximumDataLength}. (2 ^ maxDataLenBits -1)
	 * @param guidAllowed
	...
	/**
	 * Calls {@link #checkConstraints(String, byte[], int, int, int)} and actually throws the exception if there is one.
	 * 
	 * @param name
	...
	/**
	 * Checks if the values for a {@linkplain MetadataDescriptor content descriptor} match the contraints of the
	 * container type, and returns a {@link RuntimeException} if the requirements aren't met.
	 * 
	 * @param name
	 *            name of the descriptor
	 * @param data
	...
	/**
	 * Tests if the given value is less than or equal to {@link #getMaximumDataLength()}, and greater or equal to zero.<br>
	 * 
	 * @param value
	 *            The value to test
