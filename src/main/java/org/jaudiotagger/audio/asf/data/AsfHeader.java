 ...
 * This header contains other chunks. Each chunk starts with a 16 byte GUID followed by the length (in bytes) of the
 * chunk (including GUID). The length number takes 8 bytes and is unsigned. Finally the chunk's data appears. <br>
 * 
...
public final class AsfHeader extends ChunkContainer {
	...
	static {
		final Set<GUID> MULTI_CHUNKS = new HashSet<GUID>();
	...
	/**
	 * This method looks for an content description object in this header instance, if not found there, it tries to get
	 * one from a contained ASF header extension object.
	 * 
	...
	/**
	 * This method looks for an extended content description object in this header instance, if not found there, it
	 * tries to get one from a contained ASF header extension object.
	 * 
	 ...
	 * @return a container of specified type, of <code>null</code> if not contained.
	 */
