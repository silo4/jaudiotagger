 ...
 * The values are {@linkplain ContainerType#assertConstraints(String, byte[], int, int, int) checked} against the
 * capability introduced by the given {@link ContainerType} at construction.<br>
 * <br>
 * <b>Limitation</b>: Even though some container types do not restrict the data size to {@link Integer#MAX_VALUE}, this
 * implementation does it (due to java nature).<br>
...
public class MetadataDescriptor implements Comparable<MetadataDescriptor>, Cloneable {
	 ...
	 */
	/*
	 * Note: The maximum data length could be up to a 64-Bit number (unsigned), but java for now handles just int sized
	 * byte[]. Since this class stores all data in primitive byte[] this size restriction is cascaded to all dependent
	 * implementations.
	 */
	...
	/**
	 * the index of the language in the {@linkplain LanguageList language list} this descriptor applies to.<br>
	 * 
	 */
	 ...
	 * 
	 * @param type
	 *            The container type the values (the whole descriptor) is restricted to.
	 * @param propName
	 *            Name of the MetadataDescriptor.
	 * @param propType
	 *            Type of the metadata descriptor. See {@link #descriptorType}
	 ...
	 * @param language
	 *            the index of the language entry in a {@link LanguageList} this descriptor refers to.<br>
	 ...
	 * A boolean will be converted to &quot;1&quot; if <code>true</code>, otherwise &quot;0&quot;.<br>
	 * String will be interpreted as number with radix &quot;10&quot;.<br>
	 * Binary data will be interpreted as the default WORD,DWORD or QWORD binary representation, but only if the data
	 * does not exceed 8 bytes. This precaution is done to prevent creating a number of a multi kilobyte image.<br>
	 ...
	 * 
	 * @return A new metadata descriptor containing the same values as the current one.
	 */
	...
	/**
	 * This method will return a byte array, which can directly be written into an "Extended Content Description"-chunk. <br>
	 * 
	...
	/**
	 * Returns the size (in bytes) this descriptor will take when written to an ASF file.<br>
	 * 
	 * @param type
	...
	public int getCurrentAsfSize(final ContainerType type) {
		/*
		 * 2 bytes name length, 2 bytes name zero term, 2 bytes type, 2 bytes content length
		 */
		int result = 8;
	 ...
	 * 
	 * @return The content in binary representation, as it would be written to asf file. <br>
	 */
	...
	/**
	 * Returns the size (in bytes) the binary representation of the content uses. (length of {@link #getRawData()})<br>
	 * 
	 ...
	 *             if data is invalid for {@linkplain #getContainerType() container}.
	 */
	 ...
	 * <b>Consider</b>: The {@linkplain #containerType requirements} must be held.
	 * 
	 * @param language
	...
	public void setStringValue(final String value) throws IllegalArgumentException {
		if (value == null)
			...
			if (getContainerType().isWithinValueRange(tmp.length))
				...
				this.content = tmp;
			else // Normally a size violation, check if JAudiotagger my truncate
			// the string
	...
	public int writeInto(final OutputStream out, final ContainerType contType) throws IOException {
		...
		/*
		 * Booleans are stored as one byte, if a boolean is written, the data must be converted according to the
		 * container type.
		 */
		byte[] binaryData;
