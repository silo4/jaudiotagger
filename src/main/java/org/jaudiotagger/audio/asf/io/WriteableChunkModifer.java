...
public class WriteableChunkModifer implements ChunkModifier {
	 ...
	 */
	@Override
	public ModificationResult modify(final GUID guid, final InputStream chunk, OutputStream destination) throws IOException { // NOPMD
																																// by
																																// Christian
																																// Laireiter
																																// on
		...
		long oldSize = 0;
		/*
		 * Replace the outputstream with the counting one, only if assert's are evaluated.
		 */
		...
		if (!this.writableChunk.isEmpty()) {
			...
			/*
			 * If assert's are evaluated, we have replaced destination by a CountingOutpustream and can now verify if
