...
public class AsfStreamer {
	 ...
	 * The method assumes, that the GUID has already been read and will write the provided one to the destination.<br>
	 * The chunk length however will be read and used to determine the amount of bytes to copy.
	 * 
	 * @param guid
	 *            GUID of the current chunk.
	 * @param source
	 *            source of an ASF chunk, which is to be located at the chunk length field.
	 * @param destination
	...
	/**
	 * Reads the <code>source</code> and applies the modifications provided by the given <code>modifiers</code>, and
	 * puts it to <code>dest</code>.<br>
	 * Each {@linkplain ChunkModifier modifier} is used only once, so if one should be used multiple times, it should be
	 * added multiple times into the list.<br>
	 * 
	 * @param source
	 *            the source ASF file
	 * @param dest
	...
	public void createModifiedCopy(final InputStream source, final OutputStream dest, final List<ChunkModifier> modifiers) throws IOException {
		...
		if (GUID.GUID_HEADER.equals(readGUID)) {
			...
			/*
			 * bos will get all unmodified and modified header chunks. This is necessary, because the header chunk (and
			 * file properties chunk) need to be adjusted but are written in front of the others.
			 */
	...
	/**
	 * This is a slight variation of {@link #copyChunk(GUID, InputStream, OutputStream)}, it only handles file property
	 * chunks correctly.<br>
	 * The copied chunk will have the file size field modified by the given <code>fileSizeDiff</code> value.
	 * 
	 * @param source
	 *            source of file properties chunk, located at its chunk length field.
	 * @param destination
	 *            the destination to copy the chunk to.
	 * @param fileSizeDiff
	 *            the difference which should be applied. (negative values would subtract the stored file size)
	 * @throws IOException
	 *             on I/O errors.
	 */
