...
public class StreamChunkReader implements ChunkReader {
	...
	public Chunk read(final GUID guid, final InputStream stream, final long chunkStart) throws IOException {
		...
		if (GUID.GUID_AUDIOSTREAM.equals(streamTypeGUID) || GUID.GUID_VIDEOSTREAM.equals(streamTypeGUID)) {
			...
			/*
			 * Read a bit field. (Contains stream number, and whether the stream content is encrypted.)
			 */
			...
			/*
			 * very important to set for every stream type. The size of bytes read by the specific stream type, in order
			 * to skip the remaining unread bytes of the stream chunk.
			 */
			...
			/*
			 * Now skip remainder of chunks bytes. chunk-length - 24 (size of GUID and chunklen) -
			 * streamSpecificBytes(stream type specific data) - 54 (common data)
			 */
