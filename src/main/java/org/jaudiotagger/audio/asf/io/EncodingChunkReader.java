...
class EncodingChunkReader implements ChunkReader {
	...
	public Chunk read(final GUID guid, final InputStream stream, final long chunkStart) throws IOException {
		...
		/*
		 * What do I think of this data, well it seems to be another GUID. Then followed by a UINT16 indicating a length
		 * of data following (by half). My test files just had the length of one and a two bytes zero.
		 */
		stream.skip(20);
		readBytes += 20;
