...
/**
 * This class represents a reader implementation, which is able to read ASF objects (chunks) which store other objects
 * (chunks) within them.<br>
 * 
...
abstract class ChunkContainerReader<ChunkType extends ChunkContainer> implements ChunkReader {
	...
	/**
	 * If <code>true</code> due to a {@linkplain #register(Class) registered} chunk reader, all {@link InputStream}
	 * objects passed to {@link #read(GUID, InputStream, long)} must support mark/reset.
	 */
	 ...
	 * 
	 * @param toRegister
	 *            List of {@link ChunkReader} class instances, which are to be utilized by the instance.
	 * @param readChunkOnce
	 *            if <code>true</code>, each chunk type (identified by chunk GUID) will handled only once, if a reader
	 *            is available, other chunks will be discarded.
	 */
	...
	/**
	 * This method is called by {@link #read(GUID, InputStream, long)} in order to create the resulting object.
	 * Implementations of this class should now return a new instance of their implementation specific result <b>AND</b>
	 * all data should be read, until the list of chunks starts. (The {@link ChunkContainer#getChunkEnd()} must return a
	 * sane result, too)<br>
	 * 
	...
	/**
	 * Gets a configured {@linkplain ChunkReader reader} instance for ASF objects (chunks) with the specified
	 * <code>guid</code>.
	 * 
	 * @param guid
	 ...
	 *             Read errors.
	 * @throws IllegalArgumentException
	 *             If one used {@link ChunkReader} could {@linkplain ChunkReader#canFail() fail} and the stream source
	...
	public ChunkType read(final GUID guid, final InputStream stream, final long chunkStart) throws IOException, IllegalArgumentException {
		...
		/*
		 * now read implementation specific information until the chunk collection starts and create the resulting
		 * object.
		 */
		...
		while (currentPosition < result.getChunkEnd()) {
			...
			Chunk chunk;
			/*
			 * If one reader tells it could fail (new method), then check the input stream for mark/reset. And use it if
			 * failed.
			 */
