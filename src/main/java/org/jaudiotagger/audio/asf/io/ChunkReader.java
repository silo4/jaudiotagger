...
public interface ChunkReader {
	 ...
	 * The current Use would be a modified version of {@link StreamChunkReader}, which is configured to only manage
	 * audio streams. However, the primary GUID for audio and video streams is the same. So if a stream shows itself to
	 * be a video stream, the reader would return <code>null</code>.<br>
	 * 
	 * @return <code>true</code>, if further analysis of the chunk can show, that the reader is not applicable, despite
	 *         the header GUID {@linkplain #getApplyingIds() identification} told it can handle the chunk.
	 */
	boolean canFail();
	 ...
	 *            No {@link GUID} is expected at the currents stream position. The length of the chunk is about to
	 *            follow.
