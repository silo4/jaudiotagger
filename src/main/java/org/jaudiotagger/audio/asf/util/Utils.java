...
public class Utils {
	...
	/**
	 * This method checks given string will not exceed limit in bytes[] when converted UTF-16LE encoding (2 bytes per
	 * character) and checks whether the length doesn't exceed 65535 bytes. <br>
	 * 
	 * @param value
	 *            The string to check.
	 ...
	 */
	public static void checkStringLengthNullSafe(final String value) throws IllegalArgumentException {
		if (value != null)
	 ...
	 * 
	 * @param value
	 *            String to check for null
	 * @return true unless string is too long
	 */
	public static boolean isStringLengthValidNullSafe(final String value) {
		if (value != null)
			if (value.length() > MAXIMUM_STRING_LENGTH_ALLOWED)
				return false;
		return true;
	 ...
	 * @throws IOException
	 *             on I/O errors, and if the source stream depletes before all bytes have been copied.
	 */
	public static void copy(final InputStream source, final OutputStream dest, final long amount) throws IOException {
		...
		while (copied < amount) {
			...
			final int read = source.read(buf, 0, toRead);
			if (read == -1)
	...
	/**
	 * This method will create a byte[] at the size of <code>byteCount</code> and insert the bytes of <code>value</code>
	 * (starting from lowset byte) into it. <br>
	 * You can easily create a Word (16-bit), DWORD (32-bit), QWORD (64 bit) out of the value, ignoring the original
	 * type of value, since java automatically performs transformations. <br>
	 ...
	 * @return A byte[] with the size of <code>byteCount</code> containing the lower byte values of <code>value</code>.
	 */
	...
	/**
	 * Since date values in ASF files are given in 100 ns steps since first january of 1601 a little conversion must be
	 * done. <br>
	 ...
	 */
	/*
	 * Old method that ran very slowely and doesnt logical correct, how does dividing something at 10-4 by 10,000
	 * convert it to 10 -3 public static GregorianCalendar getDateOf(final BigInteger fileTime) { final
	 * GregorianCalendar result = new GregorianCalendar(1601, 0, 1); // lose anything beyond milliseconds, because
	 * calendar can't handle // less value BigInteger time = fileTime.divide(new BigInteger("10000")); //$NON-NLS-1$
	 * final BigInteger maxInt = new BigInteger(String .valueOf(Integer.MAX_VALUE)); while (time.compareTo(maxInt) > 0)
	 * { result.add(Calendar.MILLISECOND, Integer.MAX_VALUE); time = time.subtract(maxInt); }
	 * result.add(Calendar.MILLISECOND, time.intValue()); return result; }
	 */
	...
	public static GregorianCalendar getDateOf(final BigInteger fileTime) {
		...
		final BigInteger time = fileTime.divide(new BigInteger("10"));
		// Construct Date taking into the diff between 1601 and 1970
		final Date date = new Date(time.longValue() - DIFF_BETWEEN_ASF_DATE_AND_JAVA_DATE);
		result.setTime(date);
		return result;
	}
	...
	/**
	 * Reads 8 bytes from stream and interprets them as a UINT64 which is returned as {@link BigInteger}.<br>
	 * 
	 * @param stream
	 *            stream to readm from.
	 * @return a BigInteger which represents the read 8 bytes value.
	 * @throws IOException
	 *             if problem reading bytes
	 */
	public static BigInteger readBig64(final InputStream stream) throws IOException {
		final byte[] bytes = new byte[8];
		final byte[] oa = new byte[8];
	 ...
	 *             on I/O errors.
	 */
	public static byte[] readBinary(final InputStream stream, final long size) throws IOException {
	...
	/**
	 * This method reads a UTF-16 String, which length is given on the number of characters it consists of. <br>
	 ...
	 *             read errors
	 */
	public static String readCharacterSizedString(final InputStream stream) throws IOException {
		final StringBuilder result = new StringBuilder();
		final int strLen = readUINT16(stream);
	 ...
	 * The ASF specification recommends that those strings end with a terminating zero. However it also says that it is
	 * not always the case.
	 * 
	 * @param stream
	 *            Input source
	 ...
	 *             read errors.
	 */
	public static String readFixedSizeUTF16Str(final InputStream stream, final int strLen) throws IOException {
		byte[] strBytes = new byte[strLen];
		final int read = stream.read(strBytes);
		if (read == strBytes.length) {
			if (strBytes.length >= 2)
				/*
				 * Zero termination is recommended but optional. So check and if, remove.
				 */
				if (strBytes[strBytes.length - 1] == 0 && strBytes[strBytes.length - 2] == 0) {
					final byte[] copy = new byte[strBytes.length - 2];
	...
	/**
	 * This Method reads a GUID (which is a 16 byte long sequence) from the given <code>raf</code> and creates a
	 * wrapper. <br>
	 * <b>Warning </b>: <br>
	 * There is no way of telling if a byte sequence is a guid or not. The next 16 bytes will be interpreted as a guid,
	 ...
	 */
	public static GUID readGUID(final InputStream stream) throws IOException {
		if (stream == null)
			throw new IllegalArgumentException("Argument must not be null"); //$NON-NLS-1$
		final int[] binaryGuid = new int[GUID.GUID_LENGTH];
	 ...
	 *             on I/O Errors.
	 */
	public static int readUINT16(final InputStream stream) throws IOException {
	 ...
	 *             on I/O Errors.
	 */
	public static long readUINT32(final InputStream stream) throws IOException {
		long result = 0;
	 ...
	 */
	public static long readUINT64(final InputStream stream) throws IOException {
		long result = 0;
	 ...
	 * The String is terminated with as 16-bit ZERO. <br>
	 * 
	 * @param stream
	 *            Input source
	 * @return read String.
	 ...
	 *             read errors.
	 */
	public static String readUTF16LEStr(final InputStream stream) throws IOException {
		final int strLen = readUINT16(stream);
		...
		if (read == strLen || (strLen == 0 && read == -1)) {
			...
			if (buf.length >= 2)
				if (buf[buf.length - 1] == 0 && buf[buf.length - 2] == 0) {
					final byte[] copy = new byte[buf.length - 2];
	 ...
	 *             On I/O errors
	 */
	public static void writeUINT16(final int number, final OutputStream out) throws IOException {
		if (number < 0)
			throw new IllegalArgumentException("positive value expected."); //$NON-NLS-1$
		final byte[] toWrite = new byte[2];
		for (int i = 0; i <= 8; i += 8)
	 ...
	 *             On I/O errors
	 */
	public static void writeUINT32(final long number, final OutputStream out) throws IOException {
		if (number < 0)
			throw new IllegalArgumentException("positive value expected."); //$NON-NLS-1$
		final byte[] toWrite = new byte[4];
		for (int i = 0; i <= 24; i += 8)
			toWrite[i / 8] = (byte) ((number >> i) & 0xFF);
		out.write(toWrite);
	}
	 ...
	 *             On I/O errors
	 */
	public static void writeUINT64(final long number, final OutputStream out) throws IOException {
		if (number < 0)
			throw new IllegalArgumentException("positive value expected."); //$NON-NLS-1$
		final byte[] toWrite = new byte[8];
		for (int i = 0; i <= 56; i += 8)
			toWrite[i / 8] = (byte) ((number >> i) & 0xFF);
		out.write(toWrite);
	}
