...
/**
 * This class provides functionality to convert {@link org.jaudiotagger.audio.asf.data.AsfHeader}objects into
...
public final class TagConverter {
	...
	/**
	 * This method assigns those tags of <code>tag</code> which are defined to be common by jaudiotagger. <br>
	 * 
	 * @param tag
	 ...
	 */
	public static void assignCommonTagValues(final Tag tag, final MetadataContainer description) {
		...
		if (!Utils.isBlank(tag.getFirst(FieldKey.GENRE))) {
			...
			description.removeDescriptorsByName(tmp.getName());
			description.addDescriptor(tmp);
			final Integer genreNum = GenreTypes.getInstanceOf().getIdForName(tag.getFirst(FieldKey.GENRE));
	...
	/**
	 * This method creates a {@link Tag}and fills it with the contents of the given {@link AsfHeader}.<br>
	 * 
	 * @param source
	 *            The ASF header which contains the information. <br>
	 * @return A Tag with all its values.
	 */
	public static AsfTag createTagOf(final AsfHeader source) {
		// TODO do we need to copy here.
		final AsfTag result = new AsfTag(true);
		for (int i = 0; i < ContainerType.values().length; i++) {
			final MetadataContainer current = source.findMetadataContainer(ContainerType.values()[i]);
			if (current != null) {
				final List<MetadataDescriptor> descriptors = current.getDescriptors();
	...
	/**
	 * This method distributes the tags fields among the {@linkplain ContainerType#getOrdered()}
	 * {@linkplain MetadataContainer containers}.
	 * 
	 * @param tag
