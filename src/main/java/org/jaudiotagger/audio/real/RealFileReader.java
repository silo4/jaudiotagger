...
public class RealFileReader extends AudioFileReader {
	...
	private RealChunk findPropChunk(final RandomAccessFile raf) throws IOException, CannotReadException {
		final RealChunk rmf = RealChunk.readChunk(raf);
		final RealChunk prop = RealChunk.readChunk(raf);
		return prop;
	}

	private RealChunk findContChunk(final RandomAccessFile raf) throws IOException, CannotReadException {
		final RealChunk rmf = RealChunk.readChunk(raf);
		final RealChunk prop = RealChunk.readChunk(raf);
	...
	protected Tag getTag(final RandomAccessFile raf) throws CannotReadException, IOException {
		...
		} catch (final FieldDataInvalidException fdie) {
