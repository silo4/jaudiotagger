...
/**
 * A <code>DefaultMutableTreeNode</code> is a general-purpose node in a tree data structure. For examples of using
 * default mutable tree nodes, see <a href="http://java.sun.com/docs/books/tutorial/uiswing/components/tree.html">How to
 * Use Trees</a> in <em>The Java Tutorial.</em>
 * 
 * <p>
 * 
 * A tree node may have at most one parent and 0 or more children. <code>DefaultMutableTreeNode</code> provides
 * operations for examining and modifying a node's parent and children and also operations for examining the tree that
 * the node is a part of. A node's tree is the set of all nodes that can be reached by starting at the node and
 * following all the possible links to parents and children. A node with no parent is the root of its tree; a node with
 * no children is a leaf. A tree may consist of many subtrees, each node acting as the root for its own subtree.
 * <p>
 * This class provides enumerations for efficiently traversing a tree or subtree in various orders or for following the
 * path between two nodes. A <code>DefaultMutableTreeNode</code> may also hold a reference to a user object, the use of
 * which is left to the user. Asking a <code>DefaultMutableTreeNode</code> for its string representation with
 * <code>toString()</code> returns the string representation of its user object.
 * <p>
 * <b>This is not a thread safe class.</b>If you intend to use a DefaultMutableTreeNode (or a tree of TreeNodes) in more
 * than one thread, you need to do your own synchronizing. A good convention to adopt is synchronizing on the root node
 * of a tree.
 * <p>
 * While DefaultMutableTreeNode implements the MutableTreeNode interface and will allow you to add in any implementation
 * of MutableTreeNode not all of the methods in DefaultMutableTreeNode will be applicable to all MutableTreeNodes
 * implementations. Especially with some of the enumerations that are provided, using some of these methods assumes the
 * DefaultMutableTreeNode contains only DefaultMutableNode instances. All of the TreeNode/MutableTreeNode methods will
...
public class DefaultMutableTreeNode extends Object implements Cloneable, MutableTreeNode, Serializable {
	...
	/**
	 * Removes <code>newChild</code> from its present parent (if it has a parent), sets the child's parent to this node,
	 * and then adds the child to this node's child array at index <code>childIndex</code>. <code>newChild</code> must
	...
	/**
	 * Sets this node's parent to <code>newParent</code> but does not change the parent's child array. This method is
	 * called from <code>insert()</code> and <code>remove()</code> to reassign a child's parent, it should not be
	...
	/**
	 * Returns the index of the specified child in this node's child array. If the specified node is not a child of this
	 * node, returns <code>-1</code>. This method performs a linear search and is O(n) where n is the number of
	 * children.
	 * 
	 * @param aChild
	 ...
	 * @return an int giving the index of the node in this node's child array, or <code>-1</code> if the specified node
	 *         is a not a child of this node
	 */
	@Override
	...
	/**
	 * Creates and returns a forward-order enumeration of this node's children. Modifying this node's child array
	 * invalidates any child enumerations created before the modification.
	 * 
	...
	/**
	 * Determines whether or not this node is allowed to have children. If <code>allows</code> is false, all of this
	...
	/**
	 * Removes the subtree rooted at this node from the tree, giving this node a null parent. Does nothing if this node
	...
	/**
	 * Removes all of this node's children, setting their parents to null. If this node has no children, this method
	 * does nothing.
	 */
	...
	/**
	 * Returns the nearest common ancestor to this node and <code>aNode</code>. Returns null, if no such ancestor exists
	 * -- if this node and <code>aNode</code> are in different trees or if <code>aNode</code> is null. A node is
	...
	/**
	 * Returns true if and only if <code>aNode</code> is in the same tree as this node. Returns false if
	 * <code>aNode</code> is null.
	 * 
	 * @see #getSharedAncestor
	 * @see #getRoot
	 * @return true if <code>aNode</code> is in the same tree as this node; false if <code>aNode</code> is null
	 */
	...
	/**
	 * Returns the depth of the tree rooted at this node -- the longest distance from this node to a leaf. If this node
	 * has no children, returns 0. This operation is much more expensive than <code>getLevel()</code> because it must
	...
	public int getDepth() {
		Object last = null;
		final Enumeration enum_ = breadthFirstEnumeration();
	 ...
	 * 
	 * @return an array of TreeNode objects giving the path, where the first element in the path is the root and the
	 *         last element is this node.
	 */
	...
	protected TreeNode[] getPathToRoot(final TreeNode aNode, int depth) {
		...
		/*
		 * Check for null, in case someone passed in a null node, or they passed in an element that isn't rooted at
		 * root.
		 */
		if (aNode == null) {
			if (depth == 0)
	...
	/**
	 * Returns the user object path, from the root, to get to this node. If some of the TreeNodes in the path have null
	 * user objects, the returned path will contain nulls.
	 */
	...
	/**
	 * Returns true if this node is the root of the tree. The root is the only node in the tree with a null parent;
	...
	/**
	 * Returns the node that follows this node in a preorder traversal of this node's tree. Returns null if this node is
	 * the last node of the traversal. This is an inefficient way to traverse the entire tree; use an enumeration,
	 * instead.
	 * 
	...
	/**
	 * Returns the node that precedes this node in a preorder traversal of this node's tree. Returns <code>null</code>
	 * if this node is the first node of the traversal -- the root of the tree. This is an inefficient way to traverse
	...
	/**
	 * Creates and returns an enumeration that traverses the subtree rooted at this node in preorder. The first node
	 * returned by the enumeration's <code>nextElement()</code> method is this node.
	 * <P>
	 * 
	...
	/**
	 * Creates and returns an enumeration that traverses the subtree rooted at this node in postorder. The first node
	 * returned by the enumeration's <code>nextElement()</code> method is the leftmost leaf. This is the same as a
	...
	/**
	 * Creates and returns an enumeration that traverses the subtree rooted at this node in breadth-first order. The
	 * first node returned by the enumeration's <code>nextElement()</code> method is this node.
	 * <P>
	 * 
	 * Modifying the tree by inserting, removing, or moving a node invalidates any enumerations created before the
	 * modification.
	...
	/**
	 * Creates and returns an enumeration that traverses the subtree rooted at this node in depth-first order. The first
	 * node returned by the enumeration's <code>nextElement()</code> method is the leftmost leaf. This is the same as a
	 * postorder traversal.
	 * <P>
	 * 
	 * Modifying the tree by inserting, removing, or moving a node invalidates any enumerations created before the
	...
	/**
	 * Creates and returns an enumeration that follows the path from <code>ancestor</code> to this node. The
	 * enumeration's <code>nextElement()</code> method first returns <code>ancestor</code>, then the child of
	 * <code>ancestor</code> that is an ancestor of this node, and so on, and finally returns this node. Creation of the
	 * enumeration is O(m) where m is the number of nodes between this node and <code>ancestor</code>, inclusive. Each
	...
	/**
	 * Returns the child in this node's child array that immediately follows <code>aChild</code>, which must be a child
	 * of this node. If <code>aChild</code> is the last child, returns null. This method performs a linear search of
	 * this node's children for <code>aChild</code> and is O(n) where n is the number of children; to traverse the
	 * entire array of children, use an enumeration instead.
	 * 
	 * @see #children
	 * @exception IllegalArgumentException
	 *                if <code>aChild</code> is null or is not a child of this node
	...
	/**
	 * Returns the child in this node's child array that immediately precedes <code>aChild</code>, which must be a child
	 * of this node. If <code>aChild</code> is the first child, returns null. This method performs a linear search of
	 * this node's children for <code>aChild</code> and is O(n) where n is the number of children.
	 * 
	 * @exception IllegalArgumentException
	 *                if <code>aChild</code> is null or is not a child of this node
	...
	/**
	 * Returns true if <code>anotherNode</code> is a sibling of (has the same parent as) this node. A node is its own
	 * sibling. If <code>anotherNode</code> is null, returns false.
	 * 
	 * @param anotherNode
	...
	/**
	 * Returns the number of siblings of this node. A node is its own sibling (if it has no parent or no siblings, this
	...
	/**
	 * Returns the next sibling of this node in the parent's children array. Returns null if this node has no parent or
	 * is the parent's last child. This method performs a linear search that is O(n) where n is the number of children;
	...
	/**
	 * Returns the previous sibling of this node in the parent's children array. Returns null if this node has no parent
	 * or is the parent's first child. This method performs a linear search that is O(n) where n is the number of
	 * children.
	 * 
	 ...
	 * leaf. Returns this node if it is a leaf.
	 * 
	 * @see #isLeaf
	 * @see #isNodeDescendant
	 ...
	 * <p>
	 * In this implementation of the <code>MutableNode</code> interface, this operation is very inefficient. In order to
	 * determine the next node, this method first performs a linear search in the parent's child-list in order to find
	 * the current node.
	 * <p>
	 * That implementation makes the operation suitable for short traversals from a known position. But to traverse all
	 * of the leaves in the tree, you should use <code>depthFirstEnumeration</code> to enumerate the nodes in the tree
	 * and use <code>isLeaf</code> on each node to determine which are leaves.
	 * 
	 * @see #depthFirstEnumeration
	 * @see #isLeaf
	 ...
	 * <p>
	 * In this implementation of the <code>MutableNode</code> interface, this operation is very inefficient. In order to
	 * determine the previous node, this method first performs a linear search in the parent's child-list in order to
	...
	/**
	 * Overridden to make clone public. Returns a shallow copy of this node; the new node has no parent or children and
	 * has a reference to the same user object, if any.
	 * 
	...
	final class PreorderEnumeration implements Enumeration<TreeNode> {
		...
		public TreeNode nextElement() {
			...
			final TreeNode node = (TreeNode) enumer.nextElement();
	...
	final class BreadthFirstEnumeration implements Enumeration<TreeNode> {
		...
		@Override
		public TreeNode nextElement() {
			final Enumeration enumer = (Enumeration) queue.firstObject();
			final TreeNode node = (TreeNode) enumer.nextElement();
			final Enumeration children = node.children();
		...
		final class Queue {
			...
			public Object dequeue() {
				...
				final Object retval = head.object;
	...
	final class PathBetweenNodesEnumeration implements Enumeration<TreeNode> {
		...
		public PathBetweenNodesEnumeration(final TreeNode ancestor, final TreeNode descendant) {
			super();
