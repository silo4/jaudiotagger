...
/**
 * Represents a path to a node. A TreePath is an array of Objects that are vended from a TreeModel. The elements of the
 * array are ordered such that the root is always the first element (index 0) of the array. TreePath is Serializable,
 * but if any components of the path are not serializable, it will not be written out.
 * <p>
 * For further information and examples of using tree paths, see <a
...
public class TreePath extends Object implements Serializable {
	...
	/**
	 * Constructs a path from an array of Objects, uniquely identifying the path from the root of the tree to a specific
	 * node, as returned by the tree's data model.
	 * <p>
	 * The model is free to return an array of any Objects it needs to represent the path. The DefaultTreeModel returns
	 * an array of TreeNode objects. The first TreeNode in the path is the root of the tree, the last TreeNode is the
	 * node identified by the path.
	 * 
	 * @param path
	...
	/**
	 * Constructs a TreePath containing only a single element. This is usually used to construct a TreePath for the the
	 * root of the TreeModel.
	 * <p>
	 * 
	 * @param singlePath
	 ...
	 * .
	 */
	protected TreePath(final TreePath parent, final Object lastElement) {
		if (lastElement == null)
			throw new IllegalArgumentException("path in TreePath must be non null.");
		parentPath = parent;
	...
	/**
	 * Primarily provided for subclasses that represent paths in a different manner. If a subclass uses this
	 * constructor, it should also override the <code>getPath</code>, <code>getPathCount</code>, and
	 * <code>getPathComponent</code> methods, and possibly the <code>equals</code> method.
	 */
	protected TreePath() {}

	/**
	 * Returns an ordered array of Objects containing the components of this TreePath. The first element (index 0) is
	 * the root.
	 * 
	...
	/**
	 * Returns the last component of this path. For a path returned by DefaultTreeModel this will return an instance of
	 * TreeNode.
	 * 
	...
	/**
	 * Tests two TreePaths for equality by checking each element of the paths for equality. Two paths are considered
	 * equal if they are of the same length, and contain the same elements (<code>.equals</code>).
	 * 
	 * @param o
	...
	/**
	 * Returns the hashCode for the object. The hash code of a TreePath is defined to be the hash code of the last
	...
	/**
	 * Returns true if <code>aTreePath</code> is a descendant of this TreePath. A TreePath P1 is a descendant of a
	 * TreePath P2 if P1 contains all of the components that make up P2's path. For example, if this object has the path
	 * [a, b], and <code>aTreePath</code> has the path [a, b, c], then <code>aTreePath</code> is a descendant of this
	 * object. However, if <code>aTreePath</code> has the path [a], then it is not a descendant of this object. By this
	 * definition a TreePath is always considered a descendant of itself. That is,
	...
	/**
	 * Returns a new path containing all the elements of this object plus <code>child</code>. <code>child</code> will be
	 * the last element of the newly created TreePath. This will throw a NullPointerException if child is null.
	 */
