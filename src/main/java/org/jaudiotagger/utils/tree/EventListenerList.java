 ...
 * the instance using the list. It is the responsiblity of the class using the EventListenerList to provide type-safe
 ...
 * 
 * The main benefits that this class provides are that it is relatively cheap in the case of no listeners, and it
 * provides serialization for event-listener lists in a single place, as well as a degree of MT safety (when used
 * correctly).
 * 
...
public class EventListenerList implements Serializable {
	...
	/**
	 * Passes back the event listener list as an array of ListenerType-listener pairs. Note that for performance
	 * reasons, this implementation passes back the actual data structure in which the listener data is stored
	 * internally! This method is guaranteed to pass back a non-null array, so that no null-checking is required in fire
	 * methods. A zero-length array of Object should be returned if there are currently no listeners.
	 * 
	 * WARNING!!! Absolutely NO modification of the data contained in this array should be made -- if any such
	 * manipulation is necessary, it should be done on a copy of the array returned rather than the array itself.
	 */
	...
	public <T extends EventListener> T[] getListeners(final Class<T> t) {
		final Object[] lList = listenerList;
		final int n = getListenerCount(lList, t);
		final T[] result = (T[]) Array.newInstance(t, n);
		int j = 0;
	...
	private int getListenerCount(final Object[] list, final Class t) {
		int count = 0;
	 ...
	 */
	public synchronized <T extends EventListener> void add(final Class<T> t, final T l) {
		if (l == null)
		...
		if (listenerList == NULL_ARRAY)
			...
			final int i = listenerList.length;
	 ...
	 */
	public synchronized <T extends EventListener> void remove(final Class<T> t, final T l) {
		if (l == null)
			// In an ideal world, we would do an assertion here
			// to help developers know they are probably doing
			// something wrong
		...
		// If so, remove it
		if (index != -1) {
			final Object[] tmp = new Object[listenerList.length - 2];
	...
	private void readObject(final ObjectInputStream s) throws IOException, ClassNotFoundException {
		...
		while (null != (listenerTypeOrNull = s.readObject())) {
			...
			final EventListener l = (EventListener) s.readObject();
