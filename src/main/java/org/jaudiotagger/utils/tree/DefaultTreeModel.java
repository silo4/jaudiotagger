...
/**
 * A simple tree data model that uses TreeNodes. For further information and examples that use DefaultTreeModel, see <a
 * href="http://java.sun.com/docs/books/tutorial/uiswing/components/tree.html">How to Use Trees</a> in
...
public class DefaultTreeModel implements Serializable, TreeModel {
	...
	/**
	 * Determines how the <code>isLeaf</code> method figures out if a node is a leaf node. If true, a node is a leaf
	 * node if it does not allow children. (If it allows children, it is not a leaf node, even if no children are
	 * present.) That lets you distinguish between <i>folder</i> nodes and <i>file</i> nodes in a file system, for
	 * example.
	 * <p>
	...
	/**
	 * Sets whether or not to test leafness by asking getAllowsChildren() or isLeaf() to the TreeNodes. If newvalue is
	 * true, getAllowsChildren() is messaged, otherwise isLeaf() is messaged.
	 */
	public void setAsksAllowsChildren(final boolean newValue) {
	 ...
	 * 
	 * @return true if only nodes which do not allow children are leaf nodes, false if nodes which have no children
	...
	/**
	 * Sets the root to <code>root</code>. A null <code>root</code> implies the tree is to display nothing, and is
	 * legal.
	 */
	...
	/**
	 * Returns the index of child in parent. If either the parent or child is <code>null</code>, returns -1.
	 * 
	 * @param parent
	 *            a note in the tree, obtained from this data source
	 * @param child
	 *            the node we are interested in
	 * @return the index of the child in the parent, or -1 if either the parent or the child is <code>null</code>
	 */
	@Override
	...
	/**
	 * Returns the child of <I>parent</I> at index <I>index</I> in the parent's child array. <I>parent</I> must be a
	 * node previously obtained from this data source. This should not return null if <i>index</i> is a valid index for
	 * <i>parent</i> (that is <i>index</i> >= 0 && <i>index</i> < getChildCount(<i>parent</i>)).
	 * 
	 * @param parent
	...
	/**
	 * Returns the number of children of <I>parent</I>. Returns 0 if the node is a leaf or if it has no children.
	 * <I>parent</I> must be a node previously obtained from this data source.
	 * 
	 * @param parent
	 *            a node in the tree, obtained from this data source
	...
	/**
	 * Invoke this method if you've modified the {@code TreeNode}s upon which this model depends. The model will notify
	 * all of its listeners that the model has changed.
	 */
	public void reload() {
		reload(root);
	}
	...
	/**
	 * This sets the user object of the TreeNode identified by path and posts a node changed. If you use custom user
	...
	/**
	 * Invoked this to insert newChild at location index in parents children. This will then message nodesWereInserted
	 * to create the appropriate event. This is the preferred way to add children as it will create the appropriate
	 * event.
	 */
	...
	/**
	 * Message this to remove node from its parent. This will message nodesWereRemoved to create the appropriate event.
	...
	public void nodeChanged(final TreeNode node) {
		if (listenerList != null && node != null) {
			...
			if (parent != null) {
				final int anIndex = parent.getIndex(node);
				if (anIndex != -1) {
	...
	/**
	 * Invoke this method if you've modified the {@code TreeNode}s upon which this model depends. The model will notify
	 * all of its listeners that the model has changed below the given node.
	 * 
	 * @param node
	 ...
	 */
	public void nodesWereInserted(final TreeNode node, final int[] childIndices) {
	...
	/**
	 * Invoke this method after you've removed some TreeNodes from node. childIndices should be the index of the removed
	 * elements and must be sorted in ascending order. And removedChildren should be the array of the children objects
	 * that were removed.
	 */
	 ...
	 * tree.
	 */
	public void nodesChanged(final TreeNode node, final int[] childIndices) {
		if (node != null)
	 ...
	 * <code><em>Foo</em>Listener.class</code>. For example, you can query a <code>DefaultTreeModel</code>
	 ...
	 *                if <code>listenerType</code> doesn't specify a class or interface that implements
	 *                <code>java.util.EventListener</code>
	 * 
