 ...
 * <p/>
 * <p>
 * This is in the same format whether encoded with Ogg or Flac except the framing bit is only present when used within
 * Ogg Vorbis
 * <p/>
 * 
 * <pre>
...
public class VorbisCommentReader {
	...
	/**
	 * max comment length that jaudiotagger can handle, this isnt the maximum column length allowed but we dont dont
	 * allow comments larger than this because of problem with allocating memory (10MB shoudl be fine for all apps)
	 */
	 ...
	 */
	public VorbisCommentTag read(final byte[] rawdata, final boolean isFramingBit) throws IOException, CannotReadException {
		...
		final int vendorStringLength = Utils.getIntLE(b);
		...
		final int userComments = Utils.getIntLE(b);
		...
		for (int i = 0; i < userComments; i++) {
			...
			final int commentLength = Utils.getIntLE(b);
			...
			if (commentLength > JAUDIOTAGGER_MAX_COMMENT_LENGTH) {
				...
				final VorbisCommentTagField fieldComment = new VorbisCommentTagField(b);
