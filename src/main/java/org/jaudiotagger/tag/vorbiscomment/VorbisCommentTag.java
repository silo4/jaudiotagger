...
public class VorbisCommentTag extends AbstractTag {
	...
	public static VorbisCommentTag createNewTag() {
		final VorbisCommentTag tag = new VorbisCommentTag();
	 ...
	 * <p/>
	 * We dont want this to be blank, when written to file this field is written to a different location to all other
	 * fields but user of library can just reat it as another field
	 * 
	 * @param vendor
	 */
	 ...
	 * @throws org.jaudiotagger.tag.KeyNotFoundException
	 */
	public List<TagField> get(final VorbisCommentFieldKey vorbisCommentKey) throws KeyNotFoundException {
	 ...
	 * @throws org.jaudiotagger.tag.KeyNotFoundException
	 */
	public String getFirst(final VorbisCommentFieldKey vorbisCommentKey) throws KeyNotFoundException {
		if (vorbisCommentKey == null)
			throw new KeyNotFoundException();
	...
	public boolean hasField(final FieldKey genericKey) {
		final VorbisCommentFieldKey vorbisFieldKey = tagFieldToOggField.get(genericKey);
	 ...
	 * @throws org.jaudiotagger.tag.KeyNotFoundException
	 */
	public void deleteField(final VorbisCommentFieldKey vorbisCommentFieldKey) throws KeyNotFoundException {
		if (vorbisCommentFieldKey == null)
			throw new KeyNotFoundException();
	...
	public byte[] getArtworkBinaryData() {
		final String base64data = this.getFirst(VorbisCommentFieldKey.COVERART);
		final byte[] rawdata = Base64Coder.decode(base64data.toCharArray());
		return rawdata;
	}
	 ...
	 * <p/>
	 * <p>
	 * Overridden because check for size of one because there is always a vendor tag unless just created an empty vorbis
	 * tag as part of flac tag in which case size could be zero
	 * 
	...
	public List<Artwork> getArtworkList() {
		final List<Artwork> artworkList = new ArrayList<Artwork>(1);
		...
		final List<TagField> metadataBlockPics = this.get(VorbisCommentFieldKey.METADATA_BLOCK_PICTURE);
		for (final TagField tagField : metadataBlockPics)
			try {
				final byte[] imageBinaryData = Base64Coder.decode(((TagTextField) tagField).getContent());
				final MetadataBlockDataPicture coverArt = new MetadataBlockDataPicture(ByteBuffer.wrap(imageBinaryData));
	...
	/**
	 * Create MetadataBlockPicture field, this is the preferred way of storing artwork in VorbisComment tag now but has
	 * to be base encoded to be stored in VorbisComment
	 * 
	 * @return MetadataBlockDataPicture
	 */
	private MetadataBlockDataPicture createMetadataBlockDataPicture(final Artwork artwork) throws FieldDataInvalidException {
		if (artwork.isLinked())
	...
	public TagField createField(final Artwork artwork) throws FieldDataInvalidException {
		try {
			final char[] testdata = Base64Coder.encode(createMetadataBlockDataPicture(artwork).getRawContent());
			final String base64image = new String(testdata);
			final TagField imageTagField = createField(VorbisCommentFieldKey.METADATA_BLOCK_PICTURE, base64image);
	 ...
	 * 
	 * <p/>
	 * Actually create two fields , the data field and the mimetype. Its is not recommended that you use this method
	 * anymore.
	 * 
	 * @param data
	 *            raw image data
	 ...
	 */
	@Deprecated
	public void setArtworkField(final byte[] data, final String mimeType) {
		final char[] testdata = Base64Coder.encode(data);
		final String base64image = new String(testdata);
		final VorbisCommentTagField dataField = new VorbisCommentTagField(VorbisCommentFieldKey.COVERART.getFieldName(), base64image);
		final VorbisCommentTagField mimeField = new VorbisCommentTagField(VorbisCommentFieldKey.COVERARTMIME.getFieldName(), mimeType);
