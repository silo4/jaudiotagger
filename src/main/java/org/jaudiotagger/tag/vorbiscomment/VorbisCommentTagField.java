...
/**
 * This class represents the name and content of a tag entry in ogg-files. <br>
 * 
...
public class VorbisCommentTagField implements TagTextField {
	...
	/**
	 * If <code>true</code>, the id of the current encapsulated tag field is specified as a common field. <br>
	 * Example is "ARTIST" which should be interpreted by any application as the artist of the media content. <br>
	 ...
	 */
	public VorbisCommentTagField(final byte[] raw) throws UnsupportedEncodingException {
		final String field = new String(raw, "UTF-8");
	 ...
	 */
	public VorbisCommentTagField(final String fieldId, final String fieldContent) {
	...
	/**
	 * This method examines the ID of the current field and modifies {@link #common}in order to reflect if the tag id is
	 * a commonly used one. <br>
	 */
	private void checkCommon() {
		this.common = id.equals(TITLE.getFieldName()) || id.equals(ALBUM.getFieldName()) || id.equals(ARTIST.getFieldName()) || id.equals(GENRE.getFieldName()) || id.equals(TRACKNUMBER.getFieldName()) || id.equals(DATE.getFieldName())
	 ...
	 *            where to copy to.
	 * @param dstOffset
	 *            at which position of <code>dst</code> the data should be copied.
	 */
	protected void copy(final byte[] src, final byte[] dst, final int dstOffset) {
		// for (int i = 0; i < src.length; i++)
		// dst[i + dstOffset] = src[i];
		/*
		 * Heared that this method is optimized and does its job very near of the system.
		 */
	...
	/**
	 * This method will try to return the byte representation of the given string after it has been converted to the
	 * given encoding. <br>
	 * 
	 * @param s
	 ...
	 * @return If <code>encoding</code> is supported the byte data of the given string is returned in that encoding.
	 * @throws UnsupportedEncodingException
	 *             If the requested encoding is not available.
	 */
	protected byte[] getBytes(final String s, final String encoding) throws UnsupportedEncodingException {
	...
	public byte[] getRawContent() throws UnsupportedEncodingException {
		final byte[] size = new byte[VorbisCommentReader.FIELD_COMMENT_LENGTH_LENGTH];
		final byte[] idBytes = Utils.getDefaultBytes(this.id, "ISO-8859-1");
		final byte[] contentBytes = getBytes(this.content, "UTF-8");
		final byte[] b = new byte[4 + idBytes.length + 1 + contentBytes.length];

		final int length = idBytes.length + 1 + contentBytes.length;
