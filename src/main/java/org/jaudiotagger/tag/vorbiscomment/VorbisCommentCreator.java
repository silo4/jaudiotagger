...
public class VorbisCommentCreator extends AbstractTagCreator {
	...
	public ByteBuffer convert(final Tag tag, final int padding) throws UnsupportedEncodingException {
		try {
			...
			// Vendor
			final String vendorString = ((VorbisCommentTag) tag).getVendor();
			final int vendorLength = Utils.getUTF8Bytes(vendorString).length;
			baos.write(Utils.getSizeLEInt32(vendorLength));
			baos.write(Utils.getUTF8Bytes(vendorString));

			// User Comment List
			final int listLength = tag.getFieldCount() - 1; // Remove Vendor from count
