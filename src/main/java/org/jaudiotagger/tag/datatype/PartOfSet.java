 ...
 * 
 * If TagOptionSingleton.getInstance().isPadNumbers() is disabled then count and track are written to file as they are
 * provided, i.e if provided pre-padded they will be stored pre-padded, if not they will not. Values read from file will
 * be returned as they are currently stored in file.
 * 
 * 
 */
...
public class PartOfSet extends AbstractString {
	 ...
	 * @param frameBody
	 */
	public PartOfSet(final String identifier, final AbstractTagFrameBody frameBody) {
	 ...
	 * @param object
	 */
	public PartOfSet(final PartOfSet object) {
		super(object);
	}
	...
	public boolean equals(final Object obj) {
		...
		final PartOfSet that = (PartOfSet) obj;

		return EqualsUtil.areEqual(value, that.value);
	}

	/**
	 * Read a 'n' bytes from buffer into a String where n is the frameSize - offset so therefore cannot use this if
	 * there are other objects after it because it has no delimiter.
	 * <p/>
	 * Must take into account the text encoding defined in the Encoding Object ID3 Text Frames often allow multiple
	 * strings separated by the null char appropriate for the encoding.
	 * 
	 * @param arr
	...
	public void readByteArray(final byte[] arr, final int offset) throws InvalidDataTypeException {
		...
		final ByteBuffer inBuffer = ByteBuffer.wrap(arr, offset, arr.length - offset).slice();
		...
		// Store value
		final String stringValue = outBuffer.toString();
	...
	public static class PartOfSetValue {
		...
		private String rawText; // raw text representation used to actually save the data IF
								// !TagOptionSingleton.getInstance().isPadNumbers()
		 ...
		 * @param value
		 */
		public PartOfSetValue(final String value) {
			this.rawText = value;
			initFromValue(value);
		}
		 ...
		 * @param total
		 */
		public PartOfSetValue(final Integer count, final Integer total) {
			this.count = count;
		...
		/**
		 * Given a raw value that could contain both a count and total and extra stuff (but needdnt contain anything
		 * tries to parse it)
		 * 
		 * @param value
		 */
		private void initFromValue(final String value) {
			try {
		 ...
		 */
		private void padNumber(final StringBuffer sb, final Integer count, final PadNumberOption padNumberLength) {
			if (count != null)
		...
		public boolean equals(final Object obj) {
			...
			final PartOfSetValue that = (PartOfSetValue) obj;

			return EqualsUtil.areEqual(getCount(), that.getCount()) && EqualsUtil.areEqual(getTotal(), that.getTotal());
		}
	}
