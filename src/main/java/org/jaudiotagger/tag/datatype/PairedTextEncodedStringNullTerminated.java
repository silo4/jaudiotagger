...
/**
 * Represents a data type that allow multiple Strings but they should be paired as key values, i.e should be 2,4,6.. But
 * keys are not unique so we don't store as a map, so could have same key pointing to two different values such as two
 * ENGINEER keys
 * <p/>
 */
public class PairedTextEncodedStringNullTerminated extends AbstractDataType {
	public PairedTextEncodedStringNullTerminated(final String identifier, final AbstractTagFrameBody frameBody) {
		super(identifier, frameBody);
		value = new PairedTextEncodedStringNullTerminated.ValuePairs();
	}

	public PairedTextEncodedStringNullTerminated(final TextEncodedStringSizeTerminated object) {
		super(object);
		value = new PairedTextEncodedStringNullTerminated.ValuePairs();
	}

	public PairedTextEncodedStringNullTerminated(final PairedTextEncodedStringNullTerminated object) {
		super(object);
	}
	...
	public boolean equals(final Object obj) {
		...
		final PairedTextEncodedStringNullTerminated that = (PairedTextEncodedStringNullTerminated) obj;
	...
	public boolean canBeEncoded() {
		for (final Pair entry : ((ValuePairs) value).mapping) {
			final TextEncodedStringNullTerminated next = new TextEncodedStringNullTerminated(identifier, frameBody, entry.getValue());
	 ...
	 */
	@Override
	public void readByteArray(final byte[] arr, int offset) throws InvalidDataTypeException {
		...
		while (true) {
			try {
				...
				try {
					// Read Value
					final TextEncodedStringNullTerminated result = new TextEncodedStringNullTerminated(identifier, frameBody);
				...
				} catch (final InvalidDataTypeException idte) {
					// Value may not be null terminated if it is the last value
					// Read Value
					if (offset >= arr.length)
						break;
					final TextEncodedStringSizeTerminated result = new TextEncodedStringSizeTerminated(identifier, frameBody);
					result.readByteArray(arr, offset);
					size += result.getSize();
					offset += result.getSize();
					if (result.getSize() == 0)
	...
	public byte[] writeByteArray() {
		...
		try {
			for (final Pair pair : ((ValuePairs) value).mapping) {
				{
					final TextEncodedStringNullTerminated next = new TextEncodedStringNullTerminated(identifier, frameBody, pair.getKey());
					buffer.write(next.writeByteArray());
					localSize += next.getSize();
				}
				{
					final TextEncodedStringNullTerminated next = new TextEncodedStringNullTerminated(identifier, frameBody, pair.getValue());
					buffer.write(next.writeByteArray());
					localSize += next.getSize();
				}
			}
	...
	public static class ValuePairs {
		...
		public boolean equals(final Object obj) {
			...
			final ValuePairs that = (ValuePairs) obj;
