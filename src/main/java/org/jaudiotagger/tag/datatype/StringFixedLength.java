...
/**
 * Represents a fixed length String, whereby the length of the String is known. The String will be encoded based upon
 * the text encoding of the frame that it belongs to.
 */
public class StringFixedLength extends AbstractString {
	 ...
	 */
	public StringFixedLength(final String identifier, final AbstractTagFrameBody frameBody, final int size) {
	...
	public StringFixedLength(final StringFixedLength copyObject) {
		super(copyObject);
	...
	public boolean equals(final Object obj) {
		if (!(obj instanceof StringFixedLength))
			return false;
		final StringFixedLength object = (StringFixedLength) obj;
	...
	public void readByteArray(final byte[] arr, final int offset) throws InvalidDataTypeException {
		...
		try {
			...
			final String str = decoder.decode(ByteBuffer.wrap(arr, offset, size)).toString();
			if (str == null)
	 ...
	 * <p/>
	 * The string will be adjusted to ensure the correct number of bytes are written, If the current value is null or to
	 * short the written value will have the 'space' character appended to ensure this. We write this instead of the
	 * null character because the null character is likely to confuse the parser into misreading the next field.
	 * 
	...
	public byte[] writeByteArray() {
		...
		if (dataBuffer != null) {
			...
			// There is more data available than allowed for this field strip
				...
				logger.warning("There was a problem writing the following StringFixedlength Field:" + value + " when converted to bytes has length of:" + dataBuffer.limit() + " but field was defined with length of:" + size
