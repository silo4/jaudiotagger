...
public class TextEncodedStringNullTerminated extends AbstractString {
	 ...
	 * @param frameBody
	 */
	public TextEncodedStringNullTerminated(final String identifier, final AbstractTagFrameBody frameBody) {
	 ...
	 * @param value
	 */
	public TextEncodedStringNullTerminated(final String identifier, final AbstractTagFrameBody frameBody, final String value) {
		super(identifier, frameBody, value);
	}

	public TextEncodedStringNullTerminated(final TextEncodedStringNullTerminated object) {
		super(object);
	}
	...
	public void readByteArray(final byte[] arr, final int offset) throws InvalidDataTypeException {
		...
		final ByteBuffer buffer = ByteBuffer.wrap(arr, offset, arr.length - offset);
		int endPosition = 0;
		...
		while (buffer.hasRemaining()) {
			...
			if (nextByte == 0x00) {
				if (nullIsOneByte) {
					...
					break;
				} else // Looking for two-byte null
			...
			} else // If UTF16, we should only be looking on 2 byte boundaries
			if (!nullIsOneByte)
				if (buffer.hasRemaining())
					buffer.get();
		}
		...
		if (bufferSize == 0)
			...
			final ByteBuffer inBuffer = ByteBuffer.wrap(arr, offset, bufferSize).slice();
			final CharBuffer outBuffer = CharBuffer.allocate(bufferSize);
			decoder.reset();
