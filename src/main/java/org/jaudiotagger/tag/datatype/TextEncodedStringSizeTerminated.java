...
public class TextEncodedStringSizeTerminated extends AbstractString {
	 ...
	 * @param frameBody
	 */
	public TextEncodedStringSizeTerminated(final String identifier, final AbstractTagFrameBody frameBody) {
	 ...
	 * @param object
	 */
	public TextEncodedStringSizeTerminated(final TextEncodedStringSizeTerminated object) {
		super(object);
	}
	...
	/**
	 * Read a 'n' bytes from buffer into a String where n is the framesize - offset so thefore cannot use this if there
	 * are other objects after it because it has no delimiter.
	 * <p/>
	 * Must take into account the text encoding defined in the Encoding Object ID3 Text Frames often allow multiple
	 * strings seperated by the null char appropriate for the encoding.
	 * 
	 * @param arr
	 *            this is the buffer for the frame
	 * @param offset
	...
	public void readByteArray(final byte[] arr, final int offset) throws InvalidDataTypeException {
		...
		if (TagOptionSingleton.getInstance().isAndroid()) {
			...
			// buffer
			final byte[] truncArr = new byte[arr.length - offset];
		...
		final CharBuffer outBuffer = CharBuffer.allocate(arr.length - offset);
		final CoderResult coderResult = decoder.decode(inBuffer, outBuffer, true);
	 ...
	 */
	private ByteBuffer writeString(final CharsetEncoder encoder, final String next, final int i, final int noOfValues) throws CharacterCodingException {
	 ...
	 * When this is called multiple times, all but the last value has a trailing null
	 * 
	 * Remember we are using this charset because the charset that writes BOM does it the wrong way for us so we use
	 * this none and then manually add the BOM ourselves.
	 * 
	 * @param next
	 * @param i
	 * @param noOfValues
	 ...
	 * @throws CharacterCodingException
	 */
	private ByteBuffer writeStringUTF16LEBOM(final String next, final int i, final int noOfValues) throws CharacterCodingException {
		final CharsetEncoder encoder = Charset.forName(TextEncoding.CHARSET_UTF_16_LE_ENCODING_FORMAT).newEncoder();
		ByteBuffer bb = null;
	 ...
	 * @throws CharacterCodingException
	 */
	private ByteBuffer writeStringUTF16BEBOM(final String next, final int i, final int noOfValues) throws CharacterCodingException {
		final CharsetEncoder encoder = Charset.forName(TextEncoding.CHARSET_UTF_16_BE_ENCODING_FORMAT).newEncoder();
		ByteBuffer bb = null;
		// Add BOM
		if ((i + 1) == noOfValues)
			bb = encoder.encode(CharBuffer.wrap('\ufeff' + next));
	...
	/**
	 * Removing trailing null from end of String, this should be there but some applications continue to write this
	 * unnecessary null char.
	 */
	...
	/**
	 * Because nulls are stripped we need to check if not removing trailing nulls whether the original value ended with
	 * a null and if so add it back in.
	 * 
	 * @param values
	 * @param stringValue
	 */
	private void checkTrailingNull(final List<String> values, final String stringValue) {
		if (!TagOptionSingleton.getInstance().isRemoveTrailingTerminatorOnWrite())
			if (stringValue.length() > 0 && stringValue.charAt(stringValue.length() - 1) == '\0') {
				final String lastVal = values.get(values.size() - 1);
				final String newLastVal = lastVal + '\0';
	...
	public byte[] writeByteArray() {
		...
		try {
			...
			final ByteBuffer outputBuffer = ByteBuffer.allocate((stringValue.length() + 3) * 3);

			// Ensure each string (if multiple values) is written with BOM by writing separately
			final List<String> values = splitByNullSeperator(stringValue);
			checkTrailingNull(values, stringValue);

			// For each value
			for (int i = 0; i < values.size(); i++) {
				final String next = values.get(i);
		...
		catch (final CharacterCodingException ce) {
	...
	protected String getTextEncodingCharSet() {
		...
		final String charSetName = TextEncoding.getInstanceOf().getValueForId(textEncoding);
	 ...
	 */
	public static List<String> splitByNullSeperator(final String value) {
		final String[] valuesarray = value.split("\\u0000");
	...
	public String getValueAtIndex(final int index) {
		...
		final List values = splitByNullSeperator((String) value);
	...
	public String getValueWithoutTrailingNull() {
		final List<String> values = splitByNullSeperator((String) value);
