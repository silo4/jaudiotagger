 ...
 * <p/>
 * <p>
 * Format is as follows: :length (4 bytes) :name 'Data' (4 bytes) :atom version (1 byte) :atom type flags (3 bytes)
 * :locale field (4 bytes) //Currently always zero :data
 */
public class Mp4DataBox extends AbstractMp4Box {
	 ...
	 *            parentHeader info
	 * @param dataBuffer
	 *            data of box (doesnt include parentHeader data)
	 */
	public Mp4DataBox(final Mp4BoxHeader header, final ByteBuffer dataBuffer) {
		this.header = header;
		// Double check
		...
		if (type == Mp4FieldType.TEXT.getFileClassId())
			...
			for (int i = 0; i < ((header.getDataLength() - PRE_DATA_LENGTH) / NUMBER_LENGTH); i++) {
				final short number = Utils.getShortBE(this.dataBuffer, PRE_DATA_LENGTH + (i * NUMBER_LENGTH), PRE_DATA_LENGTH + (i * NUMBER_LENGTH) + (NUMBER_LENGTH - 1));
				numbers.add(number);
			}

			// Make String representation (separate values with slash)
			final StringBuffer sb = new StringBuffer();
			final ListIterator<Short> iterator = numbers.listIterator();
			...
			// Songbird uses this type for trkn atom (normally implicit type) is used so just added this code so can be
			// used
