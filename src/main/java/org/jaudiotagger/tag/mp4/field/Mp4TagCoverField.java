 ...
 * <p/>
 * <p>
 * Note:Within this library we have a seperate TagCoverField for every image stored, however this does not map very
 * directly to how they are physically stored within a file, because all are stored under a single covr atom, so a more
 * complex conversion has to be done then for other fields when writing multiple images back to file.
 */
public class Mp4TagCoverField extends Mp4TagBinaryField {
	 ...
	 */
	public Mp4TagCoverField(final ByteBuffer raw, final Mp4FieldType imageType) throws UnsupportedEncodingException {
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public Mp4TagCoverField(final byte[] data) {
	...
	protected void build(final ByteBuffer raw) {
		final Mp4BoxHeader header = new Mp4BoxHeader(raw);
	 ...
	 */
	public static String getMimeTypeForImageType(final Mp4FieldType imageType) {
