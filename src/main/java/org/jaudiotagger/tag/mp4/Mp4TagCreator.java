 ...
 * <p/>
 * <p>
 * This level is was selected because the ilst atom can be recreated without reference to existing mp4 fields but fields
...
public class Mp4TagCreator extends AbstractTagCreator {
	 ...
	 */
	@Override
	public ByteBuffer convert(final Tag tag, final int padding) throws UnsupportedEncodingException {
		try {
			...
			while (it.hasNext()) {
				...
				if (frame instanceof Mp4TagCoverField) {
					if (processedArtwork) {
						...
						final ByteArrayOutputStream covrDataBaos = new ByteArrayOutputStream();

						try {
							for (final TagField artwork : tag.getFields(FieldKey.COVER_ART))
						...
						final byte[] data = covrDataBaos.toByteArray();
			...
			// Wrap into ilst box
			final ByteArrayOutputStream ilst = new ByteArrayOutputStream();
			...
			// Put into ByteBuffer
			final ByteBuffer buf = ByteBuffer.wrap(ilst.toByteArray());
			buf.rewind();
			return buf;
