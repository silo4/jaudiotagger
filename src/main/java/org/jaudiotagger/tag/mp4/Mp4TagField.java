 ...
 * <p/>
 * Note there isnt a one to one correspondance between a tag field and a box because some fields are represented by
 * multiple boxes, for example many of the MusicBrainz fields use the '----' box, which in turn uses one of mean, name
 * and data box. So an instance of a tag field maps to one item of data such as 'Title', but it may have to read
...
public abstract class Mp4TagField implements TagField {
	...
	protected Mp4TagField(final String id) {
		this.id = id;
	}
	 ...
	 */
	protected Mp4TagField(final ByteBuffer data) throws UnsupportedEncodingException {
		build(data);
	}
	 ...
	 * @throws UnsupportedEncodingException
	 */
	protected Mp4TagField(final Mp4BoxHeader parentHeader, final ByteBuffer data) throws UnsupportedEncodingException {
		this.parentHeader = parentHeader;
		build(data);
	}

	protected Mp4TagField(final String id, final ByteBuffer data) throws UnsupportedEncodingException {
		this(id);
		build(data);
	}
	...
	public boolean isCommon() {
		return id.equals(Mp4FieldKey.ARTIST.getFieldName()) || id.equals(Mp4FieldKey.ALBUM.getFieldName()) || id.equals(Mp4FieldKey.TITLE.getFieldName()) || id.equals(Mp4FieldKey.TRACK.getFieldName())
				|| id.equals(Mp4FieldKey.DAY.getFieldName()) || id.equals(Mp4FieldKey.COMMENT.getFieldName()) || id.equals(Mp4FieldKey.GENRE.getFieldName());
	}
	...
	/**
	 * Processes the data and sets the position of the data buffer to just after the end of this fields data ready for
	 * processing next field.
	 * 
	 * @param data
	 * @throws UnsupportedEncodingException
	 */
	...
	public byte[] getRawContent() throws UnsupportedEncodingException {
		...
		try {
			// Create Data Box
			final byte[] databox = getRawContentDataOnly();
