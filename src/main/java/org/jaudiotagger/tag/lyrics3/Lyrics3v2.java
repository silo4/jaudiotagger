...
public class Lyrics3v2 extends AbstractLyrics3 {
	...
	public Lyrics3v2(final Lyrics3v2 copyObject) {
		...
		final Iterator<String> iterator = copyObject.fieldMap.keySet().iterator();
	 ...
	 */
	public Lyrics3v2(final AbstractTag mp3tag) {
		if (mp3tag != null)
			...
			if (mp3tag instanceof Lyrics3v2)
				...
				while (iterator.hasNext())
					try {
						...
						// invalid frame to createField lyrics3 field. ignore
						// and keep going
					}
			}
	}
	 ...
	 * @param byteBuffer
	 */
	public Lyrics3v2(final ByteBuffer byteBuffer) throws TagNotFoundException, IOException {
		try {
	...
	public int getSize() {
		int size = 0;
		final Iterator<Lyrics3v2Field> iterator = fieldMap.values().iterator();
	...
	public boolean equals(final Object obj) {
		...
		final Lyrics3v2 object = (Lyrics3v2) obj;
	...
	public void updateField(final String identifier) {
		...
		if (identifier.equals("IND")) {
			final boolean lyricsPresent = fieldMap.containsKey("LYR");
			boolean timeStampPresent = false;

			if (lyricsPresent) {
				lyrField = fieldMap.get("LYR");

				final FieldFrameBodyLYR lyrBody = (FieldFrameBodyLYR) lyrField.getBody();
	 ...
	 * @return
	 */
	private int seekSize(final ByteBuffer byteBuffer) {
		/*
		 * byte[] buffer = new byte[11]; String lyricEnd = ""; long filePointer = 0;
		 * 
		 * // check right before the ID3 1.0 tag for the lyrics tag file.seek(file.length() - 128 - 9);
		 * file.read(buffer, 0, 9); lyricEnd = new String(buffer, 0, 9);
		 * 
		 * if (lyricEnd.equals("LYRICS200")) { filePointer = file.getFilePointer(); } else { // check the end of the
		 * file for a lyrics tag incase an ID3 // tag wasn't placed after it. file.seek(file.length() - 9);
		 * file.read(buffer, 0, 9); lyricEnd = new String(buffer, 0, 9);
		 * 
		 * if (lyricEnd.equals("LYRICS200")) { filePointer = file.getFilePointer(); } else { return -1; } }
		 * 
		 * // read the 6 bytes for the length of the tag filePointer -= (9 + 6); file.seek(filePointer);
		 * file.read(buffer, 0, 6);
		 * 
