 ...
 * <p/>
 * byte0 Picture Type byte1-4 Length of the image data mime type encoded as UTF-16LE null byte null byte description
 * encoded as UTF-16LE (optional) null byte null byte image data
 */
public class AsfTagCoverField extends AbstractAsfTagImageField {
	/**
	 * Logger Object
	 */
	public final static Logger LOGGER = Logger.getLogger("org.jaudiotagger.audio.asf.tag");
	 ...
	 * @param mimeType
	 */
	public AsfTagCoverField(final byte[] imageData, final int pictureType, final String description, final String mimeType) {
		super(new MetadataDescriptor(AsfFieldKey.COVER_ART.getFieldName(), MetadataDescriptor.TYPE_BINARY));
	...
	private byte[] createRawContent(final byte[] data, final int pictureType, final String description, String mimeType) { // NOPMD
																															// by
																															// Christian
																															// Laireiter
																															// on
	...
	public byte[] getRawImageData() {
		final ByteArrayOutputStream baos = new ByteArrayOutputStream();
		baos.write(getRawContent(), this.endOfName, this.toWrap.getRawDataSize() - this.endOfName);
		return baos.toByteArray();
	}
	...
	private void processRawContent() throws UnsupportedEncodingException {
		...
		// ImageDataSize
		this.imageDataSize = org.jaudiotagger.audio.generic.Utils.getIntLE(this.getRawContent(), 1, 2);
		...
		while (count < this.getRawContent().length - 1) {
			if (getRawContent()[count] == 0 && getRawContent()[count + 1] == 0)
				if (this.mimeType == null) {
					this.mimeType = new String(getRawContent(), 5, (count) - 5, "UTF-16LE");
					endOfMimeType = count + 2;
				} else if (this.description == null) {
					this.description = new String(getRawContent(), endOfMimeType, count - endOfMimeType, "UTF-16LE");
