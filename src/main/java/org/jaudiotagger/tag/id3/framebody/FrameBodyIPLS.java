 ...
 * <p/>
 * Since there might be a lot of people contributing to an audio file in various ways, such as musicians and
 ...
 * <p/>
 * <Header for 'Involved people list', ID: "IPLS"> Text encoding $xx People list strings <text strings according to
 * encoding>
 * <p/>
 * <p>
...
public class FrameBodyIPLS extends AbstractID3v2FrameBody implements ID3v23FrameBody {
	...
	public FrameBodyIPLS(final ByteBuffer byteBuffer, final int frameSize) throws InvalidTagException {
	 ...
	 * @param text
	 */
	public FrameBodyIPLS(final byte textEncoding, final String text) {
	 ...
	 * @param body
	 */
	public FrameBodyIPLS(final FrameBodyTIPL body) {
	...
	public void setText(final String text) {
		final PairedTextEncodedStringNullTerminated.ValuePairs value = new PairedTextEncodedStringNullTerminated.ValuePairs();
		final StringTokenizer stz = new StringTokenizer(text, "\0");
		while (stz.hasMoreTokens()) {
			final String key = stz.nextToken();
	...
	public void addPair(final String text) {
		final PairedTextEncodedStringNullTerminated.ValuePairs value = ((PairedTextEncodedStringNullTerminated) getObject(DataTypes.OBJ_TEXT)).getValue();
		final StringTokenizer stz = new StringTokenizer(text, "\0");
		if (stz.hasMoreTokens())
	...
	/**
	 * Consists of a text encoding , and then a series of null terminated Strings, there should be an even number of
	 * Strings as they are paired as involvement/involvee
	 */
	@Override
	 ...
	 */
	public String getKeyAtIndex(final int index) {
		final PairedTextEncodedStringNullTerminated text = (PairedTextEncodedStringNullTerminated) getObject(DataTypes.OBJ_TEXT);
	...
	public String getText() {
		...
		int count = 1;
		for (final Pair entry : text.getValue().getMapping()) {
