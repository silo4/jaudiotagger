 ...
 * The ASPI frame makes seeking easier by providing a list a seek points within the audio file. The seek points are a
 * fractional offset within the audio data, providing a starting point from which to find an appropriate point to start
 * decoding. The presence of an ASPI frame requires the existence of a TLEN frame, indicating the duration of the file
 * in milliseconds. There may only be one 'audio seek point index' frame in a tag.
 * <p/>
 * <Header for 'Seek Point Index', ID: "ASPI"> Indexed data start (S) $xx xx xx xx Indexed data length (L) $xx xx xx xx
 * Number of index points (N) $xx xx Bits per index point (b) $xx
 * <p/>
 * Then for every index point the following data is included;
 * <p/>
 * Fraction at index (Fi) $xx (xx)
 * <p/>
 * 'Indexed data start' is a byte offset from the beginning of the file. 'Indexed data length' is the byte length of the
 * audio data being indexed. 'Number of index points' is the number of index points, as the name implies. The
 * recommended number is 100. 'Bits per index point' is 8 or 16, depending on the chosen precision. 8 bits works well
 * for short files (less than 5 minutes of audio), while 16 bits is advantageous for long files. 'Fraction at index' is
 * the numerator of the fraction representing a relative position in the data. The denominator is 2 to the power of b.
 * <p/>
 * Here are the algorithms to be used in the calculation. The known data must be the offset of the start of the indexed
 * data (S), the offset of the end of the indexed data (E), the number of index points (N), the offset at index i (Oi).
 * We calculate the fraction at index i (Fi).
 * <p/>
...
public class FrameBodyASPI extends AbstractID3v2FrameBody implements ID3v24FrameBody {
	 ...
	 */
	public FrameBodyASPI(final ByteBuffer byteBuffer, final int frameSize) throws InvalidTagException {
