 ...
 * There may only be one text information frame of its kind in an tag. In ID3v24 All text information frames supports
 * multiple strings, stored as a null separated list, where null is represented by the termination code for the
 * character encoding. All text frame identifiers begin with "T". Only text frame identifiers begin with "T", with the
 * exception of the "TXXX" frame. All the text information frames have the following format: <Header for 'Text
 * information frame', ID: "T000" - "TZZZ", excluding "TXXX" described in 4.2.6.> Text encoding $xx Information <text
 * string(s) according to encoding>
 * <p/>
...
public abstract class AbstractFrameBodyTextInfo extends AbstractID3v2FrameBody {
	...
	/**
	 * Creates a new FrameBodyTextInformation datatype. The super.super Constructor sets up the Object list for the
	 * frame.
	 */
	 ...
	 */
	protected AbstractFrameBodyTextInfo(final AbstractFrameBodyTextInfo body) {
		super(body);
	}

	/**
	 * Creates a new FrameBodyTextInformation data type. This is used when user wants to create a new frame based on
	 * data in a user interface.
	 * 
	 * @param textEncoding
	 *            Specifies what encoding should be used to write text to file.
	 * @param text
	 *            Specifies the text String.
	 */
	protected AbstractFrameBodyTextInfo(final byte textEncoding, final String text) {
		super();
	 ...
	 */
	protected AbstractFrameBodyTextInfo(final ByteBuffer byteBuffer, final int frameSize) throws InvalidTagException {
	 ...
	 * <p/>
	 * <p>
	 * If this String contains null terminator characters these are parsed as value separators, allowing you to hold
	 * multiple strings within one text frame. This functionality is only officially support in ID3v24.
	 * 
	 * @param text
	 *            to set
	 */
	...
	public String getTextWithoutTrailingNulls() {
		final TextEncodedStringSizeTerminated text = (TextEncodedStringSizeTerminated) getObject(DataTypes.OBJ_TEXT);
	 ...
	 * 
	 * When a multiple values are stored within a single text frame this method allows access to any of the individual
	 * values.
	 * 
	 * @param index
	 ...
	 *            at index
	 */
	public void addTextValue(final String value) {
		final TextEncodedStringSizeTerminated text = (TextEncodedStringSizeTerminated) getObject(DataTypes.OBJ_TEXT);
		text.addValue(value);
	}
	...
	/**
	 * Because Text frames have a text encoding we need to check the text String does not contain characters that cannot
	 * be encoded in current encoding before we write data. If there are change the text encoding.
	 */
	@Override
