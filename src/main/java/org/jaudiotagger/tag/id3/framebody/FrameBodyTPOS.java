 ...
 * 
 * <p>
 * The 'Part of a set' frame is a numeric string that describes which part of a set the audio came from. This frame is
 * used if the source described in the "TALB" frame is divided into several mediums, e.g. a double CD. The value may be
 * extended with a "/" character and a numeric string containing the total number of parts in the set. e.g. "1/2".
 * <p/>
 * <p>
...
public class FrameBodyTPOS extends AbstractID3v2FrameBody implements ID3v23FrameBody, ID3v24FrameBody {
	...
	public FrameBodyTPOS(final byte textEncoding, final Integer discNo, final Integer discTotal) {
		super();
		setObjectValue(DataTypes.OBJ_TEXT_ENCODING, textEncoding);
	 ...
	 */
	public FrameBodyTPOS(final ByteBuffer byteBuffer, final int frameSize) throws InvalidTagException {
	...
	public void setDiscNo(final Integer discNo) {
		((PartOfSet.PartOfSetValue) getObjectValue(DataTypes.OBJ_TEXT)).setCount(discNo);
	}

	public void setDiscNo(final String discNo) {
		((PartOfSet.PartOfSetValue) getObjectValue(DataTypes.OBJ_TEXT)).setCount(discNo);
	}
	...
	public void setDiscTotal(final Integer discTotal) {
		((PartOfSet.PartOfSetValue) getObjectValue(DataTypes.OBJ_TEXT)).setTotal(discTotal);
	}

	public void setDiscTotal(final String discTotal) {
		((PartOfSet.PartOfSetValue) getObjectValue(DataTypes.OBJ_TEXT)).setTotal(discTotal);
	}
