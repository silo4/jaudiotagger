 ...
 * scheme is beyond this document, all "AENC" frames begin with a terminated string with a URL containing an email
 * address, or a link to a location where an email address can be found, that belongs to the organisation responsible
 * for this specific encrypted audio file. Questions regarding the encrypted audio should be sent to the email address
 * specified. If a $00 is found directly after the 'Frame size' and the audiofile indeed is encrypted, the whole file
 * may be considered useless.
 * </p>
 * <p>
 * After the 'Owner identifier', a pointer to an unencrypted part of the audio can be specified. The 'Preview start' and
 * 'Preview length' is described in frames. If no part is unencrypted, these fields should be left zeroed. After the
 * 'preview length' field follows optionally a datablock required for decryption of the audio. There may be more than
 * one "AENC" frames in a tag, but only one with the same 'Owner identifier'.
 * </p>
 * <p>
 * <table border=0 width="70%">
 * <tr>
 * <td colspan=2>&lt;Header for 'Audio encryption', ID: "AENC"&gt;</td>
 * </tr>
 * <tr>
...
public class FrameBodyAENC extends AbstractID3v2FrameBody implements ID3v24FrameBody, ID3v23FrameBody {
	 ...
	 */
	public FrameBodyAENC(final ByteBuffer byteBuffer, final int frameSize) throws InvalidTagException {
