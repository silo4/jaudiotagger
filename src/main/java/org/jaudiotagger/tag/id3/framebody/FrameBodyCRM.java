 ...
 * <p/>
 * This frame contains one or more encrypted frames. This enables protection of copyrighted information such as pictures
 * and text, that people might want to pay extra for. Since standardisation of such an encryption scheme is beyond this
 * document, all "CRM" frames begin with a terminated string with a URL [URL] containing an email address, or a link to
 * a location where an email adress can be found, that belongs to the organisation responsible for this specific
 * encrypted meta frame.
 * <p/>
 * Questions regarding the encrypted frame should be sent to the indicated email address. If a $00 is found directly
 * after the 'Frame size', the whole frame should be ignored, and preferably be removed. The 'Owner identifier' is then
 * followed by a short content description and explanation as to why it's encrypted. After the 'content/explanation'
 ...
 * error. There may be more than one "CRM" frames in a tag, but only one with the same 'owner identifier'.
 * <p/>
 * Encrypted meta frame "CRM" Frame size $xx xx xx Owner identifier <textstring> $00 (00) Content/explanation
 * <textstring> $00 (00) Encrypted datablock <binary data>
 */
public class FrameBodyCRM extends AbstractID3v2FrameBody implements ID3v22FrameBody {
	 ...
	 */
	public FrameBodyCRM(final ByteBuffer byteBuffer, final int frameSize) throws InvalidTagException {
