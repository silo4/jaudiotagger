...
public abstract class AbstractID3v2FrameBody extends AbstractTagFrameBody {
	...
	/**
	 * Frame Body Size, originally this is size as indicated in frame header when we come to writing data we recalculate
	 * it.
	 */
	private int size;
	 ...
	 * 
	 * @param byteBuffer
	 *            from where to read the frame body from
	 * @param frameSize
	 * @throws org.jaudiotagger.tag.InvalidTagException
	 */
	protected AbstractID3v2FrameBody(final ByteBuffer byteBuffer, final int frameSize) throws InvalidTagException {
		super();
		setSize(frameSize);
	...
	/**
	 * Return size of frame body,if frameBody already exist will take this value from the frame header but it is always
	 * recalculated before writing any changes back to disk.
	 * 
	...
	public void read(final ByteBuffer byteBuffer) throws InvalidTagException {
		...
		// correct dataType.
		{
			...
			} catch (final InvalidDataTypeException e) {
...

	{
		...
		for (final AbstractDataType object : objectList) {
			final byte[] objectData = object.writeByteArray();
			if (objectData != null)
				try {
	...
	public void createStructure() {
		...
		for (final AbstractDataType nextObject : objectList)
