...
public class ID3v22Tag extends AbstractID3v2Tag {
	...
	protected void copyPrimitives(final AbstractID3v2Tag copyObj) {
		...
		if (copyObj instanceof ID3v22Tag) {
			final ID3v22Tag copyObject = (ID3v22Tag) copyObj;
	 ...
	 * @param copyObject
	 */
	public ID3v22Tag(final ID3v22Tag copyObject) {
	 ...
	 * @param mp3tag
	 */
	public ID3v22Tag(final AbstractTag mp3tag) {
	 ...
	 * @throws TagException
	 */
	public ID3v22Tag(final ByteBuffer buffer, final String loggingFilename) throws TagException {
	 ...
	 */
	@Deprecated
	public ID3v22Tag(final ByteBuffer buffer) throws TagException {
		this(buffer, "");
	}
	...
	/**
	 * Return frame size based upon the sizes of the frames rather than the size including padding recorded in the tag
	 * header
	 * 
	 * @return size
	 */
	...
	public boolean equals(final Object obj) {
		if (!(obj instanceof ID3v22Tag))
			return false;
		final ID3v22Tag object = (ID3v22Tag) obj;
	...
	protected void addFrame(final AbstractID3v2Frame frame) {
		try {
			// Special case to handle TDRC frame from V24 that needs breaking up
			// into separate frame in V23
			if ((frame.getIdentifier().equals(ID3v24Frames.FRAME_ID_YEAR)) && (frame.getBody() instanceof FrameBodyTDRC))
				...
				final ID3v22Frame newFrame = new ID3v22Frame(frame);
	...
	public void read(final ByteBuffer byteBuffer) throws TagException {
		...
		// Slice Buffer, so position markers tally with size (i.e do not include
		// tagheader)
	...
	protected void readFrames(final ByteBuffer byteBuffer, final int size) {
		...
		/*
		 * todo not done yet. Read the first Frame, there seems to be quite a* common case of extra data being between
		 * the tag header and the first* frame so should we allow for this when reading first frame, but not subsequent
		 * frames
		 */
	 ...
	 * @param frame
	 */
	// TODO will overwrite any existing TYER or TIME frame, do we ever want
	// multiples of these
	 ...
	 */
	@Override
	public ID3v22Frame createFrame(final String id) {
	 ...
	 */
	public TagField createField(final ID3v22FieldKey id3Key, final String value) throws KeyNotFoundException, FieldDataInvalidException {
		if (id3Key == null)
	 ...
	 */
	public String getFirst(final ID3v22FieldKey id3v22FieldKey) throws KeyNotFoundException {
		if (id3v22FieldKey == null)
			throw new KeyNotFoundException();

		final FieldKey genericKey = ID3v22Frames.getInstanceOf().getGenericKeyFromId3(id3v22FieldKey);
		if (genericKey != null)
			return super.getFirst(genericKey);
		else {
			final FrameAndSubId frameAndSubId = new FrameAndSubId(id3v22FieldKey.getFrameId(), id3v22FieldKey.getSubId());
	 ...
	 * @throws org.jaudiotagger.tag.KeyNotFoundException
	 */
	public void deleteField(final ID3v22FieldKey id3v22FieldKey) throws KeyNotFoundException {
		if (id3v22FieldKey == null)
			throw new KeyNotFoundException();
	...
	protected FrameAndSubId getFrameAndSubIdFromGenericKey(final FieldKey genericKey) {
		final ID3v22FieldKey id3v22FieldKey = ID3v22Frames.getInstanceOf().getId3KeyFromGenericKey(genericKey);
		if (id3v22FieldKey == null)
			throw new KeyNotFoundException();
	...
	public Comparator<String> getPreferredFrameOrderComparator() {
		return ID3v22PreferredFrameOrderComparator.getInstance();
	}
	...
	public List<Artwork> getArtworkList() {
		...
		for (final TagField next : coverartList) {
			final FrameBodyPIC coverArt = (FrameBodyPIC) ((AbstractID3v2Frame) next).getBody();
