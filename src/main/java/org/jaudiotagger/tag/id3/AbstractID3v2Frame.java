...
public abstract class AbstractID3v2Frame extends AbstractTagFrame implements TagTextField {
	 ...
	 * @param frame
	 */
	public AbstractID3v2Frame(final AbstractID3v2Frame frame) {
		super(frame);
	}
	 ...
	 * @param body
	 */
	public AbstractID3v2Frame(final AbstractID3v2FrameBody body) {
		this.frameBody = body;
	...
	public AbstractID3v2Frame(final String identifier) {
		...
		try {
			final Class<AbstractID3v2FrameBody> c = (Class<AbstractID3v2FrameBody>) Class.forName("org.jaudiotagger.tag.id3.framebody.FrameBody" + identifier);
	 ...
	 */
	protected void setLoggingFilename(final String loggingFilename) {
		this.loggingFilename = loggingFilename;
	}

	/**
	 * Return the frame identifier, this only identifies the frame it does not provide a unique key, when using frames
	 * such as TXXX which are used by many fields *
	 * 
	 ...
	 */
	protected AbstractID3v2FrameBody readEncryptedBody(final String identifier, final ByteBuffer byteBuffer, final int frameSize) throws InvalidFrameException, InvalidDataTypeException {
		try {
			final AbstractID3v2FrameBody frameBody = new FrameBodyEncrypted(identifier, byteBuffer, frameSize);
			frameBody.setHeader(this);
			return frameBody;
		} catch (final InvalidTagException ite) {
			throw new InvalidDataTypeException(ite);
		}
	}

	protected boolean isPadding(final byte[] buffer) {
	...
	protected AbstractID3v2FrameBody readBody(final String identifier, final ByteBuffer byteBuffer, final int frameSize) throws InvalidFrameException, InvalidDataTypeException {
		// Use reflection to map id to frame body, which makes things much easier
		...
		try {
			final Class<AbstractID3v2FrameBody> c = (Class<AbstractID3v2FrameBody>) Class.forName("org.jaudiotagger.tag.id3.framebody.FrameBody" + identifier);
			final Class<?>[] constructorParameterTypes = { Class.forName("java.nio.ByteBuffer"), Integer.TYPE };
			final Object[] constructorParameterValues = { byteBuffer, frameSize };
			final Constructor<AbstractID3v2FrameBody> construct = c.getConstructor(constructorParameterTypes);
		...
		// No class defined for this frame type,use FrameUnsupported
			...
			// read method to declare it can throw InvalidtagException
				...
				throw ife;
			} catch (final InvalidTagException te) {
	 ...
	 * @throws InvalidFrameException
	 */
	protected String readIdentifier(final ByteBuffer byteBuffer) throws PaddingException, InvalidFrameException {
		final byte[] buffer = new byte[getFrameIdSize()];
	...
	/**
	 * This creates a new body based of type identifier but populated by the data in the body. This is a different type
	 * to the body being created which is why TagUtility.copyObject() can't be used. This is used when converting
	 * between different versions of a tag for frames that have a non-trivial mapping such as TYER in v3 to TDRC in v4.
	 * This will only work where appropriate constructors exist in the frame body to be created, for example a
	 * FrameBodyTYER requires a constructor consisting of a FrameBodyTDRC.
	 * <p/>
	 * If this method is called and a suitable constructor does not exist then an InvalidFrameException will be thrown
	 * 
	 * @param identifier
	 *            to determine type of the frame
	 * @param body
	 * @return newly created framebody for this type
	 * @throws InvalidFrameException
	 *             if unable to construct a framebody for the identifier and body provided.
	 */
	@SuppressWarnings("unchecked")
	// TODO using reflection is rather slow perhaps we should change this
	protected AbstractID3v2FrameBody readBody(final String identifier, final AbstractID3v2FrameBody body) throws InvalidFrameException {
		/*
		 * Use reflection to map id to frame body, which makes things much easier to keep things up to date, although
		 * slight performance hit.
		 */
		AbstractID3v2FrameBody frameBody;
		try {
			final Class<AbstractID3v2FrameBody> c = (Class<AbstractID3v2FrameBody>) Class.forName("org.jaudiotagger.tag.id3.framebody.FrameBody" + identifier);
			final Class<?>[] constructorParameterTypes = { body.getClass() };
			final Object[] constructorParameterValues = { body };
			final Constructor<AbstractID3v2FrameBody> construct = c.getConstructor(constructorParameterTypes);
			frameBody = (construct.newInstance(constructorParameterValues));
		} catch (final ClassNotFoundException cex) {
	...
	public boolean isEmpty() {
		final AbstractTagFrameBody body = this.getBody();
		if (body == null)
			return true;
	...
	public class StatusFlags {
		...
		public boolean equals(final Object obj) {
			...
			if (!(obj instanceof StatusFlags))
				return false;
			final StatusFlags that = (StatusFlags) obj;
	...
	class EncodingFlags {
		...
		protected EncodingFlags(final byte flags) {
			setFlags(flags);
		}
		...
		public boolean equals(final Object obj) {
			...
			if (!(obj instanceof EncodingFlags))
				return false;
			final EncodingFlags that = (EncodingFlags) obj;
	...
	public boolean equals(final Object obj) {
		...
		final AbstractID3v2Frame that = (AbstractID3v2Frame) obj;
	 ...
	 * 
	 * For frames consisting of different fields, this will return the value deemed to be most likely to be required
	 * 
	 * @return Content
	 */
	@Override
