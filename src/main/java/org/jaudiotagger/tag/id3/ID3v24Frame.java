...
public class ID3v24Frame extends AbstractID3v2Frame {
	...
	/**
	 * Creates a new ID3v2_4Frame of type identifier. An empty body of the correct type will be automatically created.
	 * This constructor should be used when wish to create a new frame from scratch using user input
	 * 
	 * @param identifier
	 *            defines the type of body to be created
	 */
	public ID3v24Frame(final String identifier) {
	 ...
	 * @param frame
	 */
	public ID3v24Frame(final ID3v24Frame frame) {
		super(frame);
	...
	private void createV24FrameFromV23Frame(final ID3v23Frame frame) throws InvalidFrameException {
	...
	/**
	 * Creates a new ID3v2_4Frame datatype based on another frame of different version Converts the framebody to the
	 * equivalent v24 framebody or to UnsupportedFrameBody if identifier is unknown.
	 * 
	 * @param frame
	 *            to construct a new frame from
	 * @throws org.jaudiotagger.tag.InvalidFrameException
	 * 
	 */
	public ID3v24Frame(final AbstractID3v2Frame frame) throws InvalidFrameException {
		...
		if (frame instanceof ID3v23Frame)
			...
			final ID3v23Frame v23Frame = new ID3v23Frame(frame);
	 ...
	 */
	public ID3v24Frame(final Lyrics3v2Field field) throws InvalidTagException {
		final String id = field.getIdentifier();
		String value;
		if (id.equals("IND"))
			throw new InvalidTagException("Cannot create ID3v2.40 frame from Lyrics3 indications field.");
		else if (id.equals("LYR")) {
			final FieldFrameBodyLYR lyric = (FieldFrameBodyLYR) field.getBody();
			Lyrics3Line line;
			final Iterator<Lyrics3Line> iterator = lyric.iterator();
			FrameBodySYLT sync;
	 ...
	 * 
	 */
	public ID3v24Frame(final ByteBuffer byteBuffer, final String loggingFilename) throws InvalidFrameException, InvalidDataTypeException {
	 ...
	 */
	@Deprecated
	public ID3v24Frame(final ByteBuffer byteBuffer) throws InvalidFrameException, InvalidDataTypeException {
		this(byteBuffer, "");
	}
	...
	public boolean equals(final Object obj) {
		...
		if (!(obj instanceof ID3v24Frame))
			return false;
		final ID3v24Frame that = (ID3v24Frame) obj;
	...
	/**
	 * If frame is greater than certain size it will be decoded differently if unsynchronized to if synchronized Frames
	 * with certain byte sequences should be unsynchronized but sometimes editors do not unsynchronize them so this
	 * method checks both cases and goes with the option that fits best with the data
	 * 
	 * @param byteBuffer
	 * @throws InvalidFrameException
	 */
	private void checkIfFrameSizeThatIsNotSyncSafe(final ByteBuffer byteBuffer) throws InvalidFrameException {
		if (frameSize > ID3SyncSafeInteger.MAX_SAFE_SIZE) {
			...
			final int currentPosition = byteBuffer.position();

			// Read as nonsync safe integer
			byteBuffer.position(currentPosition - getFrameIdSize());
			final int nonSyncSafeFrameSize = byteBuffer.getInt();
			...
			byteBuffer.position(currentPosition - getFrameIdSize());
			final boolean isNotSyncSafe = ID3SyncSafeInteger.isBufferNotSyncSafe(byteBuffer);
			...
			if (isNotSyncSafe) {
				...
				if (byteBuffer.remaining() < getFrameIdSize())
					...
					else // size returned will be larger so is it valid
					if (nonSyncSafeFrameSize > byteBuffer.remaining() - getFrameFlagsSize())
	 ...
	 * @throws InvalidFrameException
	 */
	private void getFrameSize(final ByteBuffer byteBuffer) throws InvalidFrameException {
	 ...
	 * 
	 * @param byteBuffer
	 *            to read the frame from
	 */
	@Override
	public void read(final ByteBuffer byteBuffer) throws InvalidFrameException, InvalidDataTypeException {
		final String identifier = readIdentifier(byteBuffer);
		...
		final int realFrameSize = frameSize - extraHeaderBytesCount;
	 ...
	 */
	@Override
	public void write(final ByteArrayOutputStream tagBuffer) {
		...
		// write bodybuffer
		final ByteBuffer headerBuffer = ByteBuffer.allocate(FRAME_HEADER_SIZE);
		...
		final int size = bodyBuffer.length;
	...
	/**
	 * Member Class This represents a frame headers Status Flags Make adjustments if necessary based on frame type and
	 * specification.
	 */
	class StatusFlags extends AbstractID3v2Frame.StatusFlags {
		 ...
		 * @param statusFlags
		 */
		StatusFlags(final ID3v23Frame.StatusFlags statusFlags) {
		 ...
		 * @return
		 */
		private byte convertV3ToV4Flags(final byte v3Flag) {
			byte v4Flag = (byte) 0;
	...
	class EncodingFlags extends AbstractID3v2Frame.EncodingFlags {
		 ...
		 * @param flags
		 */
		EncodingFlags(final byte flags) {
			super(flags);
			logEnabledFlags();
		}
		...
		public boolean isNonStandardFlags() {
			return ((flags & FileConstants.BIT7) > 0) || ((flags & FileConstants.BIT5) > 0) || ((flags & FileConstants.BIT4) > 0);
		}
	...
	/**
	 * Does the frame identifier meet the syntax for a idv3v2 frame identifier. must start with a capital letter and
	 * only contain capital letters and numbers
	 * 
	 * @param identifier
	 *            to be checked
	 * @return whether the identifier is valid
	 */
	public boolean isValidID3v2FrameIdentifier(final String identifier) {
		final Matcher m = ID3v24Frame.validFrameIdentifier.matcher(identifier);
		return m.matches();
	}
	...
	public void setEncoding(final String encoding) {
		final Integer encodingId = TextEncoding.getInstanceOf().getIdForValue(encoding);
		if (encoding != null)
			if (encodingId < 4)
