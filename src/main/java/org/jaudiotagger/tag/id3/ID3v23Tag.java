...
public class ID3v23Tag extends AbstractID3v2Tag {
	...
	protected void copyPrimitives(final AbstractID3v2Tag copyObj) {
		...
		if (copyObj instanceof ID3v23Tag) {
			final ID3v23Tag copyObject = (ID3v23Tag) copyObj;
	...
	protected void addFrame(final AbstractID3v2Frame frame) {
		try {
			...
			if ((frame.getIdentifier().equals(ID3v24Frames.FRAME_ID_YEAR)) && (frame.getBody() instanceof FrameBodyTDRC))
				...
				final ID3v23Frame newFrame = new ID3v23Frame(frame);
	...
	/**
	 * This is used when we need to translate a single frame into multiple frames, currently required for v24 TDRC
	 * frames.
	 * 
	 * @param frame
	 */
	// TODO will overwrite any existing TYER or TIME frame, do we ever want multiples of these
	protected void translateFrame(final AbstractID3v2Frame frame) {
		final FrameBodyTDRC tmpBody = (FrameBodyTDRC) frame.getBody();
	 ...
	 * @param copyObject
	 */
	public ID3v23Tag(final ID3v23Tag copyObject) {
	 ...
	 * @param mp3tag
	 */
	public ID3v23Tag(final AbstractTag mp3tag) {
	 ...
	 * @throws TagException
	 */
	public ID3v23Tag(final ByteBuffer buffer, final String loggingFilename) throws TagException {
	 ...
	 */
	@Deprecated
	public ID3v23Tag(final ByteBuffer buffer) throws TagException {
		this(buffer, "");
	}
	...
	/**
	 * Return frame size based upon the sizes of the tags rather than the physical no of bytes between start of ID3Tag
	...
	public boolean equals(final Object obj) {
		if (!(obj instanceof ID3v23Tag))
			return false;
		final ID3v23Tag object = (ID3v23Tag) obj;
	 ...
	 * @throws TagException
	 */
	private void readHeaderFlags(final ByteBuffer buffer) throws TagException {
		// Allowable Flags
	 ...
	 * @param size
	 */
	private void readExtendedHeader(final ByteBuffer buffer, int size) {
		// Int is 4 bytes.
		final int extendedHeaderSize = buffer.getInt();
	 ...
	 */
	@Override
	public void read(final ByteBuffer buffer) throws TagException {
		int size;
		if (!seek(buffer))
	 ...
	 * 
	 * @param padding
	 *            is the size of the padding portion of the tag
	 * @param size
	...
	private ByteBuffer writeHeaderToBuffer(final int padding, final int size) throws IOException {
		...
		final ByteBuffer headerBuffer = ByteBuffer.allocate(TAG_HEADER_LENGTH + TAG_EXT_HEADER_LENGTH + TAG_EXT_HEADER_CRC_LENGTH);
	 ...
	 * Write tag to file
	 * <p/>
	 * TODO:we currently never write the Extended header , but if we did the size calculation in this method would be
	 * slightly incorrect
	 * 
	 * @param file
	...
	@Override
	public ID3v23Frame createFrame(final String id) {
	 ...
	 */
	public TagField createField(final ID3v23FieldKey id3Key, final String value) throws KeyNotFoundException, FieldDataInvalidException {
		if (id3Key == null)
	 ...
	 */
	public String getFirst(final ID3v23FieldKey id3v23FieldKey) throws KeyNotFoundException {
		if (id3v23FieldKey == null)
			throw new KeyNotFoundException();

		final FieldKey genericKey = ID3v23Frames.getInstanceOf().getGenericKeyFromId3(id3v23FieldKey);
		if (genericKey != null)
			return super.getFirst(genericKey);
		else {
			final FrameAndSubId frameAndSubId = new FrameAndSubId(id3v23FieldKey.getFrameId(), id3v23FieldKey.getSubId());
	 ...
	 * @throws org.jaudiotagger.tag.KeyNotFoundException
	 */
	public void deleteField(final ID3v23FieldKey id3v23FieldKey) throws KeyNotFoundException {
		if (id3v23FieldKey == null)
			throw new KeyNotFoundException();
	...
	protected FrameAndSubId getFrameAndSubIdFromGenericKey(final FieldKey genericKey) {
		final ID3v23FieldKey id3v23FieldKey = ID3v23Frames.getInstanceOf().getId3KeyFromGenericKey(genericKey);
		if (id3v23FieldKey == null)
			throw new KeyNotFoundException();
	...
	public TagField createField(final FieldKey genericKey, final String value) throws KeyNotFoundException, FieldDataInvalidException {
		...
		if (value.length() < 4)
			...
			final AbstractID3v2Frame tyer = createFrame(ID3v23Frames.FRAME_ID_V3_TYER);
			...
			if (value.length() >= 10) {
				...
				final String day = value.substring(8, 10);
				final AbstractID3v2Frame tdat = createFrame(ID3v23Frames.FRAME_ID_V3_TDAT);
				((AbstractFrameBodyTextInfo) tdat.getBody()).setText(day + month);

				final TyerTdatAggregatedFrame ag = new TyerTdatAggregatedFrame();
				ag.addFrame(tyer);
				ag.addFrame(tdat);
				return ag;
	...
	public String getValue(final FieldKey genericKey, final int index) throws KeyNotFoundException {
		...
		final AggregatedFrame af = (AggregatedFrame) getFrame(TyerTdatAggregatedFrame.ID_TYER_TDAT);
		if (af != null)
	...
	@Override
	protected void loadFrameIntoSpecifiedMap(final HashMap<String, Object> map, final String frameId, final AbstractID3v2Frame frame) {
