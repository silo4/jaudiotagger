...
public class ID3v1Tag extends AbstractID3v1Tag implements Tag {
	...
	public ID3v1Tag(final ID3v1Tag copyObject) {
		super(copyObject);
	...
	public ID3v1Tag(final AbstractTag mp3tag) {
	 ...
	 * @throws IOException
	 */
	public ID3v1Tag(final RandomAccessFile file, final String loggingFilename) throws TagNotFoundException, IOException {
	 ...
	 */
	@Deprecated
	public ID3v1Tag(final RandomAccessFile file) throws TagNotFoundException, IOException {
		this(file, "");
	}
	...
	protected List<TagField> returnFieldToList(final ID3v1TagField field) {
		final List<TagField> fields = new ArrayList<TagField>();
		fields.add(field);
		return fields;
	}
	...
	public List<TagField> getAlbum() {
		if (getFirstAlbum().length() > 0) {
			final ID3v1TagField field = new ID3v1TagField(ID3v1FieldKey.ALBUM.name(), getFirstAlbum());
	...
	public List<TagField> getArtist() {
		if (getFirstArtist().length() > 0) {
			final ID3v1TagField field = new ID3v1TagField(ID3v1FieldKey.ARTIST.name(), getFirstArtist());
			return returnFieldToList(field);
		} else
			return new ArrayList<TagField>();
	}
	...
	public List<TagField> getComment() {
		if (getFirstComment().length() > 0) {
			final ID3v1TagField field = new ID3v1TagField(ID3v1FieldKey.COMMENT.name(), getFirstComment());
			return returnFieldToList(field);
		} else
			return new ArrayList<TagField>();
	}
	 ...
	 * <p/>
	 * <p>
	 * ID3v1 only supports genres defined in a predefined list so if unable to find value in list set 255, which seems
	 * to be the value winamp uses for undefined.
	 * 
	 * @param genreVal
	 */
	public void setGenre(final String genreVal) {
		if (genreVal == null)
			throw new IllegalArgumentException(ErrorMessage.GENERAL_INVALID_NULL_ARGUMENT.getMsg());
		final Integer genreID = GenreTypes.getInstanceOf().getIdForValue(genreVal);
		if (genreID != null)
	...
	public String getFirstGenre() {
		final Integer genreId = genre & BYTE_TO_UNSIGNED;
		final String genreValue = GenreTypes.getInstanceOf().getValueForId(genreId);
		if (genreValue == null)
			return "";
		else
			return genreValue;
	...
	public List<TagField> getGenre() {
		if (getFirst(FieldKey.GENRE).length() > 0) {
			final ID3v1TagField field = new ID3v1TagField(ID3v1FieldKey.GENRE.name(), getFirst(FieldKey.GENRE));
			return returnFieldToList(field);
		} else
			return new ArrayList<TagField>();
	}
	...
	public List<TagField> getTitle() {
		if (getFirst(FieldKey.TITLE).length() > 0) {
			final ID3v1TagField field = new ID3v1TagField(ID3v1FieldKey.TITLE.name(), getFirst(FieldKey.TITLE));
			return returnFieldToList(field);
		} else
			return new ArrayList<TagField>();
	}
	...
	public List<TagField> getYear() {
		if (getFirst(FieldKey.YEAR).length() > 0) {
			final ID3v1TagField field = new ID3v1TagField(ID3v1FieldKey.YEAR.name(), getFirst(FieldKey.YEAR));
			return returnFieldToList(field);
		} else
			return new ArrayList<TagField>();
	}
	...
	public boolean hasField(final String id) {
		try {
			final FieldKey key = FieldKey.valueOf(id.toUpperCase());
			return hasField(key);
	...
	public void setField(final TagField field) {
		final FieldKey genericKey = FieldKey.valueOf(field.getId());
		switch (genericKey) {
		case ARTIST:
	 ...
	 */
	@Override
	public TagField createField(final FieldKey genericKey, final String value) {
		final ID3v1FieldKey idv1FieldKey = tagFieldToID3v1Field.get(genericKey);
	...
	public TagField getFirstField(final FieldKey genericKey) {
		final List<TagField> l = getFields(genericKey);
	 ...
	 */
	@Override
	public String getFirst(final String genericKey) {
		final FieldKey matchingKey = FieldKey.valueOf(genericKey);
		if (matchingKey != null)
	 ...
	 * @return
	 */
	public String getSubValue(final FieldKey id, final int n, final int m) {
	...
	public void deleteField(final String id) {
		final FieldKey key = FieldKey.valueOf(id);
		if (key != null)
			deleteField(key);
	}
	...
	public boolean equals(final Object obj) {
		if (!(obj instanceof ID3v1Tag))
			return false;
		final ID3v1Tag object = (ID3v1Tag) obj;
	 ...
	 */
	@Override
	public void read(final ByteBuffer byteBuffer) throws TagNotFoundException {
		if (!seek(byteBuffer))
		...
		final byte[] dataBuffer = new byte[TAG_LENGTH];
	...
	public void write(final RandomAccessFile file) throws IOException {
		...
		final byte[] buffer = new byte[TAG_LENGTH];
		int i;
		String str;
		delete(file);
