...
/**
 * Represents common image formats support by ID3 and provides a mapping between the format field supported in ID3v22
 * and the mimetype field supported by ID3v23/ID3v24.
 * 
 * 
 * Note only JPG and PNG are mentioned specifically in the ID3 v22 Spec but it only says 'Image Format is preferably PNG
 * or JPG' , not mandatory. In the jaudiotagger library we also consider GIF as a portable format, and we recognise
...
public class ImageFormats {
	...
	static {
		...
		String value;
		for (final String key : imageFormatsToMimeType.keySet()) {
	 ...
	 * @return
	 */
	public static String getMimeTypeForFormat(final String format) {
	 ...
	 * @return
	 */
	public static String getFormatForMimeType(final String mimeType) {
	 ...
	 */
	public static boolean binaryDataIsPngFormat(final byte[] data) {
		// Read signature
		if (data.length < 4)
			return false;
	 ...
	 */
	public static boolean binaryDataIsJpgFormat(final byte[] data) {
		if (data.length < 4)
			return false;
		// Read signature
	 ...
	 */
	public static boolean binaryDataIsGifFormat(final byte[] data) {
		if (data.length < 3)
			return false;
		// Read signature
	 ...
	 */
	public static boolean binaryDataIsBmpFormat(final byte[] data) {
		// Read signature
	 ...
	 */
	public static boolean binaryDataIsPdfFormat(final byte[] data) {
		// Read signature
	 ...
	 */
	public static boolean binaryDataIsTiffFormat(final byte[] data) {
		// Read signature Intel
		return (((0x49 == (data[0] & 0xff)) && (0x49 == (data[1] & 0xff)) && (0x2a == (data[2] & 0xff)) && (0x00 == (data[3] & 0xff))) || ((0x4d == (data[0] & 0xff)) && (0x4d == (data[1] & 0xff)) && (0x00 == (data[2] & 0xff)) && (0x2a == (data[3] & 0xff))));
	}
	 ...
	 */
	public static boolean isPortableFormat(final byte[] data) {
	 ...
	 */
	public static String getMimeTypeForBinarySignature(final byte[] data) {
