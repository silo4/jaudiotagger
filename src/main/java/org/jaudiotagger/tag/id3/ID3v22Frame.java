...
public class ID3v22Frame extends AbstractID3v2Frame {
	 ...
	 * 
	 * @param body
	 *            New body and frame is based on this
	 */
	public ID3v22Frame(final AbstractID3v2FrameBody body) {
		super(body);
	}

	/**
	 * Compare for equality To be deemed equal obj must be a IDv23Frame with the same identifier and the same flags.
	 * containing the same body,datatype list ectera. equals() method is made up from all the various components
	 * 
	 * @param obj
	...
	public boolean equals(final Object obj) {
		...
		if (!(obj instanceof ID3v22Frame))
			return false;
		final ID3v22Frame that = (ID3v22Frame) obj;
	...
	public ID3v22Frame(final String identifier) {
		...
		try {
			final Class<AbstractID3v2FrameBody> c = (Class<AbstractID3v2FrameBody>) Class.forName("org.jaudiotagger.tag.id3.framebody.FrameBody" + bodyIdentifier);
	 ...
	 * @param frame
	 */
	public ID3v22Frame(final ID3v22Frame frame) {
		super(frame);
		logger.config("Creating frame from a frame of same version");
	}

	private void createV22FrameFromV23Frame(final ID3v23Frame frame) throws InvalidFrameException {
	 ...
	 * 
	 * @param frame
	 *            to construct the new frame from
	 * @throws org.jaudiotagger.tag.InvalidFrameException
	 */
	public ID3v22Frame(final AbstractID3v2Frame frame) throws InvalidFrameException {
	 ...
	 * @throws org.jaudiotagger.tag.InvalidFrameException
	 */
	public ID3v22Frame(final ByteBuffer byteBuffer, final String loggingFilename) throws InvalidFrameException, InvalidDataTypeException {
	 ...
	 */
	@Deprecated
	public ID3v22Frame(final ByteBuffer byteBuffer) throws InvalidFrameException, InvalidDataTypeException {
		this(byteBuffer, "");
	}
	...
	public void read(final ByteBuffer byteBuffer) throws InvalidFrameException, InvalidDataTypeException {
		...
		final byte[] buffer = new byte[getFrameSizeSize()];
		...
		if (frameSize < 0)
			...
			final ByteBuffer frameBodyBuffer = byteBuffer.slice();
	 ...
	 * @return
	 */
	private int decodeSize(final byte[] buffer) {
		final BigInteger bi = new BigInteger(buffer);
		final int tmpSize = bi.intValue();
		if (tmpSize < 0)
	...
	public void write(final ByteArrayOutputStream tagBuffer) {
		...
		// write body
		final ByteBuffer headerBuffer = ByteBuffer.allocate(getFrameHeaderSize());
	 ...
	 * @param size
	 */
	private void encodeSize(final ByteBuffer headerBuffer, final int size) {
	...
	public boolean isValidID3v2FrameIdentifier(final String identifier) {
		final Matcher m = ID3v22Frame.validFrameIdentifier.matcher(identifier);
		return m.matches();
	}
