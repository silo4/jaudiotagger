...
public class ID3v11Tag extends ID3v1Tag {
	...
	public ID3v11Tag(final ID3v11Tag copyObject) {
		super(copyObject);
		...
		if (mp3tag != null)
			if (mp3tag instanceof ID3v1Tag) {
				...
				if (id3tag.hasFrame(ID3v24Frames.FRAME_ID_COMMENT)) {
					final Iterator iterator = id3tag.getFrameOfType(ID3v24Frames.FRAME_ID_COMMENT);
					text = "";
	 ...
	 * @throws IOException
	 */
	public ID3v11Tag(final RandomAccessFile file, final String loggingFilename) throws TagNotFoundException, IOException {
		setLoggingFilename(loggingFilename);
		FileChannel fc;
		final ByteBuffer byteBuffer = ByteBuffer.allocate(TAG_LENGTH);
	 ...
	 */
	@Deprecated
	public ID3v11Tag(final RandomAccessFile file) throws TagNotFoundException, IOException {
		this(file, "");
	...
	/**
	 * Set the track, v11 stores track numbers in a single byte value so can only handle a simple number in the range
	 * 0-255.
	 * 
	 * @param trackValue
	 */

	public void setTrack(final String trackValue) {
		int trackAsInt;
	...
	public List<TagField> getTrack() {
		if (getFirst(FieldKey.TRACK).length() > 0) {
			final ID3v1TagField field = new ID3v1TagField(ID3v1FieldKey.TRACK.name(), getFirst(FieldKey.TRACK));
	...
	public boolean equals(final Object obj) {
		if (!(obj instanceof ID3v11Tag))
			return false;
		final ID3v11Tag object = (ID3v11Tag) obj;
	 ...
	 * @throws TagNotFoundException
	 *             if unable to read a tag in the byteBuffer
	 */
	@Override
	 ...
	 * @throws IOException
	 *             thrown if there were problems writing to the file
	 */
	@Override
