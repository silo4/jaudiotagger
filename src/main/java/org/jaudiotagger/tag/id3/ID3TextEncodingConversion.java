...
public class ID3TextEncodingConversion {
	 ...
	 * 
	 * @param header
	 *            used to identify the ID3tagtype
	 * @param textEncoding
	 *            currently set
	 * @return valid encoding according to version type and user options
	 */
	public static byte getTextEncoding(final AbstractTagFrame header, final byte textEncoding) {
		...
		if (header == null) {
			...
			if (TagOptionSingleton.getInstance().isResetTextEncodingForExistingFrames())
				...
				return textEncoding;
		} else if (TagOptionSingleton.getInstance().isResetTextEncodingForExistingFrames())
			// Replace with default
			return TagOptionSingleton.getInstance().getId3v23DefaultTextEncoding();
		else
	 ...
	 * @return
	 */
	public static byte getUnicodeTextEncoding(final AbstractTagFrame header) {
		if (header == null) {
			logger.warning("Header has not yet been set for this framebody");
	 ...
	 */
	private static byte convertV24textEncodingToV23textEncoding(final byte textEncoding) {
