...
public class ID3v24Tag extends AbstractID3v2Tag {
	...
	/**
	 * ID3v2.4 Header Footer are the same as the header flags. WHY?!?! move the flags from thier position in
	 * 2.3??????????
	 */
	/**
	 ...
	 * 
	 * 0 No restrictions 1 Images are encoded only with PNG [PNG] or JPEG [JFIF].
	 */
	 ...
	 * 
	 * 00 No restrictions 01 All images are 256x256 pixels or smaller. 10 All images are 64x64 pixels or smaller. 11 All
	 * images are exactly 64x64 pixels, unless required otherwise.
	 */
	 ...
	 * 
	 * 00 No more than 128 frames and 1 MB total tag size. 01 No more than 64 frames and 128 KB total tag size. 10 No
	 * more than 32 frames and 40 KB total tag size. 11 No more than 32 frames and 4 KB total tag size.
	 */
	 ...
	 * 
	 * 00 No restrictions 01 No string is longer than 1024 characters. 10 No string is longer than 128 characters. 11 No
	 * string is longer than 30 characters.
	 * 
	 * Note that nothing is said about how many bytes is used to represent those characters, since it is encoding
	 * dependent. If a text frame consists of more than one string, the sum of the strungs is restricted as stated.
	 */
	...
	/**
	 * Copy primitives applicable to v2.4, this is used when cloning a v2.4 datatype and other objects such as v2.3 so
	 * need to check instanceof
	 */
	@Override
	protected void copyPrimitives(final AbstractID3v2Tag copyObj) {
		logger.config("Copying primitives");
		super.copyPrimitives(copyObj);

		if (copyObj instanceof ID3v24Tag) {
			final ID3v24Tag copyObject = (ID3v24Tag) copyObj;
	...
	@Override
	protected void addFrame(final AbstractID3v2Frame frame) {
		try {
			if (frame instanceof ID3v24Frame)
				copyFrameIntoMap(frame.getIdentifier(), frame);
			else {
				final ID3v24Frame newFrame = new ID3v24Frame(frame);
				copyFrameIntoMap(newFrame.getIdentifier(), newFrame);
			}
		} catch (final InvalidFrameException ife) {
			logger.log(Level.SEVERE, "Unable to convert frame:" + frame.getIdentifier());
		}
	}

	/*
	 * Copy frame into map, whilst accounting for multiple frames of same type which can occur even if there were not
	 * frames of the dame type in the original tag
	 * 
	 * The frame already exists this shouldn't normally happen because frames that are allowed to be multiple don't call
	 * this method. Frames that aren't allowed to be multiple aren't added to hashMap in first place when originally
	 * added.
	 * 
	 * We only want to allow one of the frames going forward but we try and merge all the information into the one
	 * frame. However there is a problem here that if we then take this, modify it and try to write back the original
	 * values we could lose some information although this info is probably invalid anyway.
	 * 
	 * However converting some frames from tag of one version to another may mean that two different frames both get
	 * converted to one frame, this particularly applies to DateTime fields which were originally two fields in v2.3 but
	 * are one field in v2.4.
	 */
	@Override
	protected void copyFrameIntoMap(final String id, final AbstractID3v2Frame newFrame) {
		...
		if (frameMap.containsKey(newFrame.getIdentifier())) {
			...
			if (o instanceof AbstractID3v2Frame) {
				// Retrieve the frame with the same id we have already loaded
				// into the map
				final AbstractID3v2Frame firstFrame = (AbstractID3v2Frame) frameMap.get(newFrame.getIdentifier());

				// Two different frames both converted to TDRCFrames, now if
				// this is the case one of them
				// may have actually have been created as a FrameUnsupportedBody
				// because TDRC is only
				// supported in ID3v24, but is often created in v23 tags as well
				// together with the valid TYER
				// frame OR it might be that we have two v23 frames that map to
				// TDRC such as TYER,TIME or TDAT
				if (newFrame.getBody() instanceof FrameBodyTDRC) {
					if (firstFrame.getBody() instanceof FrameBodyTDRC) {
						...
						final FrameBodyTDRC body = (FrameBodyTDRC) firstFrame.getBody();
						final FrameBodyTDRC newBody = (FrameBodyTDRC) newFrame.getBody();
					...
					// The first frame was a TDRC frame that was not really
					// allowed, this new frame was probably a
					// valid frame such as TYER which has been converted to
					// TDRC, replace the firstframe with this frame
					else if (firstFrame.getBody() instanceof FrameBodyUnsupported)
						frameMap.put(newFrame.getIdentifier(), newFrame);
					else
						// we just lose this frame, weve already got one with
						// the correct id.
	 ...
	 * @param copyObject
	 */
	public ID3v24Tag(final ID3v24Tag copyObject) {
	...
	public ID3v24Tag(final AbstractTag mp3tag) {
		...
		if (mp3tag != null)
			...
			// IDv1
				...
				// convert id3v1 tags.
				final ID3v1Tag id3tag = (ID3v1Tag) mp3tag;
				...
				if (((id3tag.genre & ID3v1Tag.BYTE_TO_UNSIGNED) >= 0) && ((id3tag.genre & ID3v1Tag.BYTE_TO_UNSIGNED) != ID3v1Tag.BYTE_TO_UNSIGNED)) {
					final Integer genreId = id3tag.genre & ID3v1Tag.BYTE_TO_UNSIGNED;
					final String genre = "(" + genreId + ") " + GenreTypes.getInstanceOf().getValueForId(genreId);
				...
				if (mp3tag instanceof ID3v11Tag) {
					final ID3v11Tag id3tag2 = (ID3v11Tag) mp3tag;
					if (id3tag2.track > 0) {
			...
			// Lyrics 3
				...
				while (iterator.hasNext())
					try {
						...
						frameMap.put(newFrame.getIdentifier(), newFrame);
					} catch (final InvalidTagException ex) {
	 ...
	 * @throws TagException
	 */
	public ID3v24Tag(final ByteBuffer buffer, final String loggingFilename) throws TagException {
	 ...
	 */
	@Deprecated
	public ID3v24Tag(final ByteBuffer buffer) throws TagException {
		this(buffer, "");
	}
	...
	/**
	 * Return tag size based upon the sizes of the frames rather than the physical no of bytes between start of ID3Tag
	 * and start of Audio Data.
	 * 
	 * @return size
	 */
	@Override
	...
	public boolean equals(final Object obj) {
		if (!(obj instanceof ID3v24Tag))
			return false;
		final ID3v24Tag object = (ID3v24Tag) obj;
	 ...
	 * @throws TagException
	 */
	private void readHeaderFlags(final ByteBuffer byteBuffer) throws TagException {
		// Flags
		final byte flags = byteBuffer.get();
	 ...
	 */
	private void readExtendedHeader(final ByteBuffer byteBuffer, final int size) throws InvalidTagException {
		byte[] buffer;

		// int is 4 bytes.
		final int extendedHeaderSize = byteBuffer.getInt();
		...
		final byte extFlag = byteBuffer.get();
	 ...
	 * @param size
	 */
	protected void readFrames(final ByteBuffer byteBuffer, final int size) {
		...
		while (byteBuffer.position() <= size) {
			...
			// Failed reading frame but may just have invalid data but correct
			// length so lets carry on
			// in case we can read the next frame
			catch (final InvalidDataTypeException idete) {
	 ...
	 * 
	 * @param padding
	 *            is the size of the padding
	 * @param size
	 *            is the size of the body data
	 * @return ByteBuffer
	 * @throws IOException
	 */
	private ByteBuffer writeHeaderToBuffer(final int padding, final int size) throws IOException {
		// This would only be set if every frame in tag has been unsynchronized,
		// I only unsychronize frames
		// that need it, in any case I have been advised not to set it even
		// then.
		unsynchronization = false;

		// Flags,currently we never calculate the CRC
		// and if we dont calculate them cant keep orig values. Tags are not
		// experimental and we never create extended header to keep things
		// simple.
		extended = false;
		experimental = false;
		footer = false;
		...
		final ByteBuffer headerBuffer = ByteBuffer.allocate(TAG_HEADER_LENGTH);
		// TAGID
		...
		// Additional Header Size,(for completeness we never actually write the
	 ...
	 */
	@Override
	public void write(final Path file, final long audioStartLocation) throws IOException {
		setLoggingFilename(file.getFileName().toString());
		logger.config("Writing tag to file:" + getLoggingFilename());

		// Write Body Buffer
		final byte[] bodyByteBuffer = writeFramesToBuffer().toByteArray();

		// Calculate Tag Size including Padding
		final int sizeIncPadding = calculateTagSize(bodyByteBuffer.length + TAG_HEADER_LENGTH, (int) audioStartLocation);

		// Calculate padding bytes required
		final int padding = sizeIncPadding - (bodyByteBuffer.length + TAG_HEADER_LENGTH);

		final ByteBuffer headerBuffer = writeHeaderToBuffer(padding, bodyByteBuffer.length);
	...
	public void write(final WritableByteChannel channel) throws IOException {
		...
		final byte[] bodyByteBuffer = writeFramesToBuffer().toByteArray();
		final ByteBuffer headerBuffer = writeHeaderToBuffer(0, bodyByteBuffer.length);
	 ...
	 * <p/>
	 * <p>
	 * Because synchronization occurs at the frame level it is not normally desirable to unsynchronize all frames and
	 * hence this flag is not normally set.
	 * 
	 ...
	 */
	@Override
	public ID3v24Frame createFrame(final String id) {
	 ...
	 */
	public TagField createField(final ID3v24FieldKey id3Key, final String value) throws KeyNotFoundException, FieldDataInvalidException {
		if (id3Key == null)
	 ...
	 */
	public String getFirst(final ID3v24FieldKey id3v24FieldKey) throws KeyNotFoundException {
		if (id3v24FieldKey == null)
			throw new KeyNotFoundException();

		final FieldKey genericKey = ID3v24Frames.getInstanceOf().getGenericKeyFromId3(id3v24FieldKey);
		if (genericKey != null)
			return super.getFirst(genericKey);
		else {
			final FrameAndSubId frameAndSubId = new FrameAndSubId(id3v24FieldKey.getFrameId(), id3v24FieldKey.getSubId());
	 ...
	 * @throws org.jaudiotagger.tag.KeyNotFoundException
	 */
	public void deleteField(final ID3v24FieldKey id3v24FieldKey) throws KeyNotFoundException {
		if (id3v24FieldKey == null)
			throw new KeyNotFoundException();
	...
	@Override
	protected FrameAndSubId getFrameAndSubIdFromGenericKey(final FieldKey genericKey) {
		final ID3v24FieldKey id3v24FieldKey = ID3v24Frames.getInstanceOf().getId3KeyFromGenericKey(genericKey);
		if (id3v24FieldKey == null)
	...
	public Comparator<String> getPreferredFrameOrderComparator() {
		return ID3v24PreferredFrameOrderComparator.getInstance();
	}
	...
	public List<Artwork> getArtworkList() {
		...
		for (final TagField next : coverartList) {
			final FrameBodyAPIC coverArt = (FrameBodyAPIC) ((AbstractID3v2Frame) next).getBody();
	 ...
	 * @return
	 */
	public TagField createArtworkField(final byte[] data, final String mimeType) {
		final AbstractID3v2Frame frame = createFrame(getFrameAndSubIdFromGenericKey(FieldKey.COVER_ART).getFrameId());
		final FrameBodyAPIC body = (FrameBodyAPIC) frame.getBody();
