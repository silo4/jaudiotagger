...
public enum MusicalKey {
	NOTE_A("A"), NOTE_B("B"), NOTE_C("C"), NOTE_D("D"), NOTE_E("E"), NOTE_F("F"), NOTE_G("G"), FLAT("b"), SHARP("#"), MINOR("m"), OFF_KEY("o");

	private String value;

	MusicalKey(final String value) {
		this.value = value;
	}
	...
	static {
		final EnumSet<MusicalKey> groundKey = EnumSet.of(NOTE_A, NOTE_B, NOTE_C, NOTE_D, NOTE_E, NOTE_F, NOTE_G);
		groundKeyMap = new HashMap<String, MusicalKey>(MusicalKey.values().length);
		for (final MusicalKey curr : groundKey)
			groundKeyMap.put(curr.getValue(), curr);
		final EnumSet<MusicalKey> halfKey = EnumSet.of(FLAT, SHARP, MINOR);
		halfKeyMap = new HashMap<String, MusicalKey>(MusicalKey.values().length);
		for (final MusicalKey curr : halfKey)
			halfKeyMap.put(curr.getValue(), curr);
	}

	public static boolean isValid(final String musicalKey) {
